...
- `superforecasting_agent/runtime/auth.py` extends `PROVIDER_REGISTRY` with every api-key
  profile it sees (skipping `copilot`, `kimi-coding`, `kimi-coding-cn`,
  `zai`, `openrouter`, `custom` — those need bespoke token resolution).
- `superforecasting_agent/runtime/models.py` extends `CANONICAL_PROVIDERS` and calls
  `profile.fetch_models()` inside `provider_model_ids()`.
- `superforecasting_agent/runtime/doctor.py` adds a `/models` health check for each
  `auth_type="api_key"` profile.
- `superforecasting_agent/runtime/config.py` injects every `env_var` into
  `OPTIONAL_ENV_VARS` so the setup wizard knows about it.
- `superforecasting_agent/runtime/runtime_provider.py` reads `profile.api_mode` as a fallback
