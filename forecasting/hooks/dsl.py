...
_SIGNALS: dict[str, tuple[Callable[[HookContext], Any], str, str]] = {
    "components.count": (lambda c: c.component_count, "number", "Number of pooled drivers in ensemble_components."),
    "components.present": (lambda c: c.has_components, "bool", "Whether any ensemble_components are present."),
    "evidence.count": (lambda c: c.evidence_count, "number", "Total evidence items on the question."),
    "reasons.up_present": (lambda c: c.has_reasons_up, "bool", "Whether reasons_up is populated."),
    "reasons.down_present": (lambda c: c.has_reasons_down, "bool", "Whether reasons_down is populated."),
    "reasons.complete": (lambda c: c.has_reasons_up and c.has_reasons_down and c.has_change_my_mind, "bool",
    ...
    "bounds.in_range": (lambda c: c.bounds_in_range, "bool", "All interval bounds sit within the question's OutcomeSpace bounds."),
    "bounds.width_ratio": (lambda c: c.interval_width_ratio if c.interval_width_ratio is not None else 0.0, "number", "Widest interval width as a fraction of the question range (absurd if large)."),
    # v2 — confidence lean
    "confidence.sharpness": (lambda c: c.sharpness if c.sharpness is not None else 1.0, "number", "Distance from maximum hedging: 0 = at 0.5 / flat PMF, 1 = committed."),
    "confidence.under_confident": (lambda c: c.calibration_under_confident, "bool", "Ledger measured chronic under-confidence for this scope."),
    "tails.null_excess": (lambda c: c.tail_null_excess, "number", "Probability mass above the null-model tail (overweighting no-path outcomes)."),
    # v2 — quorum / panel participation
    "panel.perspective_count": (lambda c: c.panel_perspective_count, "number", "Distinct perspectives/personas on the linked panel run."),
    "quorum.model_count": (lambda c: c.quorum_model_count, "number", "Distinct models in the linked quorum run."),
    "quorum.judged": (lambda c: c.quorum_judged, "bool", "The quorum run carries a judge synthesis (consensus + contradictions)."),
    # v2 — reasoning composition
    "reasoning.method_count": (lambda c: c.reasoning_method_count, "number", "Number of distinct declared reasoning methods."),
...
from forecasting.hooks.spec import REMEDIATION_ACTIONS as _REMEDIATION_ACTIONS  # single source of truth
...
class RuleSpec:
    ...
    def from_dict(d: dict[str, Any]) -> "RuleSpec":
        return RuleSpec(
            ...
            severity=str(d.get("severity") or "warn").lower(),
            applies_to=dict(d.get("applies_to") or {}),
            check=dict(d.get("check") or {}),
            message=str(d.get("message") or ""),
            remediation_hint=str(d.get("remediation_hint") or "none"),
        )


def _validate_predicate(node: Any, issues: list[RuleIssue], depth: int = 0) -> None:
    if depth > _MAX_DEPTH:
        issues.append(RuleIssue("check", "error", f"predicate nesting exceeds max depth {_MAX_DEPTH}", "flatten the conditions"))
        return
    if not isinstance(node, dict):
        issues.append(RuleIssue("check", "error", "each predicate must be an object", 'use {signal, op, value} or {all:[...]}'))
        return
    for combiner in ("all", "any"):
        if combiner in node:
            branches = node[combiner]
            if not isinstance(branches, list) or not branches:
                issues.append(RuleIssue("check", "error", f"`{combiner}` must be a non-empty list", f"add conditions under `{combiner}`"))
                return
            for b in branches:
    ...
    if signal not in _SIGNALS:
        issues.append(RuleIssue("check", "error", f"unknown signal {signal!r}",
                                "valid signals: " + ", ".join(SIGNAL_NAMES)))
        return
    kind = _SIGNALS[signal][1]
    if op not in ALL_OPS:
        issues.append(RuleIssue("check", "error", f"unknown operator {op!r}", "operators: " + ", ".join(sorted(ALL_OPS))))
        return
    if kind == "bool" and op not in _BOOL_OPS:
        issues.append(RuleIssue("check", "error", f"operator {op!r} is invalid for boolean signal {signal!r}",
                                "use is_true / is_false / == / !="))
    if kind == "number" and op in (_BOOL_OPS - {"==", "!="}):
        issues.append(RuleIssue("check", "error", f"operator {op!r} is invalid for numeric signal {signal!r}",
                                "use a comparison (>, >=, <, <=, ==, !=)"))
    if op in _SET_OPS and not isinstance(node.get("value"), list):
        issues.append(RuleIssue("check", "error", f"operator {op!r} needs a list value", "value: [a, b, c]"))
    if op in _NUMERIC_OPS - {"==", "!="} and kind == "number" and not isinstance(node.get("value"), (int, float)):
        issues.append(RuleIssue("check", "error", f"{signal!r} {op} needs a numeric value", "value: 3"))


def validate_rule(spec: RuleSpec, *, known_ids: set[str] | None = None) -> list[RuleIssue]:
    ...
    if not spec.id:
        issues.append(RuleIssue("id", "error", "rule id is required", "give the rule a short unique slug"))
    elif spec.id in BUILTIN_RULE_IDS:
        issues.append(RuleIssue("id", "error", f"id {spec.id!r} collides with a built-in rule", "choose a different slug"))
    elif spec.id in known_ids:
        issues.append(RuleIssue("id", "error", f"duplicate rule id {spec.id!r}", "ids must be unique"))

    if spec.severity not in ("off", "warn", "error"):
        issues.append(RuleIssue("severity", "error", f"severity must be off|warn|error, got {spec.severity!r}", ""))
    if spec.category not in _CATEGORIES:
        issues.append(RuleIssue("category", "warn", f"unknown category {spec.category!r}; treated as custom",
    ...
    if spec.severity == "error" and spec.remediation_hint == "none":
        issues.append(RuleIssue("remediation_hint", "warn",
...
def _applies(spec: RuleSpec) -> Callable[[HookContext], bool]:
    ...
    def applies(ctx: HookContext) -> bool:
        if "origin" in f and ctx.forecast_origin not in f["origin"]:
            return False
        if "impact" in f and (ctx.impact or "").lower() not in [str(x).lower() for x in f["impact"]]:
            return False
...
def compile_rule(spec: RuleSpec) -> SimpleRule:
    ...
    default_sev = Severity(spec.severity) if spec.severity in ("off", "warn", "error") else Severity.WARN
    msg = spec.message or f"user rule {spec.id} failed"
    hint = spec.remediation_hint if spec.remediation_hint in _REMEDIATION_ACTIONS else "none"
