...
def _get_agent_version() -> str:
    """Return the Superforecasting Agent package version, or 'dev'."""
    try:
        from importlib.metadata import version
        return version("superforecasting-agent")
    except Exception:
        try:
...
def build_user_agent() -> str:
    ...
    Format::
        ...
        QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; SuperforecastingAgent/<agent_version>)

    Example::

        QQBotAdapter/1.0.0 (Python/3.11.15; darwin; SuperforecastingAgent/0.9.0)
    """
    py_version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
    os_name = platform.system().lower()
    agent_version = _get_agent_version()
    return f"QQBotAdapter/{QQBOT_VERSION} (Python/{py_version}; {os_name}; SuperforecastingAgent/{agent_version})"
