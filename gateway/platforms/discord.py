...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, the active agent-home ``.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_agent_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="status", description="Show forecast research session status")
        ...
        @tree.command(name="stop", description="Stop the running forecast agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan forecast skills for new or removed skills")
        ...
        @tree.command(name="update", description="Update Superforecasting Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Superforecasting Agent gateway")
        ...
        @tree.command(name="thread", description="Create a thread and start a forecast research session")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Superforecasting Agent in the thread",
        ...
        # superforecasting_agent/runtime/commands.py automatically appear as Discord slash
        ...
        try:
            from superforecasting_agent.runtime.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from superforecasting_agent.runtime.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a forecast skill",
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from superforecasting_agent.runtime.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new forecast research session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Forecast thread created: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "Forecast Session"
        if len(content) > 80:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Forecast thread created: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def create_handoff_thread(
        ...
        reason = "Forecast research session handoff"
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Forecast handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``superforecasting-agent update`` prompts.
