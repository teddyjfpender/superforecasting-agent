...
Connects Superforecasting Agent to WeChat personal accounts via Tencent's iLink Bot API.
...
class WeixinAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        if self._group_policy != "disabled":
            logger.warning(
                ...
                "these accounts, so group messages may never reach Superforecasting Agent regardless of this "
                "policy. If group delivery doesn't work, the limitation is on the iLink side, "
                "not in Superforecasting Agent.",
                self.name,
                self._group_policy,
            )
        return True
    ...
    async def _poll_loop(self) -> None:
        ...
        sync_buf = _load_sync_buf(self._agent_home, self._account_id)
        ...
        while self._running:
            try:
                ...
                if new_sync_buf:
                    ...
                    _save_sync_buf(self._agent_home, self._account_id, sync_buf)
