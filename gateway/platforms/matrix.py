...
# Uses get_agent_home() so each profile gets its own Matrix store.
from superforecasting_agent.constants import get_agent_dir as _get_agent_dir

_STORE_DIR = _get_agent_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
        ...
        if self._access_token:
            ...
            try:
                resp = await client.login(
                    ...
                    device_name="Superforecasting Agent",
                    device_id=self._device_id or None,
                )
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Superforecasting Agent" become "Agent".
        """
        if not body:
            return ""
