...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded Superforecasting Agent but never re-ran
    ``superforecasting-agent setup`` to regenerate the unit),
    ``TimeoutStopSec`` can be smaller than the configured
    ...
    # constant in superforecasting_agent/runtime/gateway.py.
    headroom = 30.0
