...
In the TUI, `/questions` shows the active forecast book with numbered rows and `/questions <row>` drills into details without copying a forecast id. `/forecast` opens the structured forecast desk panel; `/new-forecast`, `/base-rate`, `/update-forecast`, `/resolve`, `/score`, `/postmortem`, `/review`, `/alerts`, `/self-check`, `/calibration`, `/lessons`, `/errors`, `/backtest`, `/schedule`, `/performance`, `/readiness`, `/doctor`, `/pilot-report`, `/pilot-cohort`, `/export-packet`, `/import-packet`, and `/pilot-aggregate` jump to common desk workflows; and `/forecast <subcommand>` remains available for the full forecast CLI.
