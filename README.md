...
In the CLI and TUI, `/questions` shows the active forecast book with numbered rows and `/questions <row>` drills into details without copying a forecast id. `/questions <words>` and `/find <words>` search active forecasts and review-queue items by title, topic, or domain; `/open <row|id|words>` opens one matching ledger record; `/evidence-for <row|words> -- <note>` appends evidence after resolving the forecast; and `/update-for <row|words> -- --probability <p> --rationale <why>` appends an explicit update. `/forecast` opens the structured forecast desk panel; `/sources --question <id>`, `/new-forecast`, `/base-rate`, `/update-forecast`, `/resolve`, `/score`, `/postmortem`, `/review`, `/alerts`, `/self-check`, `/calibration`, `/lessons`, `/errors`, `/backtest`, `/schedule`, `/performance`, `/readiness`, `/doctor`, `/pilot-report`, `/pilot-cohort`, `/export-packet`, `/import-packet`, and `/pilot-aggregate` jump to common desk workflows; and `/forecast <subcommand>` remains available for the full forecast CLI.
