# Contributing to Superforecasting Agent
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want the forecast desk to work everywhere.
...
Bundled skills (in `skills/`) ship with every Superforecasting Agent install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `superforecasting-agent skills browse` (labeled "official") and install it with `superforecasting-agent skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `superforecasting-agent skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.superforecasting-agent/plugins/` (or via a pip entry point). The legacy `~/.hermes/plugins/` path remains supported for compatibility.
...
- Integrate with `superforecasting-agent memory setup` via `post_setup()` — no need to touch core code
...
echo "OPENROUTER_API_KEY=***" >> ~/.superforecasting-agent/.env
```
...
### User configuration (stored in `~/.superforecasting-agent/`)

New installs default to `~/.superforecasting-agent/`. Existing Hermes-compatible installs may still use `~/.hermes/`; keep legacy paths working when touching inherited runtime code.

| Path | Purpose |
|------|---------|
| `~/.superforecasting-agent/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.superforecasting-agent/.env` | API keys and secrets |
| `~/.superforecasting-agent/auth.json` | OAuth credentials (Nous Portal) |
| `~/.superforecasting-agent/forecasting/` | Forecast ledger state, evidence snapshots, scores, and learning records |
| `~/.superforecasting-agent/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.superforecasting-agent/memories/` | Compatibility memory files for inherited features |
| `~/.superforecasting-agent/state.db` | SQLite research-session database |
| `~/.superforecasting-agent/sessions/` | JSON research-session logs |
| `~/.superforecasting-agent/cron/` | Scheduled job data |
| `~/.superforecasting-agent/whatsapp/session/` | WhatsApp bridge credentials |
...
The user may skip setup and keep loading the skill. Superforecasting Agent only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `superforecasting-agent setup` or update `~/.superforecasting-agent/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Superforecasting Agent tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Superforecasting Agent" is the secondary collaborator. If the contributor's commit shows "Superforecasting Agent" as author because they used the agent to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Superforecasting Agent tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `superforecasting-agent --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Superforecasting Agent uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.superforecasting-agent/skins/<name>.yaml`:
...
Superforecasting Agent runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the inherited runtime helper (it has a stdlib fallback
...
2. **Test manually**: Run `superforecasting-agent` or `forecast` and exercise the code path you changed
