...
def test_acp_real_agent_gets_session_db_for_recall(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "agent.runtime", mod("run_agent", AIAgent=CapturingAgent))
    monkeypatch.setitem(
        sys.modules,
        "superforecasting_agent.runtime.config",
        mod("superforecasting_agent.runtime.config", load_config=lambda: {"model": {"default": "m", "provider": "p"}}),
    )
    monkeypatch.setitem(
        sys.modules,
