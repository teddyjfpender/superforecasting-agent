...
def reset_case(monkeypatch):
    ...
    monkeypatch.setattr(
        server,
        "_session_info",
        lambda agent: {"model": "replacement", "skills": {}, "tools": {}},
    )
...
def test_reset_releases_previous_agent_before_construction(reset_case):
    ...
    assert server._reset_session_agent("runtime", session) == {
        "model": "replacement",
        "skills": {},
        "tools": {},
    }
...
def test_busy_tool_change_never_saves_configuration(monkeypatch):
    ...
    server._host.sessions.register(
        "runtime", {"running": True, "history_lock": threading.Lock()}
    )
    monkeypatch.setattr(
        config, "load_config", lambda: {"platform_toolsets": {"cli": ["web"]}}
    )
    save = Mock()
...
def test_rpc_distinguishes_saved_configuration_from_activation_failure(
    reset_case, monkeypatch, failure
):
    ...
    if failure == "save":
        ...
        assert (
            "configuration saved, but session reset failed"
