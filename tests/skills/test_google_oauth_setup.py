...
class TestForecastHomeFallback:
    """Tests for _workspace_home.py fallback when superforecasting_agent.constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_workspace_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _workspace_home.py with superforecasting_agent.constants blocked."""
        monkeypatch.setitem(sys.modules, "superforecasting_agent.constants", None)
        spec = importlib.util.spec_from_file_location("_workspace_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module
    ...
    def test_fallback_uses_superforecasting_agent_home_env_var(self, monkeypatch, tmp_path):
        """When superforecasting_agent.constants is missing, SUPERFORECASTING_AGENT_HOME wins."""
        ...
        assert module.get_agent_home() == tmp_path / "custom-agent"
    ...
    def test_fallback_uses_forecast_home_env_var(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("HERMES_HOME", str(tmp_path / "legacy"))
        module = self._load_helper(monkeypatch)
        assert module.get_agent_home() == tmp_path / "forecast"
    ...
    def test_fallback_uses_hermes_home_env_var(self, monkeypatch, tmp_path):
        ...
        module = self._load_helper(monkeypatch)
        assert module.get_agent_home() == tmp_path / "custom-hermes"

    def test_fallback_defaults_to_dot_superforecasting_agent(self, monkeypatch):
        """When superforecasting_agent.constants is missing and env vars are unset, use the fork home."""
        monkeypatch.delenv("SUPERFORECASTING_AGENT_HOME", raising=False)
        monkeypatch.delenv("FORECAST_HOME", raising=False)
        monkeypatch.delenv("HERMES_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_agent_home() == Path.home() / ".superforecasting-agent"
    ...
    def test_fallback_display_agent_home_shortens_path(self, monkeypatch):
        """Fallback display_agent_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("SUPERFORECASTING_AGENT_HOME", raising=False)
        monkeypatch.delenv("FORECAST_HOME", raising=False)
        monkeypatch.delenv("HERMES_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_agent_home() == "~/.superforecasting-agent"

    def test_fallback_display_agent_home_profile_path(self, monkeypatch):
        """Fallback display_agent_home() handles profile paths under ~/."""
        monkeypatch.setenv(
        ...
        monkeypatch.delenv("HERMES_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_agent_home() == "~/.superforecasting-agent/profiles/coder"

    def test_fallback_display_agent_home_custom_path(self, monkeypatch):
        """Fallback display_agent_home() returns full path for non-home locations."""
        monkeypatch.setenv("SUPERFORECASTING_AGENT_HOME", "/opt/superforecasting-agent-custom")
        monkeypatch.delenv("FORECAST_HOME", raising=False)
        monkeypatch.delenv("HERMES_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_agent_home() == "/opt/superforecasting-agent-custom"

    def test_delegates_to_product_constants_when_available(self):
        """When superforecasting_agent.constants IS importable, _workspace_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_workspace_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import superforecasting_agent.constants
        assert module.get_agent_home is superforecasting_agent.constants.get_agent_home
        assert module.display_agent_home is superforecasting_agent.constants.display_agent_home
