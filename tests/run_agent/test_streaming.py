...
class TestCodexStreamNoneOutputFallback:
    ...
    def test_null_terminal_output_backfilled_from_streamed_item(self):
        """Primary-path null-output recovery (#33042).

        The chatgpt.com Codex backend can stream a real ``output_item.done``
        message and then send a ``response.completed`` whose ``output`` is
        ``null``.  The high-level ``responses.stream(...)`` helper used to
        crash on that with ``TypeError: 'NoneType' object is not iterable``;
        the event-driven primary path never reads the terminal frame's
        ``output`` for content, so it recovers the streamed item.
        """
        agent = self._make_agent()
        ...
        # Consumed via create(stream=True) and recovered the streamed item.
