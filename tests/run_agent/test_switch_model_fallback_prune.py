...
def _switch_to_anthropic(agent):
    with (
        ...
        patch("superforecasting_agent.runtime.timeouts.get_provider_request_timeout", return_value=None),
    ):
        agent.switch_model(
...
def test_switch_within_same_provider_preserves_chain():
    ...
    with patch("superforecasting_agent.runtime.timeouts.get_provider_request_timeout", return_value=None):
        agent.switch_model(
