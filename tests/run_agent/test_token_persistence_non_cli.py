...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    session_module = ModuleType("superforecasting_agent.storage.session")
    session_module.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "superforecasting_agent.storage.session", session_module)
