...
def test_codex_stream_wire_error_event_surfaces_stream_error_event(provider_message):
    ...
    With the migration to ``responses.create(stream=True)`` raw event
    iteration, the SDK's high-level state-machine RuntimeError no longer
    mediates between the wire and us.  When the chatgpt.com Codex backend
    (or xAI, codex-lb, custom relays) emits a ``type=error`` frame as its
    first event, our consumer raises ``_StreamErrorEvent`` straight from the
    wire payload — surfacing on the first event instead of after a wasted
    round trip.  This subsumes the old prelude RuntimeError handling for the
    ``error`` / ``codex.rate_limits`` / ``response.in_progress`` first-frames.
    """
