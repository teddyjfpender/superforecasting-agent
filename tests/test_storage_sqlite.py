...
from superforecasting_agent.storage.session import (
    SessionDB,
...
def _reset_last_init_error():
    ...
    superforecasting_agent.storage.session._set_last_init_error(None)
    yield
    superforecasting_agent.storage.session._set_last_init_error(None)
...
def _reset_wal_fallback_warned_paths():
    ...
    superforecasting_agent.storage.session._wal_fallback_warned_paths.clear()
    yield
    superforecasting_agent.storage.session._wal_fallback_warned_paths.clear()
...
class TestApplyWalWithFallback:
    ...
    def test_warning_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see superforecasting_agent/runtime/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_success_does_not_clear_prior_error(self, tmp_path):
        ...
        """
        superforecasting_agent.storage.session._set_last_init_error("OperationalError: locking protocol")
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("superforecasting_agent.storage.session.sqlite3.connect", side_effect=gated_connect):
...
class TestFormatSessionDbUnavailable:
    ...
    def test_includes_cause(self):
        ...
        superforecasting_agent.storage.session._set_last_init_error("OperationalError: generic SQLite error")
