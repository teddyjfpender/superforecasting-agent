...
def _write_manifest(root: Path, version: str) -> None:
    ...
    (manifest_dir / "agent.json").write_text(
        json.dumps(
            {
                "id": "superforecasting-agent",
                "name": "Superforecasting Agent",
                "version": version,
                "description": "test",
                "distribution": {
                    "uvx": {
                        "package": f"superforecasting-agent[acp]=={version}",
                        "args": ["superforecasting-agent", "acp"],
                    }
                },
            },
            indent=2,
...
def test_update_acp_registry_versions_bumps_manifest_and_pin(monkeypatch, tmp_path):
    ...
    assert manifest["distribution"]["uvx"]["package"] == "superforecasting-agent[acp]==0.14.0"
    # args stay untouched so we don't accidentally rewrite them.
    assert manifest["distribution"]["uvx"]["args"] == ["superforecasting-agent", "acp"]
