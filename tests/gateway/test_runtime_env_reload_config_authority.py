...
max-iterations env aliases, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_max_turns(tmp_path: Path, monkeypatch) -> None:
    ...
    (hermes_home / ".env").write_text(
        "SUPERFORECASTING_AGENT_MAX_ITERATIONS=90\n"
        "FORECAST_MAX_ITERATIONS=90\n"
    ...
    monkeypatch.setenv("SUPERFORECASTING_AGENT_MAX_ITERATIONS", "9000")
    monkeypatch.setenv("FORECAST_MAX_ITERATIONS", "9000")
    ...
    assert os.environ["SUPERFORECASTING_AGENT_MAX_ITERATIONS"] == "9000"
    assert os.environ["FORECAST_MAX_ITERATIONS"] == "9000"
...
def test_reload_runtime_env_keeps_env_max_iterations_when_config_omits_key(
    ...
    (hermes_home / ".env").write_text("FORECAST_MAX_ITERATIONS=123\n", encoding="utf-8")

    monkeypatch.setattr(gateway_run, "_hermes_home", hermes_home)
    monkeypatch.delenv("FORECAST_MAX_ITERATIONS", raising=False)

    gateway_run._reload_runtime_env_preserving_config_authority()

    assert os.environ["FORECAST_MAX_ITERATIONS"] == "123"
