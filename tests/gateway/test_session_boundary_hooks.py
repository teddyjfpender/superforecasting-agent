...
@pytest.mark.asyncio
@patch("superforecasting_agent.runtime.plugins.invoke_hook")
...
@pytest.mark.asyncio
@patch("superforecasting_agent.runtime.plugins.invoke_hook", side_effect=Exception("boom"))
async def test_hook_error_does_not_break_reset(mock_invoke_hook):
    ...
    assert "Forecast session reset" in result or "New forecast session" in result
