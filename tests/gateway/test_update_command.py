...
class TestHandleUpdateCommand:
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _agent_home to tmp_path and project_root to a dir without .git
        fake_root = tmp_path / "project"
        fake_root.mkdir()
        with patch("gateway.run._agent_home", tmp_path), \
    ...
    async def test_no_hermes_binary(self, tmp_path):
        """Returns error when hermes is not on PATH and superforecasting_agent.runtime is not importable."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "superforecasting-agent update" in result

    @pytest.mark.asyncio
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m superforecasting_agent.runtime.main when hermes not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        with patch("gateway.run._agent_home", hermes_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", return_value=None), \
        ...
        assert "Starting Superforecasting Agent update" in result
        call_args = mock_popen.call_args[0][0]
        # The update_cmd uses sys.executable -m superforecasting_agent.runtime.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "superforecasting_agent.runtime.main" in joined or "bash" in call_args[0]
