"""Tests for superforecasting_agent/storage/session.py — SessionDB SQLite CRUD, FTS5 search, export."""
...
class TestSchemaInit:
    ...
    def test_schema_sql_is_source_of_truth(self, db):
        ...
        """
        from superforecasting_agent.storage.session import SCHEMA_SQL
...
class TestConcurrentWriteSafety:
    ...
    def test_write_retries_after_another_connection_releases_lock(self, db, monkeypatch):
        """Lock contention retries preserve the write after the lock clears."""
        import sqlite3
        ...
        monkeypatch.setattr(superforecasting_agent.storage.session.time, "sleep", release_lock)
        # Exercise application retries without waiting inside SQLite itself.
        db._conn.execute("PRAGMA busy_timeout=0")
        try:
            blocker.execute("BEGIN IMMEDIATE")
            db.create_session("contended", source="cli")
            assert sleeps
            assert db.get_session("contended")["source"] == "cli"
        finally:
            blocker.close()
