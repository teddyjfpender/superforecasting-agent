...
def test_ws_events_rejects_when_token_required(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "superforecasting_agent.runtime.web_server", stub)
    monkeypatch.setattr(superforecasting_agent.runtime, "web_server", stub, raising=False)
...
def test_create_task_includes_warning_when_no_dispatcher(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "superforecasting_agent.runtime.kanban._check_dispatcher_presence",
...
# tests/runtime_cli/test_kanban_db.py. The OUTER fallback here is not, which
...
def test_board_endpoint_survives_task_age_exception(client, monkeypatch):
    ...
    monkeypatch.setattr("superforecasting_agent.runtime.kanban_db.task_age", _boom)
...
def test_create_task_probe_error_does_not_break_create(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "superforecasting_agent.runtime.kanban._check_dispatcher_presence", _raise,
    )
    r = client.post(
        "/api/plugins/kanban/tasks",
...
def test_board_surfaces_warnings_field_for_hallucinated_completions(client):
    ...
    raw event kind — see superforecasting_agent.runtime.kanban_diagnostics for the rule
    that produces it.
    """
    conn = kb.connect()
    try:
