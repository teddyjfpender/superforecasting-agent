...
def test_cli_register_includes_node_subcommand():
    """`superforecasting-agent meet` argparse tree includes the node subtree."""
    import argparse
...
def test_looks_like_human_speaker():
    ...
    for s in ("", "   ", "Unknown", "unknown", "You", "you", "Superforecasting Agent", "superforecasting agent"):
        assert not _looks_like_human_speaker(s, "Superforecasting Agent"), f"{s!r} should NOT be human"
    # Real names → human (barge-in)
    for s in ("Alice", "Bob Lee", "@teknium"):
        assert _looks_like_human_speaker(s, "Superforecasting Agent"), f"{s!r} SHOULD be human"
...
# ---------------------------------------------------------------------------
# superforecasting-agent meet install CLI
# ---------------------------------------------------------------------------
