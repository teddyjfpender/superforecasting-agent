...
def plugin_api(tmp_path, monkeypatch):
    ...
    # swap ``sys.modules['superforecasting_agent.storage.session']`` with auto-restoration. Without
...
class _FakeSessionDB:
    """Stand-in for superforecasting_agent.storage.session.SessionDB that records scan calls."""
...
def _install_fake_session_db(plugin_api, fake_db):
    ...
    ``sys.modules['superforecasting_agent.storage.session']`` swap is auto-restored at test teardown
    and cannot leak into unrelated tests in the same xdist worker.
    """
    fake_module = type(sys)("superforecasting_agent.storage.session")
    fake_module.SessionDB = lambda: fake_db
    plugin_api._test_monkeypatch.setitem(sys.modules, "superforecasting_agent.storage.session", fake_module)
