"""Tests for ACP Registry metadata shipped with Superforecasting Agent."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "superforecasting-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Superforecasting Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/NousResearch/superforecasting-agent"
    assert data["website"].startswith("https://superforecasting-agent.nousresearch.com/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"superforecasting-agent[acp]=={data['version']}"
    assert uvx["args"] == ["superforecasting-agent", "acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"superforecasting-agent[acp]=={_pyproject_version()}"
    )
