...
def test_main_setup_runs_model_configuration(monkeypatch):
    ...
    monkeypatch.setattr("superforecasting_agent.runtime.main.main", fake_hermes_main)
...
def test_main_setup_offers_browser_install_when_tty(monkeypatch):
    ...
    monkeypatch.setattr("superforecasting_agent.runtime.main.main", lambda: None)
...
def test_main_setup_browser_calls_ensure_dependency(monkeypatch):
    """`superforecasting-agent acp --setup-browser` routes through dep_ensure.ensure_dependency."""
    calls = []
    ...
    monkeypatch.setattr("superforecasting_agent.runtime.dep_ensure.ensure_dependency", fake_ensure)
