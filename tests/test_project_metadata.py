...
def test_forecast_native_package_namespace_is_exposed():
    ...
    superforecast_launcher = (root / "superforecast").read_text(encoding="utf-8")
    ...
    assert "from superforecasting_agent.cli import main" in superforecast_launcher
...
def test_contributor_and_skills_index_guidance_is_forecast_native():
    ...
    contributor_audit = (root / "scripts" / "contributor_audit.py").read_text(
        encoding="utf-8"
    )
    ...
    assert 'DEFAULT_GITHUB_REPO = "teddyjfpender/superforecasting-agent"' in contributor_audit
...
def test_forecast_cli_smoke_transcript_captures_tester_path():
    ...
    expected_markers = [
        ...
        "[forecast-smoke] packet_import_questions:",
