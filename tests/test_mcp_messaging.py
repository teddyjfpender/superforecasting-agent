...
from superforecasting_agent.mcp import data as mcp_data
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking superforecasting_agent.storage.session schema."""
...
def fake_mcp_server(populated_sessions_dir, mock_session_db, monkeypatch):
    from superforecasting_agent.mcp import server as mcp_serve

    monkeypatch.setattr(mcp_data, "_get_sessions_dir", lambda: populated_sessions_dir)
    monkeypatch.setattr(mcp_data, "_get_session_db", lambda: mock_session_db)
    monkeypatch.setattr(mcp_data, "_load_channel_directory", lambda: {})
...
class TestHelpers:
    def test_get_sessions_dir(self, tmp_path):
        from superforecasting_agent.mcp.data import _get_sessions_dir
    ...
    def test_fallback_agent_home_prefers_forecast_native_alias(self, monkeypatch, tmp_path):
        from superforecasting_agent.mcp.data import _fallback_agent_home
    ...
    def test_coerce_int_handles_invalid_and_out_of_range_values(self):
        from superforecasting_agent.mcp.data import _coerce_int
    ...
    def test_load_sessions_index_empty(self, sessions_dir, monkeypatch):
        from superforecasting_agent.mcp import server as mcp_serve
        monkeypatch.setattr(mcp_data, "_get_sessions_dir", lambda: sessions_dir)
        assert mcp_data._load_sessions_index() == {}

    def test_load_sessions_index_with_data(self, populated_sessions_dir, monkeypatch):
        from superforecasting_agent.mcp import server as mcp_serve
        monkeypatch.setattr(mcp_data, "_get_sessions_dir", lambda: populated_sessions_dir)
        result = mcp_data._load_sessions_index()
        assert len(result) == 3
...
class TestContentExtraction:
    def test_text(self):
        from superforecasting_agent.mcp.data import _extract_message_content
...
class TestAttachmentExtraction:
    def test_image_url_block(self):
        from superforecasting_agent.mcp.data import _extract_attachments
        msg = {"content": [
...
class TestEventBridge:
    def test_create(self):
        from superforecasting_agent.mcp.events import EventBridge
        b = EventBridge()
        assert b._cursor == 0
        assert b._queue == []

    def test_enqueue_and_poll(self):
        from superforecasting_agent.mcp.events import EventBridge, QueueEvent
        b = EventBridge()
    ...
    def test_queue_limit(self):
        from superforecasting_agent.mcp.events import EventBridge, QueueEvent, QUEUE_LIMIT
        b = EventBridge()
...
class TestE2EEventsPoll:
    ...
    def test_poll_with_events(self, mcp_server_e2e, _event_loop):
        from superforecasting_agent.mcp.events import QueueEvent
        server, bridge = mcp_server_e2e
...
class TestCliIntegration:
    ...
    def test_dispatcher_routes_serve(self, monkeypatch, tmp_path):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        mock_run = MagicMock()
        monkeypatch.setattr("superforecasting_agent.mcp.server.run_mcp_server", mock_run)
...
class TestEdgeCases:
    ...
    def test_sessions_without_origin(self, sessions_dir, monkeypatch):
        ...
        from superforecasting_agent.mcp import server as mcp_serve
        monkeypatch.setattr(mcp_data, "_get_sessions_dir", lambda: sessions_dir)
        entries = mcp_data._load_sessions_index()
...
class TestEventBridgePollE2E:
    ...
    def test_poll_detects_new_messages(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(mcp_data, "_get_session_db", lambda: TestDB())
    ...
    def test_poll_interval_is_200ms(self):
        ...
        from superforecasting_agent.mcp.events import POLL_INTERVAL
