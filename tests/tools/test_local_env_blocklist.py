...
Verifies that agent-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed agent secrets.
...
class TestProviderEnvBlocklist:
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is an agent inference secret
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        The local terminal is the user's trusted operator shell. A user running
        ``aws``/``terraform``/``cdk``/``boto3`` in the agent terminal must keep
        the same AWS access their own shell has. Stripping these would (a) break
...
class TestBlocklistCoverage:
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell, not to the agent, and blocklisting them would be
        unrecoverable via env_passthrough (GHSA-rhgp-j443-p4rf).
        """
        general_chain = {
            "AWS_ACCESS_KEY_ID",
