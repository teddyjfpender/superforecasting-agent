...
Covers .ipynb / .docx / .xlsx extraction (tools/read_extract.py, ported from
upstream 817f39231) and the read_file_tool integration: pagination,
...
class TestNotebookExtraction(unittest.TestCase):
    ...
    def test_cells_non_list_worksheets_int_raises_extraction_error(self):
        # Malformed structure: cells is not a list AND worksheets is a
        # non-iterable int. Must raise ExtractionError (not an uncaught
        # TypeError) so read_file's ``except ExtractionError`` falls back.
        p = os.path.join(self.tmp, "bad_struct.ipynb")
        with open(p, "w", encoding="utf-8") as fh:
            json.dump({"cells": 5, "worksheets": 7, "nbformat": 3}, fh)
        with self.assertRaises(ExtractionError):
            extract_document_text(p)

    def test_worksheet_cells_int_raises_extraction_error(self):
        # worksheets is a list but a worksheet's ``cells`` is a non-iterable.
        p = os.path.join(self.tmp, "bad_ws_cells.ipynb")
        with open(p, "w", encoding="utf-8") as fh:
            json.dump({"worksheets": [{"cells": 99}], "nbformat": 3}, fh)
        with self.assertRaises(ExtractionError):
            extract_document_text(p)

    def test_read_file_falls_back_on_malformed_notebook_structure(self):
        # End-to-end: read_file_tool must not crash on a structurally-malformed
        # notebook. Extraction raises ExtractionError (caught internally) and
        # the tool falls through to the normal text-read path; .ipynb is not a
        # binary extension, so the raw JSON is returned as plain text.
        p = os.path.join(self.tmp, "bad_struct_int.ipynb")
        with open(p, "w", encoding="utf-8") as fh:
            json.dump({"cells": 123}, fh)
        res = json.loads(read_file_tool(p))
        # No crash, and NOT treated as a successfully-extracted document.
        self.assertNotIn("extracted_document", res)
        # The raw notebook JSON is read back as text content.
        self.assertIn("content", res)
        self.assertIn("123", res["content"])
...
class TestXlsxExtraction(unittest.TestCase):
    ...
    def test_oversized_zip_member_raises(self):
        # Zip-bomb guard: a member whose UNCOMPRESSED size exceeds the cap must
        # raise ExtractionError before it is fully decompressed into memory.
        # We shrink the cap rather than build a 50MB fixture.
        import tools.read_extract as rex

        p = os.path.join(self.tmp, "bomb.xlsx")
        self._build(p)
        original = rex.MAX_XLSX_BYTES
        rex.MAX_XLSX_BYTES = 4  # smaller than any real member
        try:
            with self.assertRaises(ExtractionError):
                extract_document_text(p)
        finally:
            rex.MAX_XLSX_BYTES = original
...
class TestReadFileToolIntegration(unittest.TestCase):
    ...
    def test_pagination(self):
        ...
        # Only first 2 lines present; first line is the cell header.
        self.assertIn("# ── Code cell 1 ──", res["content"])
        self.assertNotIn("\nf", res["content"])
