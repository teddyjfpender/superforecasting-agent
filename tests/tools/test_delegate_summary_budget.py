...
def test_async_reentry_summary_is_budgeted(monkeypatch):
    """The fork's dominant delegation path is background=true. A background
    child's result re-enters via the async completion queue, bypassing the
    synchronous results.sort path — so _apply_summary_budget([result], parent)
    must trim it at the re-entry site too. This exercises that single-result
    code path directly."""
    with tempfile.TemporaryDirectory() as td:
        monkeypatch.setenv("HERMES_HOME", os.path.join(td, ".hermes"))
        big = "HEAD_MARKER\n" + ("X" * 50_000) + "\nTAIL_MARKER"
        parent = _FakeParent(context_length=131_000, used_tokens=120_000, max_tokens=8_000)
        # A single background child's result dict, as _run_single_child returns.
        result = {"task_index": 0, "summary": big, "status": "completed"}
        dt._apply_summary_budget([result], parent)
        assert result["summary_truncated"] is True
        assert len(result["summary"]) < len(big)
        assert "HEAD_MARKER" in result["summary"]
        assert "TAIL_MARKER" in result["summary"]
        assert os.path.exists(result["summary_full_path"])
