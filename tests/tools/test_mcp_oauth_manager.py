...
def test_hermes_provider_subclass_exists():
    ...
    assert issubclass(_AGENT_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_hermes_provider_subclass(tmp_path, monkeypatch):
    ...
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _AGENT_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    ...
    assert _AGENT_PROVIDER_CLS is not None
    assert isinstance(provider, _AGENT_PROVIDER_CLS)
