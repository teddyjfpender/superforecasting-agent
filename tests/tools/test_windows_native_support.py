...
class TestConfigureWindowsStdio:
    """``superforecasting_agent.runtime.stdio.configure_windows_stdio`` wiring.
    ...
    def _reset_configured(self, monkeypatch):
        ...
        sys.modules.pop("superforecasting_agent.runtime.stdio", None)
        # Fresh import now; tests import from superforecasting_agent.runtime.stdio themselves,
        # but this guarantees the module they get is a brand-new copy.
        import superforecasting_agent.runtime.stdio as _s
        _s._CONFIGURED = False
        yield
        sys.modules.pop("superforecasting_agent.runtime.stdio", None)

    def test_no_op_on_posix(self):
        from superforecasting_agent.runtime import stdio
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("superforecasting_agent.runtime.kanban_db", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / 'superforecasting_agent/runtime' / "web_server.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    """cli.py, superforecasting_agent/runtime/main.py, gateway/run.py must call configure_windows_stdio."""

    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "superforecasting_agent/runtime/main.py", "gateway/run.py"],
    )
    def test_entry_point_calls_configure_stdio(self, relpath):
        root = Path(__file__).resolve().parents[2]
        source = (root / relpath).read_text(encoding="utf-8")
        assert "configure_windows_stdio" in source, (
            f"{relpath} must call superforecasting_agent.runtime.stdio.configure_windows_stdio() "
...
class TestSubprocessCompatHelpers:
    """superforecasting_agent/runtime/_subprocess_compat.py POSIX + Windows behaviour."""

    def test_is_windows_matches_sys_platform(self):
        from superforecasting_agent.runtime import _subprocess_compat as sc
    ...
    def test_resolve_node_command_returns_absolute_on_posix(self):
        ...
        from superforecasting_agent.runtime._subprocess_compat import resolve_node_command
    ...
    def test_windows_flags_zero_on_posix(self):
        from superforecasting_agent.runtime._subprocess_compat import (
...
# ---------------------------------------------------------------------------
# superforecasting_agent/runtime/kanban_db.py waitpid guard
# ---------------------------------------------------------------------------
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / 'superforecasting_agent/runtime' / "kanban_db.py").read_text(encoding="utf-8")
...
class TestNpmBareSpawnsResolved:
    ...
    shutil.which / superforecasting_agent.runtime._subprocess_compat.resolve_node_command
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "superforecasting_agent/runtime/doctor.py",
...
class TestLocalEnvironmentWindowsTempDir:
    ...
    def test_source_has_windows_branch_using_hermes_home(self):
        ...
        assert "get_agent_home" in source
        assert 'cache_dir = get_agent_home() / "cache" / "terminal"' in source
...
class TestWorktreeSymlinkFallback:
    ...
    def test_source_has_symlink_fallback(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "superforecasting_agent/runtime/worktree_setup.py").read_text(encoding="utf-8")
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_hermes_cli_gateway_uses_compat_kwargs(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / 'superforecasting_agent/runtime' / "gateway.py").read_text(encoding="utf-8")
        assert "windows_detach_popen_kwargs" in source, (
            "superforecasting_agent/runtime/gateway.py must use the platform-aware detach helper"
        )
