"""Tests for superforecasting_agent.bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every application entry point
(hermes, hermes-agent, hermes-acp, gateway, superforecasting_agent.trajectories.batch, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every application entry point imports superforecasting_agent.bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported superforecasting_agent.bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("superforecasting_agent.bootstrap", None)
    import superforecasting_agent.bootstrap  # noqa: WPS433
    return sys.modules["superforecasting_agent.bootstrap"]
...
class TestEntryPointsImportBootstrap:
    """Every application entry point must import superforecasting_agent.bootstrap as its
    ...
    # import superforecasting_agent.bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "superforecasting_agent/runtime/main.py",   # hermes CLI (console_script)
        "run_agent.py",          # hermes-agent (console_script)
        "acp_adapter/entry.py",  # hermes-acp (console_script)
        "gateway/run.py",        # gateway
        "superforecasting_agent/trajectories/batch.py",       # batch mode
        "cli.py",                # legacy direct-launch CLI
    ]

    @pytest.mark.parametrize("path", ENTRY_POINTS)
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import superforecasting_agent.bootstrap' and that
        ...
        ``superforecasting_agent.bootstrap`` as a top-level module) leaves hermes
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # hermes start even when superforecasting_agent.bootstrap hasn't been
        ...
        assert first_import_name == "superforecasting_agent.bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'superforecasting_agent.bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import superforecasting_agent.bootstrap' line to be the first import."
        )
