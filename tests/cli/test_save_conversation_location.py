"""Tests for /save — the forecast transcript snapshot slash command.

Regression: the old implementation wrote the snapshot to the current working
directory (CWD). Users who ran /save expected the file to be discoverable via
session browsing, but CWD-resident snapshots are not indexed in the state DB and
are generally invisible.
The fix writes snapshots under the agent home's ``sessions/saved/`` and prints
...
def test_save_conversation_writes_under_hermes_home(hermes_home, tmp_path, monkeypatch, capsys):
    ...
    cwd_leak = list(work.glob("forecast_transcript_*.json"))
    ...
    files = list(saved_dir.glob("forecast_transcript_*.json"))
    ...
    assert "superforecasting-agent --resume 20260101_120000_abc123" in out, out
...
def test_save_conversation_empty_history_does_nothing(hermes_home, capsys):
    ...
    out = capsys.readouterr().out
    assert "No forecast transcript to save" in out
