...
class TestHistoryDisplay:
    ...
    def test_history_shows_recent_sessions_when_current_chat_is_empty(self, capsys):
        ...
        cli._session_db.list_sessions_rich.return_value = [
            ...
            },
            {
                ...
                "title": "Checking Forecast Agent",
                "preview": "check forecast agent gateways",
                "last_active": 0,
            },
        ]
        ...
        assert "No forecast transcript messages yet" in output
    ...
    def test_sessions_with_target_delegates_to_resume(self):
        ...
        with patch.object(cli, "_handle_resume_command") as mock_resume:
            cli.process_command("/sessions Checking Forecast Agent")

        mock_resume.assert_called_once_with(
            "/resume Checking Forecast Agent"
        )
...
class TestRootLevelProviderOverride:
    ...
    def test_nested_cli_override_preserves_canonical_defaults(self, tmp_path, monkeypatch):
        ...
        (hermes_home / "config.yaml").write_text(
            yaml.safe_dump({"collaboration": {"github": {"enabled": True}}})
        )
        ...
        for key, value in DEFAULT_CONFIG["collaboration"]["github"].items():
            if key != "enabled":
                assert config["collaboration"]["github"][key] == value
