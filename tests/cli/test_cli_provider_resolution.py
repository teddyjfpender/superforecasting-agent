...
def test_hermes_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    ...
    monkeypatch.setattr("superforecasting_agent.runtime.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
...
def test_codex_provider_replaces_incompatible_default_model(monkeypatch):
    ...
    monkeypatch.setattr("superforecasting_agent.runtime.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
    monkeypatch.setattr(
        "superforecasting_agent.runtime.codex_models.get_codex_model_ids",
...
def test_model_flow_nous_prints_subscription_guidance_without_mutating_explicit_tts(monkeypatch, capsys):
    ...
    monkeypatch.setattr("superforecasting_agent.runtime.auth._update_config_for_provider", lambda provider, url: None)
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "superforecasting_agent.runtime.config.get_env_value",
    ...
    monkeypatch.setattr("superforecasting_agent.runtime.secret_prompt.masked_secret_prompt", lambda _prompt="": next(answers))
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("superforecasting_agent.runtime.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("superforecasting_agent.runtime.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "superforecasting_agent.runtime.main._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("superforecasting_agent.runtime.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    ...
    monkeypatch.setattr("superforecasting_agent.runtime.auth._login_nous", _fake_login)
...
def test_auto_provider_name_localhost():
    from superforecasting_agent.runtime.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from superforecasting_agent.runtime.main import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "superforecasting_agent.runtime.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("superforecasting_agent.runtime.config.save_config", _save)
