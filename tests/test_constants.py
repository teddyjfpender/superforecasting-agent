"""Tests for superforecasting_agent.constants module."""
...
class TestGetDefaultHermesRoot:
    """Tests for get_default_agent_root() — Docker/custom deployment awareness."""

    def test_no_hermes_home_returns_native(self, tmp_path, monkeypatch):
        """When HERMES_HOME is not set, returns the fork-native home."""
        monkeypatch.delenv("HERMES_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_agent_root() == tmp_path / ".superforecasting-agent"
    ...
    def test_hermes_home_is_native(self, tmp_path, monkeypatch):
        ...
        assert get_default_agent_root() == native
    ...
    def test_hermes_home_is_docker(self, tmp_path, monkeypatch):
        ...
        assert get_default_agent_root() == docker_home
    ...
    def test_hermes_home_is_custom_path(self, tmp_path, monkeypatch):
        ...
        assert get_default_agent_root() == custom
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HERMES_HOME", str(profile))
        assert get_default_agent_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        """Reset the cache on the same module that owns the imported function."""
        monkeypatch.setattr(constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(constants, "_container_detected", True)
        assert is_container() is True
