"""Tests for superforecasting_agent.trajectories.batch checkpoint behavior — incremental writes, resume, atomicity."""
...
from superforecasting_agent.trajectories.batch import BatchRunner, _process_batch_worker
...
class TestBatchWorkerResumeBehavior:
    def test_discarded_no_reasoning_prompts_are_marked_completed(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("superforecasting_agent.trajectories.batch_worker._process_single_prompt", lambda *args, **kwargs: prompt_result)
...
class TestFinalCheckpointNoDuplicates:
    ...
    def _simulate_final_aggregation_fixed(self, batch_results):
        """Mirror the fixed code path in superforecasting_agent.trajectories.batch.run()."""
