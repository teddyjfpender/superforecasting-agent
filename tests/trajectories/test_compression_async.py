"""Tests for superforecasting_agent.trajectories.compression AsyncOpenAI event loop binding.
...
class TestAsyncClientLazyCreation:
    """superforecasting_agent/trajectories/compression.py — _get_async_client()"""
...
class TestSourceLineVerification:
    ...
    def _read_file() -> str:
        ...
        with open(os.path.join(base, "superforecasting_agent/trajectories/compression.py")) as f:
            return f.read()
...
async def test_generate_summary_async_kimi_omits_temperature():
    ...
    from superforecasting_agent.trajectories.compression import CompressionConfig, TrajectoryCompressor, TrajectoryMetrics
