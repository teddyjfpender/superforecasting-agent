...
# `superforecasting-agent curator status`). The full data has always been in REPORT.md on
...
def test_rename_summary_consolidation_shows_target(curator_env):
    ...
    assert "full report: superforecasting-agent curator status" in result
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `superforecasting-agent curator pin <umbrella>` in the
# rename block so users learn the command exists at the moment they care
# (a consolidation just landed against their library). The hint is gated on
# having at least one umbrella destination — pruned-only runs skip it.
# ---------------------------------------------------------------------------
...
def test_rename_summary_pin_hint_appears_when_consolidation_produced_umbrella(curator_env):
    ...
    assert "superforecasting-agent curator pin document-tools" in result
...
def test_rename_summary_pin_hint_skipped_for_pruned_only_runs(curator_env):
    ...
    assert "archived 2 skill(s):" in result
    assert "superforecasting-agent curator pin" not in result
...
def test_rename_summary_pin_hint_picks_one_umbrella_when_multiple_absorbed(curator_env):
    ...
    assert "superforecasting-agent curator pin umbrella-alpha" in result
    # Exactly one hint line, not one per umbrella.
    pin_lines = [ln for ln in result.splitlines() if "superforecasting-agent curator pin" in ln]
