...
def test_pgpassword_preserved_verbatim(monkeypatch):
    ...
    args = '{"command": "PGPASSWORD=s3cr3tpw psql -h db -U admin"}'
    assert _build(args) == args


def test_bearer_token_in_args_preserved_verbatim(monkeypatch):
    monkeypatch.setattr("agent.redact._REDACT_ENABLED", True, raising=False)
