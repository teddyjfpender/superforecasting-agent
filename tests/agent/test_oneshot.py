...
class TestRunOneshot:
    ...
    def test_main_runtime_is_forwarded(self):
        # The live session's model override must reach call_llm so the one-shot
        # inherits the user's provider/model instead of the cheap aux backend.
        ...
        with patch(
            ...
            run_oneshot(
                instructions="x", user_input="y", main_runtime=runtime
            )
        assert llm.call_args.kwargs["main_runtime"] == runtime

    def test_does_not_mutate_any_session_or_history(self):
        # A one-shot is stateless by construction: it only sends the prompt it
        # was handed and must never read or append to a conversation history.
        # We pass a sentinel "session" object and assert it is untouched, and
        # that the messages sent are exactly the rendered prompt (no history).
        session = {"history": ["prior-turn-1", "prior-turn-2"], "version": 7}
        history_snapshot = list(session["history"])

        with patch(
            "agent.oneshot.call_llm",
            return_value=self._mock_response("feat: x"),
        ) as llm:
            run_oneshot(instructions="sys", user_input="user")

        # call_llm received only our two prompt messages — no session history.
        messages = llm.call_args.kwargs["messages"]
        assert messages == [
            {"role": "system", "content": "sys"},
            {"role": "user", "content": "user"},
        ]
        # The unrelated session/history object is byte-for-byte unchanged.
        assert session["history"] == history_snapshot
