...
def test_superforecasting_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import superforecasting_client_tag

    tag = superforecasting_client_tag()
    assert tag.startswith("client=superforecasting-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import superforecasting_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=superforecasting-agent" in tags
    assert superforecasting_client_tag() in tags
    assert len(tags) == 2
