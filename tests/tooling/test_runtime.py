"""Tests for superforecasting_agent/tooling/runtime.py — function call dispatch, agent-loop interception, legacy toolsets."""
...
class TestHandleFunctionCall:
    ...
    def test_tool_hooks_receive_session_and_tool_call_ids(self):
        with (
            patch("tools.registry.registry.dispatch", return_value='{"ok":true}'),
            patch("superforecasting_agent.runtime.plugins.invoke_hook") as mock_invoke_hook,
        ):
...
class TestPreToolCallBlocking:
    ...
    def test_blocked_tool_returns_error_and_skips_dispatch(self, monkeypatch):
        ...
        monkeypatch.setattr("superforecasting_agent.runtime.plugins.invoke_hook", fake_invoke_hook)
        monkeypatch.setattr("tools.registry.registry.dispatch", fake_dispatch)
    ...
    def test_blocked_tool_skips_read_loop_notification(self, monkeypatch):
        ...
        monkeypatch.setattr("superforecasting_agent.runtime.plugins.invoke_hook", fake_invoke_hook)
        monkeypatch.setattr("tools.registry.registry.dispatch",
...
class TestCoerceNumberInfNan:
    ...
    def test_inf_returns_original_string(self):
        from superforecasting_agent.tooling.arguments import _coerce_number
