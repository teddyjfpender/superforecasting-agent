...
def _clear_cache():
    ...
    model_tools.definitions._tool_defs_cache.clear()
    yield
    model_tools.definitions._tool_defs_cache.clear()
...
class TestQuietModeCacheIsolation:
    ...
    def test_first_uncached_call_returns_fresh_list(self):
        ...
        assert len(model_tools.definitions._tool_defs_cache) == 1
        cached = next(iter(model_tools.definitions._tool_defs_cache.values()))
    ...
    def test_non_quiet_mode_does_not_use_cache(self):
        ...
        assert len(model_tools.definitions._tool_defs_cache) == 0
