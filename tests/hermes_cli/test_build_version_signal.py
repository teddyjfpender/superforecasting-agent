...
def banner(tmp_path, monkeypatch):
    """``hermes_cli.banner`` with a scratch agent-home and clean module state."""
    import hermes_cli.banner as mod
...
def test_remedy_defers_to_the_package_manager_when_managed(banner):
    with patch("hermes_cli.config.get_managed_update_command", return_value="brew upgrade x"):
...
def test_gateway_build_info_defaults_to_the_non_blocking_read():
    ...
    with patch("hermes_cli.banner.get_update_state") as spy:
...
def test_gateway_build_info_still_names_the_version_if_the_check_explodes():
    ...
    with patch("hermes_cli.banner.get_update_state", side_effect=RuntimeError("boom")):
        payload = server.build_info()
...
def _doctor_output(capsys, state):
    ...
    with patch("hermes_cli.banner.prefetch_update_check"):
        with patch("hermes_cli.banner.get_update_state", return_value=state):
...
def test_doctor_never_raises_when_the_check_explodes(capsys):
    ...
    issues: list[str] = []
    with patch("hermes_cli.banner.prefetch_update_check", side_effect=RuntimeError("boom")):
        doctor_mod._check_build_version(issues)
...
def test_bundled_tui_declares_itself_an_es_module():
    ...
    marker = root / "hermes_cli" / "tui_dist" / "package.json"

    assert marker.exists(), "hermes_cli/tui_dist/package.json is missing"
    assert json.loads(marker.read_text()) == {"type": "module"}

    gitignore = (root / ".gitignore").read_text()
    assert "!hermes_cli/tui_dist/package.json" in gitignore, "the marker would be gitignored"

    pyproject = tomllib.loads((root / "pyproject.toml").read_text())
    globs = pyproject["tool"]["setuptools"]["package-data"]["hermes_cli"]
