...
def test_recommended_update_command_pip():
    ...
    with patch("hermes_cli.config._IS_WINDOWS", False):
...
def test_release_update_check_uses_github_releases(monkeypatch, capsys):
    ...
    monkeypatch.setattr("hermes_cli.config.detect_install_method", lambda *_args: "release")
    check = MagicMock(return_value=0)
    monkeypatch.setattr("hermes_cli.banner.check_via_release", check)
