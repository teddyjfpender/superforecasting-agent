"""Tests for superforecasting_agent.runtime configuration management."""
...
class TestGetHermesHome:
    ...
    def test_forecast_home_alias_drives_default_root(self, tmp_path):
        ...
        with patch.dict(os.environ, {"FORECAST_HOME": str(custom_root)}, clear=False):
            os.environ.pop("HERMES_HOME", None)
            os.environ.pop("SUPERFORECASTING_AGENT_HOME", None)
            assert get_default_agent_root() == custom_root
...
class TestLoadConfigDefaults:
    ...
    def test_memory_defaults_are_ledger_first(self, tmp_path):
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            config = load_config()
...
class TestLoadConfigParseFailure:
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from superforecasting_agent.runtime import config as cfg_mod
        ...
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="superforecasting_agent.runtime.config"):
                config = load_config()
...
class TestSaveConfigAtomicity:
    ...
    def test_no_partial_write_on_crash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            ...
            with patch("superforecasting_agent.storage.files.yaml.dump", side_effect=OSError("disk full")):
                try:
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from superforecasting_agent.runtime.config import ENV_VARS_BY_VERSION
        all_vars = []
