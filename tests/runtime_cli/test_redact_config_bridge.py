...
aliases at module-import time. `superforecasting_agent/runtime/main.py` at
...
env aliases plus compatibility `HERMES_REDACT_SECRETS` in `superforecasting_agent/runtime/main.py`
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports superforecasting_agent.runtime.main and checks
    ...
    probe = textwrap.dedent(
        ...
        sys.path.insert(0, %r)
        import superforecasting_agent.runtime.main  # triggers the bridge + setup_logging
        import agent.redact
...
def test_redact_secrets_default_true_when_unset(tmp_path):
    ...
    probe = textwrap.dedent(
        ...
        for name in %r:
            os.environ.pop(name, None)
        sys.path.insert(0, %r)
        import superforecasting_agent.runtime.main
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        """
    ) % (REDACT_ENV_ALIASES, str(REPO_ROOT))
