...
def container_env(tmp_path, monkeypatch):
    ...
    container_mode.write_text(
        ...
        "exec_user=hermes\n"
        "hermes_bin=/data/current-package/bin/superforecasting-agent\n"
    )
    return hermes_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("superforecasting_agent.constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "superforecasting-agent"
    assert info["exec_user"] == "hermes"
    assert info["hermes_bin"] == "/data/current-package/bin/superforecasting-agent"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("superforecasting_agent.constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("superforecasting_agent.constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_agent_home": lambda: hermes_home}), \
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        ...
        "exec_user=myuser\n"
        "hermes_bin=/opt/superforecasting-agent/bin/superforecasting-agent\n"
    )
    ...
    assert info["hermes_bin"] == "/opt/superforecasting-agent/bin/superforecasting-agent"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from superforecasting_agent.runtime.main import _exec_in_container
    ...
    assert "/data/current-package/bin/superforecasting-agent" in cmd
    assert "chat" in cmd
