...
from superforecasting_agent.runtime.proxy.adapters import ADAPTERS, get_adapter
from superforecasting_agent.runtime.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from superforecasting_agent.runtime.proxy.adapters.nous_portal import NousPortalAdapter
from superforecasting_agent.runtime.proxy.adapters.xai import XAIGrokAdapter
...
def test_nous_adapter_get_credential_uses_runtime_resolver(tmp_path, monkeypatch):
    ...
    with patch(
        "superforecasting_agent.runtime.proxy.adapters.nous_portal.resolve_nous_runtime_credentials",
...
def test_nous_adapter_get_credential_raises_when_not_logged_in(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    adapter = NousPortalAdapter()
    with pytest.raises(RuntimeError, match="superforecasting-agent auth add nous"):
...
from superforecasting_agent.runtime.proxy.server import create_app  # noqa: E402
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    from superforecasting_agent.runtime.proxy.cli import cmd_proxy_status
...
def test_cmd_proxy_providers_runs(capsys):
    from superforecasting_agent.runtime.proxy.cli import cmd_proxy_list_providers
...
def test_cmd_proxy_start_refuses_unknown_provider(capsys):
    from superforecasting_agent.runtime.proxy.cli import cmd_proxy_start
...
def test_cmd_proxy_start_refuses_when_unauthenticated(capsys, tmp_path, monkeypatch):
    ...
    assert rc == 2
    err = capsys.readouterr().err
    assert "superforecasting-agent auth add nous" in err
