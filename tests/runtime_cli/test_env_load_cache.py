...
def test_load_env_caches_on_repeat_calls():
    ...
    from superforecasting_agent.runtime.config import invalidate_env_cache, load_env
...
def test_save_env_value_invalidates_cache(tmp_path, monkeypatch):
    ...
    from superforecasting_agent.runtime import config as config_mod
    from superforecasting_agent.runtime.config import invalidate_env_cache, load_env, save_env_value
    ...
    monkeypatch.setattr(config_mod, "ensure_agent_home", lambda: None)
...
def test_load_env_handles_missing_file():
    ...
    try:
        with patch("superforecasting_agent.runtime.config.get_env_path", return_value=nonexistent):
            assert load_env() == {}
