...
from superforecasting_agent.runtime import main as cli_main


# Tests in this module either exercise the REAL _detect_concurrent_agent_instances
# helper (and need the autouse stub in tests/runtime_cli/conftest.py disabled),
...
# ---------------------------------------------------------------------------
# _detect_concurrent_agent_instances
# ---------------------------------------------------------------------------
...
def test_detect_concurrent_returns_empty_when_no_other_processes(_winp, tmp_path):
    ...
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_agent_instances(scripts_dir)
...
def test_detect_concurrent_is_noop_off_windows(_winp, tmp_path):
    ...
    assert cli_main._detect_concurrent_agent_instances(tmp_path) == []
...
# ---------------------------------------------------------------------------
# _quarantine_running_agent_exe — retry + reboot-deferred fallback
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    pairs = cli_main._quarantine_running_agent_exe(tmp_path)
...
def test_quarantine_retries_then_succeeds(_winp, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli_main, "_agent_exe_shims", lambda d: [shim])
...
def test_cmd_update_force_bypasses_concurrent_check(_winp, tmp_path):
    ...
    with patch.object(
        ...
        cli_main, "_detect_concurrent_agent_instances", detect
    ), patch.object(
