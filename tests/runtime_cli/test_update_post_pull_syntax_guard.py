...
(e.g. orphan merge-conflict markers in ``superforecasting_agent/runtime/config.py``), the CLI
...
markers in ``superforecasting_agent/runtime/config.py``; users who ran ``hermes update`` in
...
def test_validate_critical_files_syntax_detects_conflict_markers(tmp_path):
    ...
    _populate_critical_tree(tmp_path, broken_file="superforecasting_agent/runtime/config.py")

    ok, failing_path, error = hermes_main._validate_critical_files_syntax(tmp_path)

    assert ok is False
    assert failing_path is not None and failing_path.endswith("superforecasting_agent/runtime/config.py")
...
def test_validate_critical_files_syntax_detects_break_in_main_py(tmp_path):
    _populate_critical_tree(tmp_path, broken_file="superforecasting_agent/runtime/main.py")

    ok, failing_path, _ = hermes_main._validate_critical_files_syntax(tmp_path)

    assert ok is False
    assert failing_path is not None and failing_path.endswith("superforecasting_agent/runtime/main.py")
...
def test_validate_critical_files_syntax_tolerates_missing_files(tmp_path):
    ...
    # Populate everything except superforecasting_agent/constants.py
    for relpath in hermes_main._UPDATE_CRITICAL_FILES:
        if relpath == "superforecasting_agent/constants.py":
            continue
