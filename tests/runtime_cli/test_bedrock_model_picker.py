...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (PROVIDER_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via PROVIDER_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in PROVIDER_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from superforecasting_agent.runtime.providers import PROVIDER_OVERLAYS
        assert "bedrock" in PROVIDER_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from superforecasting_agent.runtime.providers import PROVIDER_OVERLAYS
        assert PROVIDER_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from superforecasting_agent.runtime.providers import PROVIDER_OVERLAYS
        assert PROVIDER_OVERLAYS["bedrock"].auth_type == "aws_sdk"
