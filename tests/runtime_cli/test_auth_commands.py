...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_agent_oauth_login_pure",
        lambda: {
            "access_token": token,
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "superforecasting_agent.runtime.auth._nous_device_code_login",
        lambda **kwargs: {
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "superforecasting_agent.runtime.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "superforecasting_agent.runtime.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
    ...
    # The add path creates a distinct, self-contained ``manual:device_code``
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("superforecasting_agent.runtime.auth._codex_device_code_login", lambda: next(logins))
    ...
    payload = json.loads((tmp_path / "hermes" / "auth.json").read_text())
    # No singleton block — the add path is pool-only.
    assert "openai-codex" not in payload.get("providers", {})
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """auth add xai-oauth must write providers singleton and set active_provider.
    ...
    monkeypatch.setattr(
        "superforecasting_agent.runtime.auth._xai_oauth_loopback_login",
        lambda **kwargs: {
            "tokens": {
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from superforecasting_agent.runtime.auth_commands import auth_reset_command
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI OAuth (ChatGPT)." in out
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from superforecasting_agent.runtime.auth import _reset_config_provider
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from superforecasting_agent.runtime.auth_commands import auth_list_command
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import superforecasting_agent.runtime.copilot_auth as ca
...
def test_seed_from_singletons_respects_hermes_pkce_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(aa.credential_service, "read_agent_oauth_credentials", lambda: {
