"""Tests for dashboard ``--stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-update cleanup, so
the heavy coverage of SIGTERM / SIGKILL / Windows taskkill lives in
``test_update_stale_dashboard.py``. This file just verifies the flag dispatch:
argparse wiring, no-op when nothing is running, and correct exit codes.
"""
...
class TestDashboardStop:
    ...
    def test_stop_kills_and_exits_zero_when_all_killed(self, capsys):
        ...
        with patch("superforecasting_agent.runtime.main._find_stale_dashboard_pids",
             ...
             patch("superforecasting_agent.runtime.main._kill_stale_dashboard_processes") as mock_kill, \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(stop=True))
    ...
    def test_stop_exits_nonzero_if_kill_leaves_survivors(self):
        ...
        with patch("superforecasting_agent.runtime.main._find_stale_dashboard_pids",
                   side_effect=lambda: next(scans)), \
             patch("superforecasting_agent.runtime.main._kill_stale_dashboard_processes"), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(stop=True))
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``superforecasting-agent dashboard --stop`` must not end up ALSO
    ...
    def test_stop_does_not_fall_through_to_server_start(self):
        ...
        with patch("superforecasting_agent.runtime.main._find_stale_dashboard_pids",
                   return_value=[]), \
             patch.dict(sys.modules, {"superforecasting_agent.runtime.web_server": fake_ws}), \
             pytest.raises(SystemExit):
            cmd_dashboard(_ns(stop=True))
...
class TestArgparseWiring:
    ...
    dashboard ``--stop`` / ``--status`` actually parse."""

    def test_flags_are_registered(self):
        from superforecasting_agent.runtime.main import main as _cli_main  # noqa: F401
        ...
        import importlib
        mod = importlib.import_module("superforecasting_agent.runtime.main")
