...
def _stub_tts(monkeypatch):
    ...
    monkeypatch.setattr("superforecasting_agent.runtime.setup.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("superforecasting_agent.runtime.setup.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_gateway_skips_service_install_when_systemctl_missing(monkeypatch, capsys):
    ...
    import superforecasting_agent.runtime.gateway as gateway_mod
    ...
    assert "superforecasting-agent gateway" in out
...
def test_setup_gateway_in_container_shows_docker_guidance(monkeypatch, capsys):
    ...
    monkeypatch.setattr(superforecasting_agent.constants, "is_container", lambda: True)
...
def test_select_provider_and_model_accepts_named_provider_from_providers_section(
    ...
    monkeypatch.setattr(
        "superforecasting_agent.runtime.main._prompt_provider_choice",
...
def test_modal_setup_can_use_nous_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("superforecasting_agent.runtime.setup.managed_nous_tools_enabled", lambda: True)
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    config = load_config()
    ...
    monkeypatch.setattr("superforecasting_agent.runtime.setup.prompt", fake_prompt)
    monkeypatch.setattr("superforecasting_agent.runtime.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "superforecasting_agent.runtime.setup.get_nous_subscription_features",
    ...
    from superforecasting_agent.runtime.setup import setup_terminal_backend
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("superforecasting_agent.runtime.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("superforecasting_agent.runtime.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("superforecasting_agent.runtime.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "superforecasting_agent.runtime.setup.get_nous_subscription_features",
        lambda config: type("Features", (), {"nous_auth_present": True})(),
