...
def _make_run_side_effect(
    ...
    def side_effect(cmd, **kwargs):
        ...
        if "status" in joined and "--porcelain" in joined:
            out = " M superforecasting_agent/runtime/main.py\n" if dirty else ""
...
class TestUpdateYesConfigMigration:
    ...
    @patch("superforecasting_agent.runtime.config.migrate_config")
    @patch("superforecasting_agent.runtime.config.check_config_version", return_value=(1, 2))
    @patch("superforecasting_agent.runtime.config.get_missing_config_fields", return_value=[])
    @patch("superforecasting_agent.runtime.config.get_missing_env_vars", return_value=["NEW_KEY"])
    ...
    def test_no_yes_flag_still_prompts_in_tty(
        ...
        # real ``sys`` module instead of replacing ``superforecasting_agent.runtime.main.sys`` with
        # a MagicMock. The MagicMock approach was flaky under ``pytest-xdist``
        # — a sibling test that imported ``superforecasting_agent.runtime.main`` first could leave
