"""Tests for the Kanban DB layer (superforecasting_agent.runtime.kanban_db)."""
...
def test_connect_migrates_legacy_db_before_optional_column_indexes(tmp_path):
    ...
    with kb.connection(db_path) as migrated:
        task_columns = {
...
def test_create_task_unknown_parent_errors(kanban_home):
    with kb.connection() as conn, pytest.raises(ValueError, match="unknown parent"):
        kb.create_task(conn, title="orphan", parents=["t_ghost"])


def test_workspace_kind_validation(kanban_home):
    with kb.connection() as conn, pytest.raises(ValueError, match="workspace_kind"):
...
def test_branch_name_requires_worktree_workspace(kanban_home):
    with kb.connection() as conn, pytest.raises(ValueError, match="worktree"):
        kb.create_task(
            conn,
            title="bad branch",
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import superforecasting_agent.runtime.kanban_db as _kb
...
class TestSharedBoardPaths:
    ...
    def test_docker_custom_hermes_home_uses_env_path_directly(
        ...
        # `get_default_agent_root()` returns env_home directly when it
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # `get_default_agent_root()` walks up to /opt/hermes because
    ...
    def test_hermes_kanban_db_pin_beats_kanban_home(
        ...
        # HERMES_KANBAN_HOME and the `get_default_agent_root()` path.
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see superforecasting_agent.storage.session.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_resolve_hermes_argv_prefers_path_shim(monkeypatch):
    ...
    import shutil
    import superforecasting_agent.runtime.kanban_db as kb
...
def test_resolve_hermes_argv_forecast_bin_wins_over_hermes_bin(monkeypatch, tmp_path):
    """FORECAST_BIN is the short fork-native override before legacy HERMES_BIN."""
    import shutil
    import superforecasting_agent.runtime.kanban_db as kb

    short = tmp_path / "bin" / "forecast"
    legacy = tmp_path / "bin" / "hermes"
    short.parent.mkdir()
    short.write_text("#!/bin/sh\n", encoding="utf-8")
    legacy.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.delenv("SUPERFORECASTING_AGENT_BIN", raising=False)
    monkeypatch.setenv("FORECAST_BIN", str(short))
...
def test_resolve_hermes_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m superforecasting_agent.runtime.main`.
...
def test_resolve_hermes_argv_module_actually_runs():
    ...
    sufficient — if `superforecasting_agent.runtime.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m superforecasting_agent.runtime.main --version`
...
def test_create_task_without_workspace_inherits_board_default_workdir(kanban_home, monkeypatch):
    ...
    with kb.connection(board="work-proj") as conn:
...
def test_create_task_without_workspace_no_default_stays_none(kanban_home):
    ...
    with kb.connection(board="empty-board") as conn:
...
def test_create_task_with_explicit_workspace_ignores_board_default(kanban_home):
    ...
    with kb.connection(board="custom-ws-board") as conn:
