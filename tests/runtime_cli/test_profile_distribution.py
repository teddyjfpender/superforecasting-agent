"""Tests for superforecasting_agent.runtime.profile_distribution — git-based profile installs.
...
# ---------------------------------------------------------------------------
# Isolated profile env (matches tests/runtime_cli/test_profiles.py)
# ---------------------------------------------------------------------------
...
class TestInstall:
    ...
    def test_install_enforces_hermes_requires(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr(superforecasting_agent.runtime, "__version__", "0.1.0", raising=False)
...
class TestUpdate:
    ...
    def test_update_missing_manifest_errors(self, profile_env):
        ...
        from superforecasting_agent.runtime.profiles import create_profile
...
class TestSecurity:
    ...
    def test_user_owned_exclude_covers_credentials(self):
        assert "hermes_state.db" in USER_OWNED_EXCLUDE
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        from superforecasting_agent.runtime.profiles import get_profile_dir
        ...
        monkeypatch.setattr(
            "superforecasting_agent.runtime.profile_distribution.datetime", _FakeDT, raising=True
        )

        from superforecasting_agent.runtime.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_plain_profile_has_no_distribution_fields(self, profile_env):
        from superforecasting_agent.runtime.profiles import create_profile, list_profiles
        create_profile(name="plain", no_alias=True)
        rows = {p.name: p for p in list_profiles()}
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from superforecasting_agent.runtime.profiles import create_profile, list_profiles, get_profile_dir
