...
from superforecasting_agent.runtime import claw as claw_mod
...
class TestFindMigrationScript:
    ...
    def test_native_name_and_legacy_installed_fallback(self, tmp_path, native_exists):
        native = tmp_path / "installed" / "openclaw_to_forecast.py"
        native.parent.mkdir()
        legacy = native.with_name("openclaw_to_hermes.py")
        legacy.write_text("# legacy installed helper")
        if native_exists:
            native.write_text("# native installed helper")
        with (
            patch.object(claw_mod, "_OPENCLAW_SCRIPT", tmp_path / "bundled" / native.name),
            patch.object(claw_mod, "_OPENCLAW_SCRIPT_INSTALLED", native),
        ):
            assert claw_mod._find_migration_script() == (native if native_exists else legacy)
