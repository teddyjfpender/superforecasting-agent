"""Tests for superforecasting_agent.runtime.skin_engine — the data-driven skin/theme system."""
...
def reset_skin_state():
    ...
    from superforecasting_agent.runtime import skin_engine
    skin_engine._active_skin = None
    skin_engine._active_skin_name = skin_engine._RUNTIME_DEFAULT_SKIN
    yield
    skin_engine._active_skin = None
    skin_engine._active_skin_name = skin_engine._RUNTIME_DEFAULT_SKIN
...
class TestSkinConfig:
    def test_default_skin_has_required_fields(self):
        from superforecasting_agent.runtime.skin_engine import load_skin
    ...
    def test_get_color_with_fallback(self):
        from superforecasting_agent.runtime.skin_engine import load_skin
        skin = load_skin("default")
        assert skin.get_color("banner_title") == "#E9E5F4"
...
class TestBuiltinSkins:
    ...
    def test_unknown_skin_falls_back_to_runtime_default(self):
        from superforecasting_agent.runtime.skin_engine import load_skin
        skin = load_skin("nonexistent_skin_xyz")
        assert skin.name == "forecast"

    def test_all_builtin_skins_have_complete_colors(self):
        from superforecasting_agent.runtime.skin_engine import _BUILTIN_SKINS, _build_skin_config
...
class TestSkinManagement:
    def test_set_active_skin(self):
        from superforecasting_agent.runtime.skin_engine import set_active_skin, get_active_skin, get_active_skin_name
        skin = set_active_skin("ares")
        assert skin.name == "ares"
        assert get_active_skin_name() == "ares"
        assert get_active_skin().name == "ares"

    def test_get_active_skin_uses_runtime_default(self):
        from superforecasting_agent.runtime.skin_engine import get_active_skin
        skin = get_active_skin()
        assert skin.name == "forecast"

    def test_set_unknown_skin_reports_loaded_runtime_default(self):
        from superforecasting_agent.runtime.skin_engine import set_active_skin, get_active_skin_name

        skin = set_active_skin("nonexistent_skin_xyz")

        assert skin.name == "forecast"
        assert get_active_skin_name() == "forecast"
    ...
    def test_init_skin_from_config(self):
        from superforecasting_agent.runtime.skin_engine import init_skin_from_config, get_active_skin_name
    ...
    def test_init_skin_from_null_display(self):
        """display: null should fall back to the runtime default, not crash."""
        from superforecasting_agent.runtime.skin_engine import init_skin_from_config, get_active_skin_name
        init_skin_from_config({"display": None})
        assert get_active_skin_name() == "forecast"

    def test_init_skin_from_non_dict_display(self):
        """display: <non-dict> should fall back to the runtime default."""
        from superforecasting_agent.runtime.skin_engine import init_skin_from_config, get_active_skin_name
...
class TestUserSkins:
    def test_load_user_skin_from_yaml(self, tmp_path, monkeypatch):
        from superforecasting_agent.runtime.skin_engine import load_skin, _skins_dir
        ...
        # Patch skins dir
        monkeypatch.setattr("superforecasting_agent.runtime.skin_engine._skins_dir", lambda: skins_dir)
        ...
        assert skin.get_color("banner_border") == "#3A3350"  # from default
...
class TestDisplayIntegration:
    ...
    def test_get_skin_tool_prefix_custom(self):
        from superforecasting_agent.runtime.skin_engine import set_active_skin
        from agent.display import get_skin_tool_prefix
...
class TestCliBrandingHelpers:
    ...
    def test_active_prompt_symbol_ares(self):
        from superforecasting_agent.runtime.skin_engine import set_active_skin, get_active_prompt_symbol

        set_active_skin("ares")
        assert get_active_prompt_symbol() == "⚔ "

    def test_active_help_header_ares(self):
        from superforecasting_agent.runtime.skin_engine import set_active_skin, get_active_help_header

        set_active_skin("ares")
        assert get_active_help_header() == "Forecast Desk Commands"

    def test_active_goodbye_ares(self):
        from superforecasting_agent.runtime.skin_engine import set_active_skin, get_active_goodbye

        set_active_skin("ares")
        assert get_active_goodbye() == "Goodbye."

    def test_prompt_toolkit_style_overrides_cover_tui_classes(self):
        from superforecasting_agent.runtime.skin_engine import set_active_skin, get_prompt_toolkit_style_overrides
        set_active_skin("ares")
