...
from superforecasting_agent.runtime.skills_hub import (
    do_check,
    do_install,
    do_list,
    do_search,
...
def _capture_update(monkeypatch, results) -> tuple[str, list[tuple[str, str, bool]]]:
    import tools.skills_hub as hub
    import superforecasting_agent.runtime.skills_hub as cli_hub
...
def test_skills_command_usage_prefers_forecast_native_cli(monkeypatch):
    sink = StringIO()
    console = Console(file=sink, force_terminal=False, color_system=None, width=120)
    monkeypatch.setattr("superforecasting_agent.runtime.skills_hub._console", console)
