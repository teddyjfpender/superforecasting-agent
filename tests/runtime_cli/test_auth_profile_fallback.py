...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    ...
    from superforecasting_agent.credentials.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: HERMES_HOME is set to a custom path that is NOT under
    # ~/.hermes/profiles/ — get_default_agent_root() returns HERMES_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from superforecasting_agent.credentials.auth import read_credential_pool, write_credential_pool
