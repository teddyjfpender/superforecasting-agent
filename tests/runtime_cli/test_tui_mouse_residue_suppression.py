...
from superforecasting_agent.runtime.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    ...
    def test_writes_disable_sequence_when_hermes_tui_env_set(self, monkeypatch):
        # HERMES_TUI remains a live alias in this fork.
        monkeypatch.setattr(sys, "argv", ["hermes"])
        monkeypatch.delenv("SUPERFORECASTING_AGENT_TUI", raising=False)
        monkeypatch.delenv("FORECAST_TUI", raising=False)
    ...
    def test_writes_disable_sequence_when_canonical_tui_env_set(self, monkeypatch):
        # Canonical rebranded env var triggers the same suppression.
        monkeypatch.setattr(sys, "argv", ["superforecasting-agent"])
        monkeypatch.delenv("HERMES_TUI", raising=False)
        monkeypatch.delenv("FORECAST_TUI", raising=False)
        monkeypatch.setenv("SUPERFORECASTING_AGENT_TUI", "1")
        monkeypatch.delenv("SUPERFORECASTING_AGENT_TUI_NO_EARLY_DISABLE", raising=False)
        monkeypatch.delenv("HERMES_TUI_NO_EARLY_DISABLE", raising=False)
