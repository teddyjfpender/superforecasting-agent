...
def test_ensure_dependency_skips_when_present():
    ...
    with patch("superforecasting_agent.runtime.dep_ensure.shutil") as mock_shutil:
...
def test_ensure_dependency_returns_false_when_missing_noninteractive():
    ...
    with patch("superforecasting_agent.runtime.dep_ensure.shutil") as mock_shutil:
        ...
        with patch("superforecasting_agent.runtime.dep_ensure._find_install_script", return_value=(None, None)):
            result = ensure_dependency("node", interactive=False)
...
def test_find_install_script_from_checkout(tmp_path):
    ...
    from superforecasting_agent.runtime.dep_ensure import _find_install_script
    scripts_dir = tmp_path / "scripts"
    scripts_dir.mkdir()
    (scripts_dir / "install.sh").write_text("#!/bin/bash", encoding="utf-8")
    with patch("superforecasting_agent.runtime.dep_ensure._IS_WINDOWS", False):
        path, shell = _find_install_script(package_dir=tmp_path / 'superforecasting_agent/runtime', repo_root=tmp_path)
...
def test_find_install_script_from_wheel(tmp_path):
    ...
    from superforecasting_agent.runtime.dep_ensure import _find_install_script
    bundled = tmp_path / 'superforecasting_agent/runtime' / "scripts"
...
def test_find_install_script_prefers_ps1_on_windows(tmp_path):
    ...
    scripts_dir = tmp_path / 'superforecasting_agent/runtime' / "scripts"
    scripts_dir.mkdir(parents=True)
    (scripts_dir / "install.ps1").write_text("# fake")
    (scripts_dir / "install.sh").write_text("# fake")
    from superforecasting_agent.runtime.dep_ensure import _find_install_script
    with patch("superforecasting_agent.runtime.dep_ensure._IS_WINDOWS", True):
        path, shell = _find_install_script(package_dir=tmp_path / 'superforecasting_agent/runtime')
...
def test_find_install_script_falls_back_to_repo_root(tmp_path):
    ...
    from superforecasting_agent.runtime.dep_ensure import _find_install_script
    with patch("superforecasting_agent.runtime.dep_ensure._IS_WINDOWS", False):
        path, shell = _find_install_script(package_dir=tmp_path / 'superforecasting_agent/runtime', repo_root=repo_root)
...
def test_has_system_browser_checks_windows_names():
    from superforecasting_agent.runtime.dep_ensure import _has_system_browser
    with patch("superforecasting_agent.runtime.dep_ensure._IS_WINDOWS", True), \
         patch("superforecasting_agent.runtime.dep_ensure.shutil") as mock_shutil:
...
def test_has_hermes_agent_browser_windows_path(tmp_path):
    ...
    from superforecasting_agent.runtime.dep_ensure import _has_agent_browser
    with patch("superforecasting_agent.runtime.dep_ensure._IS_WINDOWS", True), \
         patch("superforecasting_agent.constants.get_agent_home", return_value=tmp_path):
        assert _has_agent_browser() is True
...
def test_ensure_dependency_uses_powershell_on_windows(tmp_path):
    ...
    (scripts_dir / "install.ps1").write_text("# fake")
    with patch("superforecasting_agent.runtime.dep_ensure._IS_WINDOWS", True), \
         patch("superforecasting_agent.runtime.dep_ensure._DEP_CHECKS", {"node": lambda: False}), \
         patch("superforecasting_agent.runtime.dep_ensure._find_install_script", return_value=(scripts_dir / "install.ps1", "powershell")), \
         patch("superforecasting_agent.runtime.dep_ensure.shutil") as mock_shutil, \
         patch("superforecasting_agent.constants.get_agent_home", return_value=tmp_path / "fakehome"), \
        ...
        assert "-ForecastHome" in cmd
        assert "-HermesHome" not in cmd
