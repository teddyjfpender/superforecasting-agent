...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hermes` should use `superforecasting_agent.runtime.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "hermes"
    called = []

    fake_main_module = types.ModuleType("superforecasting_agent.runtime.main")

    def fake_main():
        called.append("superforecasting_agent.runtime.main")
    ...
    assert called == ["superforecasting_agent.runtime.main"]
...
def test_launcher_late_missing_runtime_dependency_points_to_forecast_desk(
    ...
    assert exc.value.code == 1
    captured = capsys.readouterr()
    # Same fork-native guidance for late import failures inside superforecasting_agent.runtime.main.
    assert "needs optional CLI runtime" in captured.err
    assert "forecast desk" in captured.err
    assert 'uv pip install -e ".[all,dev]"' in captured.err
    assert "Traceback" not in captured.err
