"""Tests for superforecasting_agent/runtime/completion.py — shell completion script generation."""
...
from superforecasting_agent.runtime.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real CLI structure."""
    p = argparse.ArgumentParser(prog="superforecasting-agent")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_superforecasting_agent_completion()" in out
        assert "complete -F _superforecasting_agent_completion superforecasting-agent hermes" in out
...
class TestGenerateZsh:
    def test_contains_compdef_header(self):
        ...
        assert "#compdef superforecasting-agent hermes" in out
    ...
    def test_registers_compdef_instead_of_invoking_completion_function(self):
        out = generate_zsh(_make_parser())
        assert 'compdef _superforecasting_agent superforecasting-agent hermes' in out
        assert '_superforecasting_agent "$@"' not in out
    ...
    def test_preserves_valid_zsh_arguments_alias_syntax(self):
        ...
        assert "'(-)'{-p,--profile}'[Profile name]:profile:_superforecasting_agent_profiles'" in out
        ...
        try:
            result = subprocess.run(
                [
                    "zsh",
                    "-fc",
                    f"autoload -Uz compinit && compinit -D; source {path}; [[ ${{_comps[superforecasting-agent]}} == _superforecasting_agent && ${{_comps[hermes]}} == _superforecasting_agent ]]",
                ],
                capture_output=True,
                text=True,
            )
...
class TestGenerateFish:
    def test_disables_file_completion(self):
        ...
        assert "complete -c superforecasting-agent -f" in out
...
class TestSubcommandDrift:
    def test_SUBCOMMANDS_covers_required_commands(self):
        ...
        """
        import inspect
        from superforecasting_agent.runtime.main import _coalesce_session_name_args
...
class TestProfileCompletion:
    ...
    def test_bash_has_profiles_helper(self):
        out = generate_bash(_make_parser())
        assert "_superforecasting_agent_profiles()" in out
        assert 'profiles_dir="${SUPERFORECASTING_AGENT_HOME:-$HOME/.superforecasting-agent}/profiles"' in out
        assert 'legacy_profiles_dir="${HERMES_HOME:-$HOME/.hermes}/profiles"' in out
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'superforecasting-agent profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have profile completion for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_superforecasting_agent_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with profile names"

    def test_zsh_has_profiles_helper(self):
        out = generate_zsh(_make_parser())
        assert "_superforecasting_agent_profiles()" in out
        assert ".superforecasting-agent}/profiles" in out
        assert ".hermes}/profiles" in out
    ...
    def test_fish_has_profiles_helper(self):
        out = generate_fish(_make_parser())
        assert "__superforecasting_agent_profiles" in out
        assert "$HOME/.superforecasting-agent/profiles" in out
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__superforecasting_agent_profiles" in out
        count = out.count("(__superforecasting_agent_profiles)")
