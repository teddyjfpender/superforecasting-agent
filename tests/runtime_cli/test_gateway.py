"""Tests for superforecasting_agent.runtime.gateway."""
...
import superforecasting_agent.runtime.gateway as gateway
...
def test_run_gateway_refuses_root_in_official_docker(monkeypatch, tmp_path, capsys):
    project_root = tmp_path / "opt" / "superforecasting-agent"
    ...
    assert exc_info.value.code == 1
    out = capsys.readouterr().out
    assert "Refusing to run the Superforecasting Agent gateway as root" in out
    assert str(project_root / "docker" / "entrypoint.sh") in out
...
def test_gateway_restart_on_windows_without_service_uses_detached_backend(monkeypatch):
    ...
    """
    import superforecasting_agent.runtime.gateway_windows as gateway_windows
...
def test_conflicting_systemd_units_warning(monkeypatch, tmp_path, capsys):
    ...
    assert "superforecasting-agent gateway uninstall" in out
...
def test_install_linux_gateway_from_setup_system_choice_without_root_prints_followup(monkeypatch, capsys):
    ...
    assert "sudo superforecasting-agent gateway install --system --run-as-user alice" in out
    assert "sudo superforecasting-agent gateway start --system" in out
...
def test_module_has_logger():
    ...
    assert gateway.logger.name == "superforecasting_agent.runtime.gateway"


@pytest.mark.parametrize("install_path, expected", [
    ("/opt/superforecasting-agent", True),
    ("/opt/hermes", True),
    ("/tmp/source-checkout", False),
])
def test_official_docker_checkout_paths(monkeypatch, install_path, expected):
    from pathlib import Path

    monkeypatch.setattr(gateway, "PROJECT_ROOT", Path(install_path))
    monkeypatch.setattr(Path, "is_file", lambda path: path.name == "entrypoint.sh")
    assert gateway._is_official_docker_checkout() is expected
