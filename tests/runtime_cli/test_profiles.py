"""Comprehensive tests for superforecasting_agent.runtime.profiles module.
...
def profile_env(tmp_path, monkeypatch):
    ...
    * HERMES_HOME  -> tmp_path/.hermes  (so get_agent_home() agrees)
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("superforecasting_agent.runtime.profiles._cleanup_gateway_service"):
...
class TestWrapperScripts:
    ...
    def test_wrapper_prefers_forecast_native_command(self, profile_env):
        ...
        with patch("superforecasting_agent.runtime.profiles._get_wrapper_dir", return_value=wrapper_dir):
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        with patch("superforecasting_agent.runtime.profiles.check_alias_collision", return_value="skip"):
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for f in ("state.db", "hermes_state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".forecast_history", ".hermes_history",
        ...
        excluded_files = [
            "default/state.db", "default/hermes_state.db", "default/gateway.pid",
            "default/gateway_state.json", "default/processes.json",
            "default/errors.log", "default/.forecast_history", "default/.hermes_history",
...
class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_agent_home()."""
    ...
    def test_profiles_root_uses_fork_native_default_without_legacy_home(
        ...
        assert _get_default_agent_home() == tmp_path / ".superforecasting-agent"
    ...
    def test_default_hermes_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_agent_home() returns HERMES_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HERMES_HOME", str(docker_home))
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        ...
        from superforecasting_agent.runtime.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HERMES_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from superforecasting_agent.runtime.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".hermes"
