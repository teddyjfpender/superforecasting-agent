"""Tests for superforecasting_agent/runtime/webhook.py — webhook subscription CLI."""
...
from superforecasting_agent.runtime.webhook import (
    webhook_command,
    _load_subscriptions,
    _save_subscriptions,
    _subscriptions_path,
...
def _isolate(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "superforecasting_agent.runtime.webhook._is_webhook_enabled", lambda: True
    )
...
class TestWebhookEnabledGate:
    def test_blocks_when_disabled(self, capsys, monkeypatch):
        monkeypatch.setattr("superforecasting_agent.runtime.webhook._is_webhook_enabled", lambda: False)
...
def test_missing_webhook_action_uses_forecast_native_usage(capsys):
    ...
    def test_real_check_disabled(self, monkeypatch):
        monkeypatch.setattr(
            "superforecasting_agent.runtime.webhook._get_webhook_config",
            lambda: {},
        )
        monkeypatch.setattr(
            "superforecasting_agent.runtime.webhook._is_webhook_enabled",
            lambda: bool({}.get("enabled")),
        )
        import superforecasting_agent.runtime.webhook as wh_mod
