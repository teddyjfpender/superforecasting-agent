"""Regression test: ``superforecasting-agent mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``superforecasting_agent/runtime/_parser.py``).  The dispatcher in ``superforecasting_agent/runtime/main.py``
...
result, ``superforecasting-agent mcp add foo --url ...`` silently launches chat instead
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`superforecasting-agent mcp add foo --url ...` keeps args.command == "mcp".
