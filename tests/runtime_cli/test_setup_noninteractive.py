...
import pytest
from superforecasting_agent.runtime.config import DEFAULT_CONFIG, load_config, save_config
...
class TestNonInteractiveSetup:
    ...
    def test_cmd_setup_allows_noninteractive_flag_without_tty(self):
        ...
        from superforecasting_agent.runtime.main import cmd_setup

        args = _make_setup_args(non_interactive=True)

        with (
            patch("superforecasting_agent.runtime.setup.run_setup_wizard") as mock_run_setup,
    ...
    def test_non_interactive_flag_skips_wizard(self, capsys):
        ...
        assert "superforecasting-agent config set model.provider custom" in out
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        """Bare CLI should not prompt for input when no provider exists and stdin is headless."""
        from superforecasting_agent.runtime.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("superforecasting_agent.runtime.main._has_any_provider_configured", return_value=False),
            patch("superforecasting_agent.runtime.main.cmd_setup") as mock_setup,
            patch("sys.stdin") as mock_stdin,
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            mock_stdin.isatty.return_value = False
