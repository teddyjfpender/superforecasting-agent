"""Tests for superforecasting_agent.runtime.relaunch — unified self-relaunch utility."""

import sys

import pytest

from superforecasting_agent.runtime import relaunch as relaunch_mod


class TestResolveCliBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        fake = "/nix/store/abc/bin/superforecasting-agent"
        ...
        assert relaunch_mod.resolve_cli_bin() == fake

    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        fake = tmp_path / "superforecasting-agent"
        fake.write_text("#!/bin/sh\n")
        fake.chmod(0o755)
        monkeypatch.setattr(sys, "argv", [str(fake.name)])
        monkeypatch.chdir(tmp_path)
        # Ensure we don't accidentally match a real CLI on PATH.
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_cli_bin() == str(fake)

    def test_prefers_forecast_command_on_path(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])  # not a real path

        def fake_which(name):
            if name == "superforecasting-agent":
                return "/usr/bin/superforecasting-agent"
            if name == "hermes":
                return "/usr/bin/hermes"
            return None

        monkeypatch.setattr(relaunch_mod.shutil, "which", fake_which)
        assert relaunch_mod.resolve_cli_bin() == "/usr/bin/superforecasting-agent"

    def test_falls_back_to_legacy_path_which(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])  # not a real path
        monkeypatch.setattr(
            relaunch_mod.shutil,
            "which",
    ...
    def test_legacy_helper_name_delegates_to_forecast_lookup(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])
        monkeypatch.setattr(
            relaunch_mod.shutil,
            "which",
            lambda name: "/usr/bin/superforecasting-agent"
            if name == "superforecasting-agent"
            else None,
        )
        assert relaunch_mod.resolve_hermes_bin() == "/usr/bin/superforecasting-agent"

    def test_returns_none_when_unresolvable(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_cli_bin() is None
...
class TestBuildRelaunchArgv:
    def test_uses_bin_when_available(self, monkeypatch):
        monkeypatch.setattr(
            relaunch_mod, "resolve_cli_bin", lambda: "/usr/bin/superforecasting-agent"
        )
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv[0] == "/usr/bin/superforecasting-agent"

    def test_falls_back_to_python_module(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_cli_bin", lambda: None)
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv == [sys.executable, "-m", "superforecasting_agent.runtime.main", "--resume", "abc"]
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/superforecasting-agent", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [
            (
                "/usr/bin/superforecasting-agent",
                ["/usr/bin/superforecasting-agent", "--resume", "abc"],
            )
        ]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        the CLI). relaunch() must detect win32 and use subprocess.run +
        sys.exit instead."""
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(
            relaunch_mod,
            "resolve_cli_bin",
            lambda: r"C:\Users\test\superforecasting-agent.exe",
        )
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\superforecasting-agent.exe", "chat"]]
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(
            relaunch_mod, "resolve_cli_bin", lambda: r"C:\superforecasting-agent.exe"
        )
    ...
    def test_windows_surfaces_oserror_with_help(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_cli_bin", lambda: r"C:\missing.exe")
...
class TestResolveCliBinWindowsPyGuard:
    """On Windows, resolve_cli_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a superforecasting-agent.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\superforecasting-agent.exe"
            if name == "superforecasting-agent"
            else None,
        )

        bin_path = relaunch_mod.resolve_cli_bin()
        # Must NOT be the .py — must be the forecast-native PATH entry.
        assert bin_path == r"C:\venv\Scripts\superforecasting-agent.exe"
    ...
    def test_posix_still_accepts_py_argv0(self, monkeypatch, tmp_path):
        ...
        script = tmp_path / "superforecasting-agent"
        script.write_text("#!/usr/bin/env python3\n")
        script.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(script), "chat"])
        assert relaunch_mod.resolve_cli_bin() == str(script)

    def test_windows_py_argv0_with_no_cli_on_path_returns_none(self, monkeypatch, tmp_path):
        """If argv0 is .py on Windows and no CLI shim is on PATH, fall back."""
        script = tmp_path / "main.py"
        script.write_text("# stub")
