...
import superforecasting_agent.runtime.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_refresh_refuses_to_bake_any_tempdir_home_into_real_user_unit(
        ...
        poisons the unit identically (seen live upstream 2026-06-11 — an E2E
        probe ran ``gateway restart`` with a /tmp HERMES_HOME exported; the
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_tmp_home_in_alias_env_vars(self):
        # The fork bakes the agent home under alias names too — the guard
        # must catch a temp home in any of them.
        unit = (
            "[Service]\n"
            'Environment="SUPERFORECASTING_AGENT_HOME=/tmp/hermes-e2e-7"\n'
        )
        assert gateway_cli._temp_home_in_service_definition(unit) is not None
        unit = '[Service]\nEnvironment="FORECAST_HOME=/var/tmp/sf-home"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is not None
    ...
    def test_accepts_unit_without_hermes_home(self):
        unit = "[Service]\nExecStart=/usr/bin/python -m superforecasting_agent.runtime.main gateway run\n"
        assert gateway_cli._temp_home_in_service_definition(unit) is None
...
class TestGatewaySystemServiceRouting:
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: superforecasting-agent gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup superforecasting-agent gateway" in out
...
class TestSystemUnitHermesHome:
    ...
    def test_user_unit_unaffected_by_change(self):
        ...
        hermes_home = str(gateway_cli.get_agent_home().resolve())
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "superforecasting-agent gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_agent_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(gateway_cli, "get_agent_home", lambda: root_home / ".hermes")
...
class TestLegacyHermesUnitDetection:
    ...
    _OUR_UNIT_TEXT = (
        ...
        "ExecStart=/usr/bin/python -m superforecasting_agent.runtime.main gateway run --replace\n"
    )
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m superforecasting_agent.runtime.main gateway run
          - python path/to/superforecasting_agent/runtime/main.py gateway run
          - hermes gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m superforecasting_agent.runtime.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/hermes/superforecasting_agent/runtime/main.py gateway run",
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "superforecasting-agent gateway migrate-legacy" in out
...
class TestRemoveLegacyHermesUnits:
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo superforecasting-agent gateway migrate-legacy" in out
...
class TestMigrateLegacyCommand:
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import superforecasting_agent.runtime.main as cli_main
        ...
        if project_root is None:
            import superforecasting_agent.runtime.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "superforecasting_agent.runtime.main", "gateway", "--help"],
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "superforecasting_agent.runtime.main", "gateway", "status", "-l", "--help"],
