...
def fresh_home(tmp_path, monkeypatch):
    ...
    # Also reset superforecasting_agent.constants cache so get_default_agent_root() re-reads.
    try:
        import superforecasting_agent.constants
        superforecasting_agent.constants._cached_default_hermes_root = None  # type: ignore[attr-defined]
    except Exception:
        pass
...
class TestBoardCRUD:
    ...
    def test_remove_clears_init_cache_for_recreated_db(self, fresh_home, archive):
        ...
        with kb.connection(board="recycle") as conn:
...
class TestConnectionIsolation:
    def test_tasks_do_not_leak_across_boards(self, fresh_home):
        ...
        with kb.connection(board="default") as conn:
    ...
    def test_connect_without_args_uses_current(self, fresh_home):
        ...
        with kb.connection(board="curr") as conn:
    ...
    def test_connect_env_var_overrides_current(self, fresh_home, monkeypatch):
        ...
        with kb.connection(board="envwin") as conn:
            assert [t.title for t in kb.list_tasks(conn)] == ["via-env"]
        with kb.connection(board="persist") as conn:
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [sys.executable, "-m", "superforecasting_agent.runtime.main", "kanban"] + args,
        env=env,
        capture_output=True,
        text=True,
        cwd=str(_WORKTREE),
