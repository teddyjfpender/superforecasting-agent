...
Complements tests/runtime_cli/test_kanban_db.py (schema + CAS atomicity)
and tests/runtime_cli/test_kanban_cli.py (end-to-end run_slash).  The
...
def test_parse_duration_accepts_formats():
    from superforecasting_agent.runtime.kanban import _parse_duration
...
def test_cli_bulk_complete_with_summary_rejects(kanban_home):
    ...
    # Note: superforecasting_agent.runtime.main doesn't propagate sub-command exit codes
...
def test_cli_create_on_fresh_home_auto_inits(tmp_path, monkeypatch):
    ...
    r = _sp.run(
        [_sys.executable, "-m", "superforecasting_agent.runtime.main", "kanban",
...
def test_check_dispatcher_presence_silent_when_gateway_running(monkeypatch):
    from superforecasting_agent.runtime import kanban as kb_cli
...
def test_check_dispatcher_presence_warns_when_no_gateway(monkeypatch):
    ...
    assert "superforecasting-agent gateway start" in msg
...
def test_cli_create_warns_when_no_gateway(kanban_home, monkeypatch, capsys):
    ...
    assert "superforecasting-agent gateway start" in captured.err
...
def test_cli_create_silent_when_gateway_up(kanban_home, monkeypatch, capsys):
    ...
    assert kb_cli._cmd_create(ns) == 0
    captured = capsys.readouterr()
    assert "superforecasting-agent gateway start" not in captured.err
...
def test_cli_create_no_warn_on_triage(kanban_home, monkeypatch, capsys):
    ...
    assert "superforecasting-agent gateway start" not in err
...
def test_cli_daemon_without_force_prints_deprecation_exits_2(kanban_home, capsys):
    ...
    assert "superforecasting-agent gateway start" in err
...
def test_cli_daemon_help_marks_deprecated():
    ...
    assert found_deprecation, (
        ...
        "the migration guidance in kanban --help output"
    )
