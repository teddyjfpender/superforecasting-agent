"""Tests for superforecasting_agent.runtime.web_server and related config utilities."""
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hermes_home):
        ...
        from superforecasting_agent.runtime.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(superforecasting_agent.storage.session, "DEFAULT_DB_PATH", get_agent_home() / "state.db")
    ...
    def test_legacy_dashboard_session_header_still_accepted(self):
        from starlette.testclient import TestClient
        from superforecasting_agent.runtime.web_server import app, _SESSION_TOKEN
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from superforecasting_agent.runtime.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_no_token(self, tmp_path):
        ...
        from starlette.testclient import TestClient
        from superforecasting_agent.runtime.web_server import app
        from superforecasting_agent.runtime.config import save_env_value
    ...
    def test_reveal_env_var_bad_token(self, tmp_path):
        ...
        from superforecasting_agent.runtime.config import save_env_value
        from superforecasting_agent.runtime.web_server import _SESSION_HEADER_NAME
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from superforecasting_agent.runtime.config import save_env_value
        from superforecasting_agent.runtime.web_server import _SESSION_TOKEN
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/superforecasting_agent/runtime/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from superforecasting_agent.runtime.web_server import CONFIG_SCHEMA
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from superforecasting_agent.runtime.web_server import _build_schema_from_config
...
class TestNewEndpoints:
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        from superforecasting_agent.constants import get_agent_home
        import superforecasting_agent.runtime.profiles as profiles_mod
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_agent_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profiles_create_with_clone_from_default_copies_default_skills(self, monkeypatch):
        ...
        default_skill = get_agent_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = get_agent_home() / "profiles" / "cloned" / "skills" / "custom" / "new-skill" / "SKILL.md"
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_agent_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import superforecasting_agent.runtime.skills_config as skills_config
        import superforecasting_agent.runtime.web_server as web_server
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from superforecasting_agent.runtime.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from superforecasting_agent.runtime.web_server import _denormalize_config_from_web
...
class TestModelInfoEndpoint:
    ...
    def test_model_info_with_dict_config(self, monkeypatch):
        import superforecasting_agent.runtime.web_server as ws
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from superforecasting_agent.runtime.web_server import _normalise_theme_definition
...
class TestPluginAPIAuth:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        Use the read-only Kanban configuration endpoint. With a valid token
        the real plugin handler should run; without one the middleware
        should reject the request before the handler is reached.
        """
        # Without auth: middleware blocks before reaching the handler.
        resp = self.client.get("/api/plugins/kanban/config")
        assert resp.status_code == 401

        # With auth: handler runs.
        resp = self.auth_client.get("/api/plugins/kanban/config")
        assert resp.status_code == 200
        assert resp.json()["render_markdown"] is True
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_achievements_plugin_is_hidden_from_default_navigation(self, tmp_path, monkeypatch):
        ...
        web_server._dashboard_plugins_cache = None
        plugins = web_server._get_dashboard_plugins(force_rescan=True)
        assert "example" not in {p["name"] for p in plugins}
...
# or ``sh -c 'printf …'``) instead of the real Forecast Desk TUI command.  The
...
class TestPtyWebSocket:
    ...
    def _setup(self, monkeypatch, _isolate_hermes_home):
        ...
        # One server loop for publisher and subscriber connections, as in ASGI.
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from superforecasting_agent.runtime.pty_bridge import PtyUnavailableError
        ...
        import superforecasting_agent.runtime.web_server as ws_mod
    ...
    def test_pub_broadcasts_to_events_subscribers(self, monkeypatch):
        ...
        import time
        from urllib.parse import urlencode
        from superforecasting_agent.runtime import web_server as ws_mod
