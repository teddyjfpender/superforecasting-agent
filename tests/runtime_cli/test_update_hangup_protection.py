"""Tests for SIGHUP protection and stdout mirroring in update runs.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``superforecasting_agent/runtime/main.py``.  These exist so
that ``superforecasting-agent update`` survives a terminal disconnect mid-install
...
def _rebind_from_current_main():
    """Rebind the symbols under test from the *live* ``superforecasting_agent.runtime.main``.

    Sibling tests (e.g. ``test_curator_recent_run_notice.py``) call
    ``importlib.reload(superforecasting_agent.runtime.main)``, which re-executes the module IN
    ...
    import superforecasting_agent.runtime.main as _m

    _m = importlib.import_module("superforecasting_agent.runtime.main")
    g = globals()
...
class TestInstallHangupProtection:
    ...
    def test_installs_sighup_ignore(self, tmp_path, monkeypatch):
        ...
        # Clear cached get_agent_home if present
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            assert "superforecasting-agent update started" in contents
        finally:
            _finalize_update_output(state)
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_agent_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "superforecasting_agent.runtime.config.get_agent_home", _boom, raising=True
        )
