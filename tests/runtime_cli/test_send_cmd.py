...
``superforecasting_agent.runtime.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from superforecasting_agent.runtime import send_cmd
...
def test_load_hermes_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    # Force get_agent_home() to re-resolve under the patched env.
