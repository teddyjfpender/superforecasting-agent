...
from superforecasting_agent.runtime.main import cmd_update, PROJECT_ROOT

pytestmark = pytest.mark.usefixtures("git_installation")
...
class TestCmdUpdateSnapshotBranch:
    ...
    def test_update_refreshes_repo_and_tui_node_dependencies(
        ...
        from superforecasting_agent.runtime import main as hm
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "superforecasting_agent.runtime.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "superforecasting_agent.runtime.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("superforecasting_agent.runtime.config.check_config_version", return_value=(1, 2)), patch(
            "superforecasting_agent.runtime.config.migrate_config",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ), patch("superforecasting_agent.runtime.main.sys") as mock_sys:
            ...
            from superforecasting_agent.runtime.config import migrate_config
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        with (
            patch("superforecasting_agent.runtime.profiles.list_profiles", return_value=all_profiles),
            patch("superforecasting_agent.runtime.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("superforecasting_agent.runtime.profiles.list_profiles", return_value=[default_p]),
            patch("superforecasting_agent.runtime.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
