"""Tests for `superforecasting-agent fallback` chain management."""
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from superforecasting_agent.runtime.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestListCommand:
    def test_list_empty(self, isolated_home, capsys):
        ...
        from superforecasting_agent.runtime.fallback_cmd import cmd_fallback_list
        cmd_fallback_list(types.SimpleNamespace())
        out = capsys.readouterr().out
        assert "No fallback providers configured" in out
        assert "superforecasting-agent fallback add" in out
    ...
    def test_list_with_entries(self, isolated_home, capsys):
        ...
        assert "website/docs/user-guide/features/fallback-providers.md" in out
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("superforecasting_agent.runtime.main.select_provider_and_model", side_effect=fake_picker), \
                patch("superforecasting_agent.runtime.main._require_tty"):
            from superforecasting_agent.runtime.fallback_cmd import cmd_fallback_add
...
class TestRemoveCommand:
    def test_remove_empty_chain(self, isolated_home, capsys):
        _write_config(isolated_home, {})
        from superforecasting_agent.runtime.fallback_cmd import cmd_fallback_remove
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        with patch("superforecasting_agent.runtime.setup._curses_prompt_choice", return_value=1):
            from superforecasting_agent.runtime.fallback_cmd import cmd_fallback_remove
            cmd_fallback_remove(types.SimpleNamespace())
...
class TestClearCommand:
    def test_clear_empty_chain(self, isolated_home, capsys):
        _write_config(isolated_home, {})
        from superforecasting_agent.runtime.fallback_cmd import cmd_fallback_clear
...
class TestArgparseWiring:
    """Verify `superforecasting-agent fallback` is wired into main.py's argparse tree.
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "superforecasting_agent.runtime.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
