"""Tests for superforecasting_agent.runtime.tools_config platform tool persistence."""
...
def test_apply_toolset_change_from_default_does_not_enable_default_off_toolsets():
    ...
    with patch("superforecasting_agent.runtime.tools_config.save_config"):
...
def test_visible_providers_include_nous_subscription_when_logged_in(monkeypatch):
    monkeypatch.setattr("superforecasting_agent.runtime.tools_config.managed_nous_tools_enabled", lambda: True)
    config = {"model": {"provider": "nous"}}

    monkeypatch.setattr(
        "superforecasting_agent.runtime.nous_subscription.get_nous_auth_status",
...
def test_visible_providers_hide_nous_subscription_when_feature_flag_is_off(monkeypatch):
    monkeypatch.setattr("superforecasting_agent.runtime.tools_config.managed_nous_tools_enabled", lambda: False)
    config = {"model": {"provider": "nous"}}
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("superforecasting_agent.runtime.tools_config._run_post_setup", lambda key: None)
...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "superforecasting_agent.runtime.tools_config._toolset_has_keys",
    ...
    monkeypatch.setattr("superforecasting_agent.runtime.tools_config._prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "superforecasting_agent.runtime.tools_config._configure_tool_category_for_reconfig",
        lambda ts_key, cat, config: configured.append(ts_key),
    )
    monkeypatch.setattr("superforecasting_agent.runtime.tools_config.save_config", lambda config: None)
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    monkeypatch.setattr("superforecasting_agent.runtime.tools_config.managed_nous_tools_enabled", lambda: True)
    monkeypatch.setattr("superforecasting_agent.runtime.nous_subscription.managed_nous_tools_enabled", lambda: True)
    config = {
        "model": {"provider": "nous"},
    ...
    monkeypatch.setattr(
        "superforecasting_agent.runtime.tools_config._prompt_toolset_checklist",
    ...
    monkeypatch.setattr(
        "superforecasting_agent.runtime.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
        "superforecasting_agent.runtime.nous_subscription.get_nous_auth_status",
    ...
    configured = []
    monkeypatch.setattr(
        "superforecasting_agent.runtime.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from superforecasting_agent.runtime.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from superforecasting_agent.runtime.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from superforecasting_agent.runtime.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        ...
        from superforecasting_agent.runtime.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from superforecasting_agent.runtime.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("superforecasting_agent.runtime.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("superforecasting_agent.runtime.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("superforecasting_agent.runtime.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_get_platform_tools_unknown_platform_prefers_forecast_composite():
    ...
    with patch("superforecasting_agent.tooling.toolsets.TOOLSETS", fake_toolsets):
...
def test_discord_toolsets_not_available_on_other_platforms():
    ...
    from superforecasting_agent.runtime.tools_config import _toolset_allowed_for_platform
...
def test_save_platform_tools_strips_restricted_toolsets():
    ...
    from superforecasting_agent.runtime.tools_config import _save_platform_tools
    config = {}
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    rows in a row.
    """
    from superforecasting_agent.runtime.tools_config import _get_effective_configurable_toolsets
...
def test_reconfigure_provider_runs_post_setup_for_env_var_providers(
    ...
    called = []
    monkeypatch.setattr("superforecasting_agent.runtime.tools_config._run_post_setup", lambda key: called.append(key))
    monkeypatch.setattr("superforecasting_agent.runtime.tools_config.get_env_value", lambda k: None)
    monkeypatch.setattr("superforecasting_agent.runtime.tools_config._prompt", lambda *a, **kw: "")
    monkeypatch.setattr("superforecasting_agent.runtime.tools_config.save_env_value", lambda k, v: None)
...
def test_tools_command_guidance_is_fork_native(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr(superforecasting_agent.constants, "display_agent_home", lambda: str(tmp_path))
