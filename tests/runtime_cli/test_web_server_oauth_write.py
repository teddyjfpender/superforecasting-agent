...
from superforecasting_agent.runtime.web_server import _save_anthropic_oauth_creds
...
def test_dashboard_oauth_write_uses_atomic_replace_and_cleans_temp_files(
    ...
    # atomic_replace is imported inside the function from `utils`; patch the
    # underlying os.replace it calls so the temp file is left for the finally
    # block to clean up.
    monkeypatch.setattr('superforecasting_agent.storage.files.os.replace', flaky_replace)
