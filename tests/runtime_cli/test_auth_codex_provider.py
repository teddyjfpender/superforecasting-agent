...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``auth add openai-codex``.
    ...
    (hermes_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``auth add openai-codex`` for accounts B and C
    ...
    (hermes_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``auth add openai-codex`` (account B and account C).
...
def test_save_codex_tokens_still_refreshes_legacy_manual_alias(tmp_path, monkeypatch):
    ...
    ``auth add openai-codex`` as a workaround when there was no
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    assert "superforecasting-agent" not in rendered
