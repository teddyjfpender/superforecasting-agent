...
- ``superforecasting_agent.runtime.providers``        -- canonical provider identity + overlays
- ``superforecasting_agent.runtime.model_normalize``  -- per-provider name formatting
...
_HERMES_MODEL_WARNING = (
    ...
    "for use with Superforecasting Agent. They lack the tool-calling capabilities "
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by
    ``superforecasting-agent config set model.aliases.xxx``) and converts
    simple string entries (``ds-flash: deepseek/deepseek-v4-flash``) into
    DirectAlias objects. The provider is parsed from the ``provider/`` prefix
    in the value; if no slash, the current provider is used.
    """
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from superforecasting_agent.runtime.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check '{_PRIMARY_CLI} model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += f"\n\nRun '{_PRIMARY_CLI} doctor' — config issues detected:"
    ...
    # Mirrors the same logic in superforecasting_agent.runtime.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from superforecasting_agent/runtime/models.py (OPENROUTER_MODELS,
    ...
    # the hosted Superforecasting Agent model-catalog JSON so
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # section 2 (PROVIDER_OVERLAYS) with proper auth store checking.
    ...
    for pid, overlay in PROVIDER_OVERLAYS.items():
    ...
    # in PROVIDER_TO_MODELS_DEV or PROVIDER_OVERLAYS (keeps /model in sync
    # with `superforecasting-agent model`).
    try:
        from superforecasting_agent.runtime.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # (see superforecasting_agent/runtime/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # run_agent.py, superforecasting_agent/runtime/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`superforecasting_agent.runtime.models.fetch_openrouter_models`, which filters the
    ...
    """
    from superforecasting_agent.runtime.models import fetch_openrouter_models
