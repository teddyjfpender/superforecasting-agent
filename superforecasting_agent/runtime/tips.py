...
TIPS = [
    ...
    "superforecasting-agent dump outputs a compact setup summary — great for bug reports.",
    "superforecasting-agent config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "superforecasting-agent config edit opens config.yaml in your default editor.",
    "superforecasting-agent config check scans for missing or stale configuration options.",
    "superforecasting-agent sessions browse opens an interactive forecast-session picker with search.",
    "superforecasting-agent sessions stats shows session counts by platform and database size.",
    "superforecasting-agent sessions prune --older-than 30 cleans up old forecast sessions.",
    "superforecasting-agent skills search react --source skills-sh searches the skills.sh public directory.",
    "superforecasting-agent skills check scans installed hub skills for upstream updates.",
    "superforecasting-agent skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "superforecasting-agent skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "superforecasting-agent mcp add github --command npx adds MCP servers from the command line.",
    "superforecasting-agent mcp serve runs Superforecasting Agent itself as an MCP server for other agents.",
    "superforecasting-agent auth add lets you add multiple API keys for credential pool rotation.",
    "superforecasting-agent completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "superforecasting-agent logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "superforecasting-agent backup creates a zip backup of your entire Superforecasting Agent home directory.",
    "superforecasting-agent profile create coder creates an isolated profile that becomes its own command.",
    "superforecasting-agent profile create work --clone copies your current config and keys to a new profile.",
    "superforecasting-agent update syncs new bundled skills to ALL profiles automatically.",
    "superforecasting-agent gateway install sets up Superforecasting Agent as a system service (systemd/launchd).",
    "superforecasting-agent memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "superforecasting-agent webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: superforecasting-agent tools disables unused tools, superforecasting-agent skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "superforecasting-agent models routes vision, compression, and aux tasks to cheaper models without changing your main model.",
    ...
    "Set worktree: true in config.yaml to make explicit forecast-support sessions create a git worktree.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Superforecasting Agent tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'superforecasting-agent profile create coder' creates the 'coder' command.",
    "superforecasting-agent profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Superforecasting Agent prints a resume command with session ID and stats.",
    "superforecasting-agent sessions export backup.jsonl exports all sessions for backup or analysis.",
    "superforecasting-agent -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "superforecasting-agent skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: superforecasting-agent cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Superforecasting Agent runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "superforecasting-agent gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Superforecasting Agent via a local macOS server.",
    ...
    "SOUL.md at ~/.superforecasting-agent/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Superforecasting Agent loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — superforecasting-agent tools can toggle them per platform.",
    ...
    # --- Batch & Data ---
    "superforecasting_agent/trajectories/batch.py processes hundreds of prompts in parallel for training data generation.",
    ...
    "superforecasting-agent plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "superforecasting-agent status --deep runs deeper diagnostic checks across all components.",
    ...
    "The skills audit log at ~/.superforecasting-agent/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Every interrupt during an agent run is logged to ~/.superforecasting-agent/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'superforecasting-agent profile create ls' would be rejected.",
    "superforecasting-agent profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "superforecasting-agent claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via superforecasting-agent tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.superforecasting-agent/.env.",
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Superforecasting Agent is working.',
    ...
    "SUPERFORECASTING_AGENT_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.superforecasting-agent/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.superforecasting-agent/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'superforecasting-agent curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "superforecasting-agent curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'superforecasting-agent curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'superforecasting-agent auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'Drop a YAML in ~/.superforecasting-agent/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.superforecasting-agent/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.superforecasting-agent/cache/piper-voices/.',
    ...
    # --- Batch ---
    "superforecasting_agent/trajectories/batch.py --resume content-matches completed prompts by text so dataset reorders don't re-run finished work.",
    ...
    'superforecasting-agent fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'superforecasting-agent pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'superforecasting-agent setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'superforecasting-agent status --deep runs the full health sweep across every component; plain superforecasting-agent status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'SUPERFORECASTING_AGENT_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'SUPERFORECASTING_AGENT_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.hermes/plugins/ — trust-gated by design.',
    "SUPERFORECASTING_AGENT_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'SUPERFORECASTING_AGENT_ALLOW_PRIVATE_URLS=true lets web tools hit localhost/private networks; off by default in gateway mode.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Superforecasting Agent block commands when the tirith scanner itself errors out.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.superforecasting-agent/dashboard-plugins/.',
]
