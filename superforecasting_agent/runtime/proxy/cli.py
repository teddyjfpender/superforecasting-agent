"""CLI handlers for the ``superforecasting-agent proxy`` subcommand."""
...
from superforecasting_agent.runtime.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "superforecasting-agent proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"superforecasting-agent login {adapter.name}")
        print(
    ...
    print(
        f"Starting Superforecasting Agent proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Superforecasting Agent proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: superforecasting-agent proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``superforecasting-agent proxy <subcommand>``."""
    ...
    print(
        "superforecasting-agent proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  superforecasting-agent proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
