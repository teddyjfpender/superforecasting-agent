"""Superforecasting Agent OpenClaw migration commands.

Usage:
    superforecasting-agent claw migrate              # Preview then migrate
    superforecasting-agent claw migrate --dry-run    # Preview only, no changes
    superforecasting-agent claw migrate --yes        # Skip confirmation prompt
    superforecasting-agent claw migrate --preset full --overwrite --migrate-secrets
    superforecasting-agent claw migrate --no-backup  # Skip pre-migration snapshot
    superforecasting-agent claw cleanup              # Archive leftover OpenClaw dirs
    superforecasting-agent claw cleanup --dry-run    # Preview archive actions
"""
...
from superforecasting_agent.runtime.config import get_agent_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        f"{_PRODUCT_NAME} may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if the gateway is running with connected platforms.
    ...
    print()
    print_error(
        f"{_PRODUCT_NAME} gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info(f"Recommendation: stop the gateway first with '{_PRIMARY_CLI} stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_forecast.py script in known locations."""
    for candidate in [_OPENCLAW_SCRIPT, _OPENCLAW_SCRIPT_INSTALLED]:
        if candidate.exists():
            return candidate
        legacy = candidate.with_name("openclaw_to_hermes.py")
        if legacy.exists():
            return legacy
    return None
...
def _load_migration_module(script_path: Path):
    ...
    return load_migration_module(script_path)
...
def claw_command(args):
    """Route OpenClaw migration subcommands."""
    ...
    if action == "migrate":
        ...
        print(f"Usage: {_PRIMARY_CLI} claw <command> [options]")
        print()
        print("Commands:")
        print(f"  migrate          Migrate settings from OpenClaw to {_PRODUCT_NAME}")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print(f"Run '{_PRIMARY_CLI} claw <command> --help' for options.")
...
def _cmd_migrate(args):
    ...
    print(
        color(
            "│          ⚕ Superforecasting Agent — OpenClaw Migration │",
            Colors.MAGENTA,
        )
    )
    print(
    ...
    if not source_dir.is_dir():
        ...
        print_info(f"You can specify a custom path: {_PRIMARY_CLI} claw migrate --source /path/to/.openclaw")
        return
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in the runtime home. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: {_PRIMARY_CLI} claw migrate --yes")
            return
        if not prompt_yes_no("Proceed with migration?", default=True):
            print_info("Migration cancelled.")
            return

    # ── Phase 2b: Pre-apply backup of the runtime home ─────────
    # Delegates to superforecasting_agent.runtime.backup.create_pre_migration_backup(), which
    # shares implementation with the pre-update backup (same exclusion
    # rules, same SQLite safe-copy, zip format) so the archive is
    # restorable with the import command.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            from superforecasting_agent.runtime.backup import create_pre_migration_backup, _format_size
            ...
            if backup_archive:
                ...
                print_info(f"Restore with: {_PRIMARY_CLI} import {backup_archive.name}")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the runtime home."
            )
    ...
    # 'superforecasting-agent claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    print(
        color(
            "│          ⚕ Superforecasting Agent — OpenClaw Cleanup   │",
            Colors.MAGENTA,
        )
    )
    print(
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info(f"Aborted. Stop OpenClaw first, then re-run: {_PRIMARY_CLI} claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info(f"To execute, re-run with: {_PRIMARY_CLI} claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  {_PRIMARY_CLI} claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info(f"  {_PRIMARY_CLI} claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info(f"  {_PRIMARY_CLI} config set OPENROUTER_API_KEY sk-or-v1-...")
