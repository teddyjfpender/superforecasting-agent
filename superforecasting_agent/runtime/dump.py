"""
Dump command for the Superforecasting Agent CLI.

Outputs a compact, plain-text summary of the user's Superforecasting Agent setup
...
from superforecasting_agent.runtime.config import get_agent_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``superforecasting-agent dump`` formats empty values as blank, not as
    ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from superforecasting_agent.runtime.gateway import get_gateway_runtime_snapshot
...
def run_dump(args):
    ...
    lines = []
    lines.append("--- superforecasting-agent dump ---")
    ...
    lines.append(f"runtime_home:     {display_agent_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["forecast-desk"])
