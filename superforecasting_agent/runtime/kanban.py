"""CLI for the Superforecasting Agent Kanban board.

Exposes the full Kanban command surface documented in the design spec
(``docs/plans/kanban-v1-spec.pdf``).  All DB work is delegated to
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    Used by ``superforecasting-agent kanban create`` (and callers) to warn when a task
    ...
    if pid and not dispatch_on:
        return (
            ...
            f"standalone daemon (`{_PRIMARY_CLI} kanban daemon --force`)."
        )
    return (
        False,
        "No gateway is running — the task will sit in 'ready' until you "
        "start it. Run:\n"
        f"    {_PRIMARY_CLI} gateway start\n"
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            f"Durable SQLite-backed task board shared across {_PRODUCT_NAME} profiles. "
            "Tasks are claimed atomically, can depend on other tasks, and "
            "are executed by a named profile in an isolated workspace. "
            "See docs/plans/kanban-v1-spec.pdf for the full design."
        ),
    )
    # --- global --board flag ---
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help=f"DEPRECATED - dispatcher now runs in the gateway. Use `{_PRIMARY_CLI} gateway start`.",
    )
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of runtime profiles/ and current assignees on the board)",
    )
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from kanban argparse dispatch.
...
def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle kanban board-management actions.
...
def _board_task_counts(slug: str) -> dict[str, int]:
    ...
    try:
        ...
        if not path.exists():
            return {}
        with kb.connection(board=slug) as conn:
            rows = conn.execute(
...
def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `superforecasting-agent kanban boards delete <slug>` (alias), the
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    # through to `superforecasting-agent -p <name>`.
    try:
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        _emit(
            ...
            "\n"
            f"    {_PRIMARY_CLI} gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                _emit(
                    ...
                    f"`{_PRIMARY_CLI} kanban list --status ready` / "
                    f"`{_PRIMARY_CLI} kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real Superforecasting Agent profile (i.e. one the
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from superforecasting_agent.runtime import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from superforecasting_agent.runtime import kanban_decompose as decomp
