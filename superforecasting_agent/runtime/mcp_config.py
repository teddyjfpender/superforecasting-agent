...
from superforecasting_agent.runtime.config import (
    ...
    get_env_value,
    save_env_value,
    get_agent_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from superforecasting_agent.runtime.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    # superforecasting_agent/runtime/main.py for why the dest is renamed.
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_agent_home()}/.env as {env_key}")
    ...
    print()
    _success(f"Saved '{name}' to {display_agent_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def mcp_command(args):
    ...
    if action == "serve":
        from superforecasting_agent.mcp.server import run_mcp_server
