"""`superforecasting-agent checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store. Actions:

    superforecasting-agent checkpoints               # same as `status`
    superforecasting-agent checkpoints status        # total size, project count, breakdown
    superforecasting-agent checkpoints list          # per-project checkpoint counts + workdir
    superforecasting-agent checkpoints prune [opts]  # force a sweep
    superforecasting-agent checkpoints clear [-f]    # nuke the entire base
    superforecasting-agent checkpoints clear-legacy  # delete legacy-* archives

Examples::

    superforecasting-agent checkpoints
    superforecasting-agent checkpoints prune --retention-days 3 --max-size-mb 200
    superforecasting-agent checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print(f"Clear with: {_PRIMARY_CLI} checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``superforecasting-agent checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare checkpoints → status
