"""``superforecasting-agent logs`` — view and filter Superforecasting Agent log files.

Supports tailing, following, session filtering, level filtering,
component filtering, and relative time ranges.  All log files live
under the active agent home's ``logs/`` directory.

Usage examples::

    superforecasting-agent logs                    # last 50 lines of agent.log
    superforecasting-agent logs -f                 # follow agent.log in real time
    superforecasting-agent logs errors             # last 50 lines of errors.log
    superforecasting-agent logs gateway -n 100     # last 100 lines of gateway.log
    superforecasting-agent logs --level WARNING    # only WARNING+ lines
    superforecasting-agent logs --session abc123   # filter by session ID substring
    superforecasting-agent logs --component tools  # only tool-related lines
    superforecasting-agent logs --since 1h         # lines from the last hour
    superforecasting-agent logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_agent_home() / "logs" / filename
    ...
    if follow:
        print(f"--- {display_agent_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_agent_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_agent_home()}/logs/")
        return

    print(f"Log files in {display_agent_home()}/logs/:\n")
    found = False
