"""
Doctor command for the Superforecasting Agent CLI.

Diagnoses issues with Superforecasting Agent setup.
"""
...
from superforecasting_agent.runtime.config import get_project_root, get_agent_home, get_env_path
...
_DHH = display_agent_home()  # active user-facing agent home path
...
from superforecasting_agent.constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when the active agent-home .env contains provider settings."""
...
def _check_build_version(issues: list[str]) -> None:
    ...
    try:
        from superforecasting_agent.runtime.banner import get_update_state, prefetch_update_check
    except Exception as e:
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from superforecasting_agent.runtime.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as the CLI.
    from superforecasting_agent.environment import INTERACTIVE_ENV_NAMES

    for name in INTERACTIVE_ENV_NAMES:
        os.environ.setdefault(name, "1")

    # Handle `superforecasting-agent doctor --ack <id>` as a fast path. Persist
    # the ack and return without running the rest of the diagnostics — the user has
    ...
    print(color("│          🩺 Superforecasting Agent Doctor               │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`superforecasting-agent doctor --ack {hit.advisory.id}`."
                )
    ...
    # Check the active agent-home .env (primary location for user config)
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append(f"Run '{_PRIMARY_CLI} setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info(f"Run '{_PRIMARY_CLI} setup' to configure API keys")
                fixed_count += 1
            else:
                check_info(f"Run '{_PRIMARY_CLI} setup' to create one")
                issues.append(f"Run '{_PRIMARY_CLI} setup' to create .env")
    
    # Check the active agent-home config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from superforecasting_agent.runtime.config import get_compatible_custom_providers as _compatible_custom_providers
                from superforecasting_agent.runtime.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run '{_PRIMARY_CLI} config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
                ...
                try:
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            f"(check {_DHH}/.env or run '{_PRIMARY_CLI} setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run '{_PRIMARY_CLI} setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with '{_PRIMARY_CLI} config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from superforecasting_agent.runtime.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from superforecasting_agent.runtime.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append(f"Run '{_PRIMARY_CLI} setup' to migrate config")
                else:
                    issues.append(
                        f"Run '{_PRIMARY_CLI} doctor --fix' or '{_PRIMARY_CLI} setup' to migrate config"
                    )
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append(
                        f"Stale root-level provider/base_url in config.yaml — run '{_PRIMARY_CLI} doctor --fix'"
                    )
        except Exception:
            pass
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append(f"Large WAL file — run '{_PRIMARY_CLI} doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / _PRIMARY_CLI
        ...
        _cmd_link = _cmd_link_dir / _PRIMARY_CLI

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                f"({_PRIMARY_CLI} not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/{_PRIMARY_CLI} → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/{_PRIMARY_CLI} points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/{_PRIMARY_CLI} → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(
                            f"Broken symlink at {_cmd_link_display}/{_PRIMARY_CLI} — "
                            f"run '{_PRIMARY_CLI} doctor --fix'"
                        )
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/{_PRIMARY_CLI} exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/{_PRIMARY_CLI} not found",
                    f"({_PRIMARY_CLI} command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/{_PRIMARY_CLI} → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(
                        f"Missing {_cmd_link_display}/{_PRIMARY_CLI} symlink — run '{_PRIMARY_CLI} doctor --fix'"
                    )
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "Install the Vercel optional dependency: pip install 'superforecasting-agent[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `superforecasting-agent doctor` invocation.
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     f"Fix: run '{_PRIMARY_CLI} config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``superforecasting-agent doctor``.
    ...
    # never the legitimate source for `superforecasting-agent doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append(f"Run '{_PRIMARY_CLI} setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", f"(run: {_PRIMARY_CLI} skills list)")
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", f"run: {_PRIMARY_CLI} memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    f"run: {_PRIMARY_CLI} memory setup",
                    f"No Honcho API key — run '{_PRIMARY_CLI} memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    f"(set MEM0_API_KEY in .env or run {_PRIMARY_CLI} memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(
                    ...
                    f"run: {_PRIMARY_CLI} memory status",
                )
            else:
                check_warn(f"{_active_memory_provider} plugin not found", f"run: {_PRIMARY_CLI} memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from superforecasting_agent.runtime.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color(f"  Tip: run '{_PRIMARY_CLI} doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
