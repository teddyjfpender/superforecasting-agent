...
def _pending_file() -> Path:
    ...
    """
    return get_agent_home() / "pastes" / "pending.json"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from superforecasting_agent.runtime.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_agent_home() / "logs" / filename) if filename else None
