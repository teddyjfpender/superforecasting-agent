"""``superforecasting-agent slack ...`` CLI subcommands.

Today only ``superforecasting-agent slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ superforecasting-agent slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    it stays in sync with the rest of Superforecasting Agent. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Superforecasting Agent deployment — users can tweak them in the Slack UI after
    pasting.
    """
    from superforecasting_agent.runtime.commands import slack_app_manifest
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your forecasting desk on Slack")[:140],
        ...
        "features": {
            ...
            "assistant_view": {
                "assistant_description": "Review forecasts and alerts with Superforecasting Agent.",
            },
        },
        "oauth_config": {
            "scopes": {
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``superforecasting_agent/runtime/main.py``):
    ...
    """
    name = getattr(args, "name", None) or "Superforecast"
    description = getattr(args, "description", None) or "Your forecasting desk on Slack"
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Superforecasting Agent app\n"
