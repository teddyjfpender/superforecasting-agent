"""``superforecasting-agent plugins`` CLI subcommand.

Plugins are installed from Git repositories into the runtime plugins directory.
...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other runtime subprocess resolution: :func:`shutil.which` first,
...
def _plugins_dir() -> Path:
    ...
    plugins = get_agent_home() / "plugins"
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from superforecasting_agent.runtime.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_agent_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_agent_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_agent_home()}/.env later)[/dim]")
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update Superforecasting Agent.",
                ) from None
        ...
        if target.exists():
            if not force:
                raise PluginOperationError(
                    ...
                    f"or run `{_PRIMARY_CLI} plugins update {plugin_name}`.",
                )
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (
        ...
        console.print(
            ...
            f"or __init__.py. It may not be a valid Superforecasting Agent plugin.",
        )
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `superforecasting-agent plugins enable {installed_name}` to activate.[/dim]",
        )

    console.print("[dim]Restart the gateway for the plugin to take effect:[/dim]")
    console.print("[dim]  superforecasting-agent gateway restart[/dim]")
    console.print()
    ...
    if user_dir.is_dir():
        ...
        for child in user_dir.iterdir():
            ...
            if manifest.get("name") == name:
                return True
    # Bundled: <repo>/plugins/<name>/ (or bundled-plugin env aliases on Nix).
    from superforecasting_agent.runtime.plugins import get_bundled_plugins_dir
...
def cmd_list() -> None:
    ...
    if not entries:
        ...
        console.print(f"[dim]Install with:[/dim] {_PRIMARY_CLI} plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print(f"[dim]Interactive toggle:[/dim] {_PRIMARY_CLI} plugins")
    console.print(f"[dim]Enable/disable:[/dim] {_PRIMARY_CLI} plugins enable/disable <name>")
