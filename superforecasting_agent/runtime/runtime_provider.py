...
from superforecasting_agent.runtime.nous_env import nous_min_key_ttl_seconds, nous_timeout_seconds
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from superforecasting_agent.runtime.models import copilot_model_api_mode
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from superforecasting_agent.runtime.models import azure_foundry_model_api_mode
        ...
        if provider in {"opencode-zen", "opencode-go"}:
            ...
            # Refs #16878.
            from superforecasting_agent.runtime.models import opencode_model_api_mode
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if not requested_norm.startswith("custom:"):
        try:
            ...
            # the built-in. See tests/runtime_cli/test_runtime_provider_resolution.py
...
def _resolve_openrouter_runtime(
    *,
    ...
    if _is_openrouter_url:
        ...
        # Gate each provider key on its own authoritative host — sending
        # OPENAI_API_KEY or OPENROUTER_API_KEY to an unrelated custom endpoint
        # (DeepSeek, Groq, Mistral, …) leaks credentials and causes 401s
        # (#28660). Mirrors the OLLAMA_API_KEY host-gate (GHSA-76xc-57q6-vm5m).
        # Unrecognized custom endpoints fall through to "no-key-required".
        api_key_candidates = [
            explicit_api_key,
    ...
    *,
    ...
    if not explicit_base_url and not explicit_api_key:
        ...
        if _bypass_cfg_base_url and _bypass_cfg_provider in ("auto", ""):
            ...
            if not any(
                base_url_host_matches(_bypass_cfg_base_url, host)
