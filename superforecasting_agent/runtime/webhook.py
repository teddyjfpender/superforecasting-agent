"""superforecasting-agent webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to the agent home's webhook_subscriptions.json and are
...
def _setup_hint() -> str:
  ...
  Then start the gateway: superforecasting-agent gateway run
"""
...
def webhook_command(args):
    """Entry point for 'superforecasting-agent webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: superforecasting-agent webhook {subscribe|list|remove|test}")
        print("Run 'superforecasting-agent webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (superforecasting-agent gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: superforecasting-agent webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from superforecasting-agent webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (superforecasting-agent gateway run)")
