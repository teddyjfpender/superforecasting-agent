    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                ...
                if (
                    'superforecasting_agent.runtime' in content
...
def uninstall_gateway_service():
    ...
    try:
        from superforecasting_agent.runtime.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from superforecasting_agent.runtime.gateway import get_launchd_plist_path
...
def _is_default_hermes_home(hermes_home: Path) -> bool:
    ...
    try:
        ...
        return hermes_home.resolve() == get_default_agent_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    #    Use `python -m superforecasting_agent.runtime.main` so we don't depend on a `hermes`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "superforecasting_agent.runtime.main", "--profile", name]
