...
Usage:
    python -m superforecasting_agent.runtime.main dashboard          # Start on http://127.0.0.1:9119
    python -m superforecasting_agent.runtime.main dashboard --port 8080
"""
...
async def get_status():
    ...
    runtime_home = str(get_agent_home())
...
_ACTION_LOG_DIR: Path = get_agent_home() / "logs"
...
def _spawn_forecast_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    Uses the running interpreter's ``superforecasting_agent.runtime.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "superforecasting_agent.runtime.main", *subcommand]
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    The inherited runtime accepts ``model`` as either a bare string
    (``"anthropic/claude-sonnet-4"``) or a dict
    (``{default: ..., provider: ..., base_url: ...}``). The schema is built from
    DEFAULT_CONFIG where ``model`` is a string, but user configs often have the
...
# in superforecasting_agent/runtime/config.py — listed here for deterministic ordering in the UI.
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from superforecasting_agent.runtime import auth as hauth
        if provider_id == "nous":
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    # auth writers (superforecasting_agent/runtime/auth.py). A bare write_text() relied on the
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "superforecasting-agent-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        # Persist via the canonical Codex save path: writes the
        # providers.openai-codex singleton (setting active_provider) and syncs
        # the credential pool. The previous hand-rolled pool.add_entry() wrote
...
async def get_logs(
    ...
    from superforecasting_agent.runtime.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_agent_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from superforecasting_agent.logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
async def get_skills():
    ...
    from superforecasting_agent.runtime.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from superforecasting_agent.runtime.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
try:
    from superforecasting_agent.runtime.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    """
    from superforecasting_agent.runtime.main import PROJECT_ROOT, _make_tui_argv
...
def _discover_user_themes() -> list:
    ...
    """
    themes_dir = get_agent_home() / "dashboard-themes"
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as superforecasting_agent.runtime.plugins):
    1. User plugins:    ~/.superforecasting-agent/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.hermes/plugins/  (only when project-plugin env aliases opt in)
    """
    plugins = []
    seen_names: set = set()

    from superforecasting_agent.runtime.plugins import get_bundled_plugins_dir, project_plugins_enabled
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_agent_home() / "plugins", "user"),
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_agent_home() / "plugins").resolve()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from superforecasting_agent.runtime.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from superforecasting_agent.runtime.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from superforecasting_agent.runtime.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from superforecasting_agent.runtime.plugins_cmd import dashboard_remove_user_plugin
