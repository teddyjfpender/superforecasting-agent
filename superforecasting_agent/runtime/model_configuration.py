...
def persist_model_selection(
    ...
    *,
    ...
    with _CONFIG_LOCK, yaml_update_lock(path):
        return _persist_model_selection(
            path, model=model, provider=provider, base_url=base_url, api_mode=api_mode
        )
...
def _persist_model_selection(
    ...
    *,
    ...
    def update(cfg: dict[str, Any]) -> None:
        ...
        if current.get("provider") != selected["provider"] or (
            current.get("base_url") or ""
        ).rstrip("/") != selected["base_url"].rstrip("/"):
