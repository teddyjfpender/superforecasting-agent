...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m superforecasting_agent.runtime.main gateway run --replace`` invocation) and
...
_TASK_NAME_DEFAULT = "Superforecasting_Agent_Gateway"
_LEGACY_TASK_NAME_DEFAULT = "Hermes_Gateway"
_TASK_DESCRIPTION = "Superforecasting Agent Gateway - Messaging Platform Integration"
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current agent profile."""
    from superforecasting_agent.runtime.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "superforecasting_agent.runtime.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
        args.extend(extra_args)
    params = subprocess.list2cmdline(args)
    cwd = str(get_install_root())
...
def get_task_name() -> str:
    ...
    Default profile: ``Superforecasting_Agent_Gateway``
    Named profile X: ``Superforecasting_Agent_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during superforecasting_agent.runtime boot.
    from superforecasting_agent.runtime.gateway import _profile_suffix
...
def _task_script_path_for(task_name: str) -> Path:
    ...
    script_dir = Path(get_agent_home()) / "gateway-service"
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - invokes ``pythonw -m superforecasting_agent.runtime.main [--profile X] gateway run``
    ...
    # if someone imports superforecasting_agent.constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    pythonw_path = _derive_venv_pythonw(python_path)
    prog_args = [pythonw_path, "-m", "superforecasting_agent.runtime.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    ...
    hermes_home = str(Path(get_agent_home()).resolve())
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older inherited builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "superforecasting_agent.runtime.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m superforecasting_agent.runtime.main gateway run``
    ...
    log_dir = Path(get_agent_home()) / "logs"
...
def _prompt_install_choices(
    ...
    from superforecasting_agent.runtime.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `superforecasting-agent -p <profile> gateway install` must be
    # safe.
    ...
    from superforecasting_agent.runtime.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = (
            f"{_PRIMARY_CLI} {profile_arg} gateway start"
            if profile_arg
            else f"{_PRIMARY_CLI} gateway start"
        )
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print(f"  Run later with: {_PRIMARY_CLI} gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Superforecasting Agent gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print(f"  Approve the Windows UAC prompt, then run: {_PRIMARY_CLI} gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print(f"  Start manually with: {_PRIMARY_CLI} gateway start")
        _print_next_steps()
        return
    ...
    if _should_fall_back(1, detail):
        ...
        # Re-running `superforecasting-agent -p <profile> gateway install` must
        # be safe.
        # Startup-folder fallback only installs login persistence. Starting is
        # controlled by the pre-UAC start_now answer so all user decisions happen
        # before any elevation prompt.
...
def _wait_for_gateway_ready(timeout_s: float = 6.0, interval_s: float = 0.4) -> list[int]:
    ...
    """
    from superforecasting_agent.runtime.gateway import find_gateway_pids
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from superforecasting_agent.runtime.config import get_agent_home
        print(f"    type {Path(get_agent_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_agent_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from superforecasting_agent.runtime.config import get_agent_home

    hermes_home = Path(get_agent_home()).resolve()
    print()
    print("Next steps:")
    print(f"  {_PRIMARY_CLI} gateway status              # Check status")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Superforecasting Agent gateway uninstall prompt.")
                    print(f"  Approve the Windows UAC prompt, then run: {_PRIMARY_CLI} gateway status")
                    return
...
def status(deep: bool = False) -> None:
    ...
    if not task_installed and not startup_installed and not pids:
        print()
        print("To install:")
        print(f"  {_PRIMARY_CLI} gateway install")
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print(f"  Run: {_PRIMARY_CLI} gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print(f"  If a UAC prompt opened, approve it, then run: {_PRIMARY_CLI} gateway start")
            return
...
def stop() -> None:
    ...
    _assert_windows()
    from superforecasting_agent.runtime.gateway import kill_gateway_processes
