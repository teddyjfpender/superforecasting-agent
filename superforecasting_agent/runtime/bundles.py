"""Implementation of the ``superforecasting-agent bundles`` CLI subcommand.

Mirrors the structure of ``superforecasting_agent/runtime/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `superforecasting-agent bundles list | grep ...`
    # doesn't
...
def register_cli(subparser) -> None:
    """Build the ``superforecasting-agent bundles`` argparse tree.

    Called from ``superforecasting_agent/runtime/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help=(
            "Human-readable description shown in /help and "
...
def bundles_command(args) -> None:
    """Dispatch ``superforecasting-agent bundles <subcommand>``."""
