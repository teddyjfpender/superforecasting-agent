"""
Unified self-relaunch for Superforecasting Agent CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``superforecasting-agent sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``superforecasting-agent`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
from superforecasting_agent.runtime._parser import (
...
def resolve_cli_bin() -> Optional[str]:
    ...
    Priority:
      ...
      4. ``None`` → caller should fall back to ``python -m superforecasting_agent.runtime.main``.
    ...
    ``sys.executable -m superforecasting_agent.runtime.main`` fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    if bin_path:
        ...
        argv = [sys.executable, "-m", "superforecasting_agent.runtime.main"]
...
def relaunch(
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    ``python -m superforecasting_agent.runtime.main`` or a ``.cmd`` batch file, and both raise
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            print(
                ...
                f"Fix: open a new terminal so PATH picks up, then re-run superforecasting-agent.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
