"""
Status command for the Superforecasting Agent CLI.

Shows the status of all Superforecasting Agent components.
"""
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``superforecasting-agent config``'s
...
def show_status(args):
    """Show status of all Superforecasting Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│          ⚕ Superforecasting Agent Status               │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: superforecasting-agent auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: superforecasting-agent model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: superforecasting-agent auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: superforecasting-agent auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: superforecasting-agent model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from superforecasting_agent.runtime.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'superforecasting-agent[vercel]')"
    ...
    try:
        from superforecasting_agent.runtime.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   superforecasting-agent gateway")
    ...
    jobs_file = get_agent_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_agent_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'superforecasting-agent doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'superforecasting-agent setup' to configure", Colors.DIM))
    print()
