...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Superforecasting Agent config.")
...
def _translate_one_server(
    ...
    for key in hermes_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            ...
            skipped.append(f"{key} (unknown agent MCP key)")
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Superforecasting Agent)")
...
def _build_hermes_tools_mcp_entry() -> dict:
    ...
    # (not get_agent_home()) on purpose: when the env var is unset we want
    ...
    out: dict[str, Any] = {
        ...
        "args": ["-m", "agent.transports.forecast_tools_mcp_server"],
    }
    if env:
        out["env"] = env
    ...
    *,
    ...
    if not isinstance(hermes_servers, dict):
        report.errors.append(
            "mcp_servers in Superforecasting Agent config is not a dict; cannot migrate."
        )
        return report
    ...
    # The server itself is agent/transports/forecast_tools_mcp_server.py
