"""
Unified tool configuration for Superforecasting Agent.

`superforecasting-agent tools` and `superforecasting-agent setup tools` enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to the runtime config under the
...
from superforecasting_agent.runtime.cli_output import (  # noqa: E402 — late import block
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Superforecasting Agent routes X searches through xAI's built-in x_search "
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``superforecasting-agent
      update`` and by ``superforecasting-agent computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `superforecasting-agent update` calls this
            # for every user; only macOS users with cua-driver care.
            return False
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which("cua-driver"):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / Superforecasting Agent process.")
            return True
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            _print_warning("    Node.js not found - browser tools require npm install in the Superforecasting Agent checkout")
            return
        ...
        if not camofox_dir.exists() and _npm_bin:
            ...
            # package.json so that `superforecasting-agent update` does not
            # silently pull the ~300MB Camoufox Firefox-fork binary for every
            # user.
        ...
        try:
            ...
            _print_info(f"    Run later: {_PRIMARY_CLI} auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                f"Skip — configure later via `{_PRIMARY_CLI} auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    f"Run later: {_PRIMARY_CLI} auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    f"    No API key provided. Run later: {_PRIMARY_CLI} auth add xai-oauth"
                )
        else:
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    from superforecasting_agent.tooling.selection import set_platform_tools

    set_platform_tools(config, platform, enabled_toolset_keys)
    save_config(config)
...
def _estimate_tool_tokens() -> Dict[str, int]:
    ...
    try:
        ...
        from superforecasting_agent.tooling import runtime as model_tools  # noqa: F401
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("Superforecasting Agent Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    print(color(f"  Tool configuration saved to {display_agent_home()}/config.yaml", Colors.DIM))
    print(color(f"  Changes take effect on next '{_PRIMARY_CLI}' or gateway restart.", Colors.DIM))
    print()
...
def _apply_toolset_change(config: dict, platform: str, toolset_names: List[str], action: str):
    ...
    apply_toolset_change(config, platform, toolset_names, action)
    save_config(config)


from superforecasting_agent.tooling.selection import apply_mcp_change as _apply_mcp_change
...
def _print_tools_list(enabled_toolsets: set, mcp_servers: dict, platform: str = "cli"):
    ...
    print(describe_tool_configuration(enabled_toolsets, mcp_servers, platform=platform))
