#!/usr/bin/env python3
"""
Superforecasting Agent CLI - Main entry point.
...
Usage:
    ...
    superforecasting-agent gateway status      # Show gateway status
    superforecasting-agent gateway install     # Install gateway service
    superforecasting-agent gateway uninstall   # Uninstall gateway service
    ...
    superforecasting-agent cron list           # List cron jobs
    superforecasting-agent cron status         # Check if cron scheduler is running
    superforecasting-agent doctor              # Check configuration and dependencies
    superforecasting-agent honcho setup        # Configure Honcho AI memory integration
    superforecasting-agent honcho status       # Show Honcho config and connection status
    superforecasting-agent honcho sessions     # List directory → session name mappings
    superforecasting-agent honcho map <name>   # Map current directory to a session name
    superforecasting-agent honcho peer         # Show peer names and dialectic settings
    superforecasting-agent honcho peer --user NAME         # Set user peer name
    superforecasting-agent honcho peer --ai NAME           # Set AI peer name
    superforecasting-agent honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    superforecasting-agent honcho mode                     # Show current memory mode
    superforecasting-agent honcho mode [hybrid|honcho|local]  # Set memory mode
    superforecasting-agent honcho tokens                   # Show token budget settings
    superforecasting-agent honcho tokens --context N       # Set session.context() token cap
    superforecasting-agent honcho tokens --dialectic N     # Set dialectic result char cap
    superforecasting-agent honcho identity                 # Show AI peer identity representation
    superforecasting-agent honcho identity <file>          # Seed AI peer identity from a file
    superforecasting-agent honcho migrate                  # OpenClaw native → Superforecasting Agent + Honcho
    superforecasting-agent version             Show version
...
# IMPORTANT: superforecasting_agent.bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Keep the recovery command usable if an interrupted update leaves the
# package unavailable. Windows UTF-8 setup is skipped in that degraded state.
try:
...
_DASHBOARD_TUI_ENV_VARS = (
    "SUPERFORECASTING_AGENT_DASHBOARD_TUI",
    "FORECAST_DASHBOARD_TUI",
...
def _env_flag_enabled(names: tuple[str, ...]) -> bool:
    value = _first_env_value(names)
    return value is not None and value.lower() in {"1", "true", "yes", "on"}


def _configured_dashboard_web_dist() -> str | None:
    return _first_env_value(_DASHBOARD_WEB_DIST_ENV_VARS)


def _dashboard_tui_env_enabled() -> bool:
    return _env_flag_enabled(_DASHBOARD_TUI_ENV_VARS)
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (tools, setup, model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'superforecasting-agent {command_name}' requires an interactive terminal.\n"
...
# Falls back to the canonical agent root's active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    ...
    # Mirrors superforecasting_agent.runtime.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If an explicit home env var is already set and no explicit flag was
    # given, bridge forecast-native aliases into HERMES_HOME for legacy modules
    # that still read the old variable directly. Trust it only when it already
    ...
    if configured_home_env:
        os.environ["HERMES_HOME"] = configured_home_env
    ...
    if profile_name is None:
        try:
            ...
            active_path = get_default_agent_root() / "active_profile"
...
# BEFORE superforecasting_agent.logging imports agent.redact (which snapshots the flag at
...
try:
    if not any(name in os.environ for name in _REDACT_SECRETS_ENV_NAMES):
        ...
        _cfg_path = get_agent_home() / "config.yaml"
        if _cfg_path.exists():
...
try:
    from superforecasting_agent.logging import setup_logging as _setup_logging
...
try:
    from superforecasting_agent.runtime.config import load_config as _load_config_early
    from superforecasting_agent.constants import apply_ipv4_preference as _apply_ipv4
...
from superforecasting_agent.constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL

logger = logging.getLogger(__name__)


from superforecasting_agent.runtime.session_browser import _relative_time as _relative_time
...
def _has_any_provider_configured() -> bool:
    ...
    auth_file = get_agent_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
from superforecasting_agent.runtime.session_browser import _session_browse_picker as _session_browse_picker
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo superforecasting-agent {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo superforecasting-agent {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  superforecasting-agent --tui --resume {target}")
    if title:
        print(f'  superforecasting-agent --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @superforecasting/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@superforecasting" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
def _ensure_tui_node() -> None:
    ...
    ``SUPERFORECASTING_AGENT_SKIP_NODE_BOOTSTRAP=1``,
    ``FORECAST_SKIP_NODE_BOOTSTRAP=1``, or the legacy
    ``HERMES_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if _runtime_env_value("SKIP_NODE_BOOTSTRAP"):
        return
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = _runtime_env_value("NODE")
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if _runtime_env_value("QUIET") is None:
    ...
    if tui_dev:
        # Keep the local @superforecasting/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @superforecasting/ink resolves through
        # packages/forecast-ink/dist/entry-exports.js. If that dist bundle is
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from superforecasting_agent.runtime.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    try:
        from superforecasting_agent.runtime.config import load_config as _load_cfg_for_theme
    ...
    configure_tui_runtime(env, PROJECT_ROOT, dev=tui_dev)
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `superforecasting-agent update` so
...
def _pin_kanban_board_env() -> None:
    ...
    """
    try:
        from superforecasting_agent.runtime.kanban_db import KANBAN_BOARD_ENV_NAMES, get_current_board
...
def cmd_chat(args):
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Superforecasting Agent isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  superforecasting-agent setup")
        print()
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'superforecasting-agent setup' at any time to configure.")
        sys.exit(1)
    ...
    # user's ~/.superforecasting-agent/config.yaml and return built-in defaults. Set BEFORE
...
def cmd_gateway(args):
    ...
    from superforecasting_agent.runtime.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from superforecasting_agent.runtime.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Superforecasting Agent?")
        print()
    ...
    session_dir = get_agent_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: superforecasting-agent gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and the gateway command skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  superforecasting-agent gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with 'Superforecasting Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  superforecasting-agent gateway")
        ...
        print()
        print("  Or install as a service: superforecasting-agent gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'superforecasting-agent whatsapp' to try again.")
...
def cmd_postinstall(args):
    ...
    from superforecasting_agent.runtime.config import stamp_install_method
    from superforecasting_agent.runtime.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Superforecasting Agent post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    from superforecasting_agent.runtime.providers import resolve_provider_full
    ...
    from superforecasting_agent.runtime.custom_provider_catalog import named_custom_provider_map

    def _norm_base_url(url: str) -> str:
        return str(url or "").strip().rstrip("/").lower()

    # Add user-defined custom providers from config.yaml
    _custom_provider_map = named_custom_provider_map(
        config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check "
                "'superforecasting-agent model' for available providers, or run "
                "'superforecasting-agent doctor' to diagnose config issues."
            )
    ...
    if selected_provider == "openrouter":
        ...
        provider_info = named_custom_provider_map(load_config()).get(selected_provider)
...
def _clear_stale_openai_base_url():
    ...
    """
    from superforecasting_agent.runtime.config import get_env_value, save_env_value, load_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Superforecasting Agent only falls back to a lightweight backend (OpenRouter,")
...
def _aux_flow_provider_model(
    ...
    from superforecasting_agent.runtime.auth import _prompt_model_selection
    from superforecasting_agent.runtime.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from superforecasting_agent.runtime.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from superforecasting_agent.runtime.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from superforecasting_agent.runtime.models import ai_gateway_model_ids, get_pricing_for_provider
...
from superforecasting_agent.runtime.oauth_setup import _model_flow_nous as _model_flow_nous


from superforecasting_agent.runtime.oauth_setup import _model_flow_openai_codex as _model_flow_openai_codex


from superforecasting_agent.runtime.oauth_setup import _model_flow_xai_oauth as _model_flow_xai_oauth
...
def _model_flow_custom(config):
    ...
    from superforecasting_agent.runtime.models import probe_api_models
    ...
    if probe.get("used_fallback") and probe.get("resolved_base_url"):
        ...
        print(
            ...
            f"Superforecasting Agent will still save it."
        )
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `superforecasting-agent model` to set a model.")
...
from superforecasting_agent.runtime.custom_provider_setup import _prompt_custom_api_mode_selection as _prompt_custom_api_mode_selection


from superforecasting_agent.runtime.custom_provider_setup import _auto_provider_name as _auto_provider_name


from superforecasting_agent.runtime.custom_provider_setup import _custom_provider_api_key_config_value as _custom_provider_api_key_config_value


from superforecasting_agent.runtime.custom_provider_setup import _custom_provider_base_url_config_value as _custom_provider_base_url_config_value


from superforecasting_agent.runtime.custom_provider_setup import _save_custom_provider as _save_custom_provider


from superforecasting_agent.runtime.azure_setup import _model_flow_azure_foundry as _model_flow_azure_foundry
...
def _model_flow_copilot(config, current_model=""):
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from superforecasting_agent.runtime.copilot_auth import copilot_device_code_login
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates forecast-scoped turns to `copilot --acp`.")
    print("  Superforecasting Agent currently starts its own ACP subprocess for each request.")
    print("  Superforecasting Agent uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set SUPERFORECASTING_AGENT_COPILOT_ACP_COMMAND, "
            "FORECAST_COPILOT_ACP_COMMAND, or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
from superforecasting_agent.runtime.api_key_setup import _prompt_api_key as _prompt_api_key
...
from superforecasting_agent.runtime.bedrock_setup import _model_flow_bedrock_api_key as _model_flow_bedrock_api_key


from superforecasting_agent.runtime.bedrock_setup import _model_flow_bedrock as _model_flow_bedrock


def _model_flow_api_key_provider(config, provider_id, current_model=""):
    """Generic flow for API-key providers (z.ai, MiniMax, OpenCode, etc.)."""
    from superforecasting_agent.runtime import api_key_setup
    return api_key_setup._model_flow_api_key_provider(
        config, provider_id, current_model,
        _PROVIDER_MODELS=_PROVIDER_MODELS, _prompt_api_key=_prompt_api_key,
    )


from superforecasting_agent.runtime.anthropic_setup import _run_anthropic_oauth_flow as _run_anthropic_oauth_flow


from superforecasting_agent.runtime.anthropic_setup import _model_flow_anthropic as _model_flow_anthropic


def cmd_login(args):
    """Authenticate Superforecasting Agent CLI with a provider."""
    from superforecasting_agent.runtime.auth import login_command
...
def cmd_logout(args):
    ...
    from superforecasting_agent.runtime.auth import logout_command
...
def cmd_auth(args):
    ...
    from superforecasting_agent.runtime.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from superforecasting_agent.runtime.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from superforecasting_agent.runtime.cron import cron_command
...
def cmd_webhook(args):
    ...
    from superforecasting_agent.runtime.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: superforecasting-agent slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `superforecasting-agent slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from superforecasting_agent.runtime.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from superforecasting_agent.runtime.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from superforecasting_agent.runtime.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from superforecasting_agent.runtime.doctor import run_doctor
...
def cmd_security(args):
    ...
    if sub in ("audit", None):
        from superforecasting_agent.runtime.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from superforecasting_agent.runtime.dump import run_dump
...
def cmd_config(args):
    ...
    from superforecasting_agent.runtime.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up the Superforecasting Agent home directory to a zip file."""
    if getattr(args, "quick", False):
        from superforecasting_agent.runtime.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from superforecasting_agent.runtime.backup import run_backup
...
def cmd_import(args):
    ...
    from superforecasting_agent.runtime.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"Superforecasting Agent v{__version__} ({__release_date__})")
...
def cmd_uninstall(args):
    """Uninstall Superforecasting Agent."""
    _require_tty("uninstall")
    from superforecasting_agent.runtime.uninstall import run_uninstall
...
# Critical files that every CLI invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``superforecasting-agent update`` again to roll forward. The post-pull syntax
...
_UPDATE_CRITICAL_FILES = (
    ...
    "superforecasting_agent/runtime/__init__.py",
    "cli.py",
    "superforecasting_agent/runtime/worktree_setup.py",
    "superforecasting_agent/runtime/worktrees.py",
    "superforecasting_agent/runtime/assistant_text.py",
    "superforecasting_agent/runtime/session_maintenance.py",
    "superforecasting_agent/runtime/interactive_config.py",
    "superforecasting_agent/runtime/forecast_search.py",
    "superforecasting_agent/runtime/interactive_routing.py",
    "superforecasting_agent/runtime/console_output.py",
    "superforecasting_agent/runtime/goal_commands.py",
    "superforecasting_agent/runtime/browser_commands.py",
    "superforecasting_agent/runtime/resume_display.py",
    "superforecasting_agent/runtime/forecast_commands.py",
    "superforecasting_agent/runtime/interactive_defaults.py",
    "superforecasting_agent/runtime/openclaw_loader.py",
    "run_agent.py",
    "agent/session_persistence.py",
    "agent/api_errors.py",
    "agent/image_preprocessing.py",
    "agent/status_output.py",
    "agent/session_lifecycle.py",
    "agent/openai_clients.py",
    "agent/credential_recovery.py",
    "superforecasting_agent/tooling/runtime.py",
    "superforecasting_agent/tooling/definitions.py",
    "superforecasting_agent/tooling/dispatch.py",
    "superforecasting_agent/tooling/arguments.py",
    "superforecasting_agent/tooling/errors.py",
    "superforecasting_agent/tooling/async_bridge.py",
    "superforecasting_agent/tooling/toolsets.py",
    "superforecasting_agent/tooling/catalogs/__init__.py",
    "superforecasting_agent/tooling/catalogs/aliases.py",
    "superforecasting_agent/tooling/catalogs/capabilities.py",
    "superforecasting_agent/tooling/catalogs/core.py",
    "superforecasting_agent/tooling/catalogs/forecast.py",
    "superforecasting_agent/tooling/catalogs/legacy.py",
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``superforecasting-agent update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``superforecasting_agent/runtime/web_dist/`` (per vite.config.ts
    outDir: "../superforecasting_agent/runtime/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / 'superforecasting_agent/runtime' / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``superforecasting-agent update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m superforecasting_agent.runtime.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (dashboard will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of dashboard processes other than ourselves.

    ``superforecasting-agent dashboard`` is a long-lived server process commonly
    started and forgotten.  Legacy ``hermes dashboard`` remains a compatibility
    command.  When ``superforecasting-agent update`` replaces files on disk, the running
    ...
    patterns = [
        ...
        "superforecast dashboard",
        "superforecasting_agent dashboard",
        "superforecasting_agent.cli dashboard",
        "hermes dashboard",
        "superforecasting_agent.runtime.main dashboard",
        "superforecasting_agent/runtime/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # with `superforecasting_agent.runtime.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `superforecasting-agent update`.
    ...
    print("  Preview now:  superforecasting-agent curator run --dry-run")
    print("  Pause it:     superforecasting-agent curator pause")
    print("  Docs:         website/docs/user-guide/features/curator.md")
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``superforecasting-agent update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``superforecasting-agent update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: superforecasting-agent curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    Called at the end of ``superforecasting-agent update`` (default ``reason``)
    and also from ``superforecasting-agent dashboard --stop`` or legacy
    ``hermes dashboard --stop`` (which override ``reason``).  The
    ...
    if killed:
        ...
        print("    superforecasting-agent dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Superforecasting Agent by downloading a ZIP archive.
    ...
    try:
        ...
        # GitHub ZIPs extract to superforecasting-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"superforecasting-agent-{branch}")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Superforecasting Agent behaves unexpectedly.")
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Superforecasting Agent couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Superforecasting Agent couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Superforecasting Agent behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `superforecasting-agent update`
# =========================================================================
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_agent_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_agent_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    ``superforecasting-agent update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from superforecasting_agent.constants import get_default_agent_root

    default_home = get_default_agent_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``superforecasting-agent update`` is still progressing even if pip/uv itself is silent.
    """
...
_WINDOWS_ENTRYPOINT_SHIM_NAMES = (
    "forecast.exe",
    "superforecast.exe",
    "superforecasting-agent.exe",
    "hermes.exe",
    "hermes-agent.exe",
    "hermes-gateway.exe",
)
...
def _agent_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [scripts_dir / name for name in _WINDOWS_ENTRYPOINT_SHIM_NAMES]


def _detect_concurrent_agent_instances(
    ...
    A running CLI, dashboard backend, gateway, or legacy Hermes Desktop child
    can hold one of the generated console-script shims while
    ``superforecasting-agent update`` is trying to replace it. The quarantine
    rename then fails with ``[WinError 32]`` and uv inherits the lock.

    This helper enumerates processes whose ``exe`` matches one of the venv's
    shims and returns ``(pid, process_name)`` pairs. The caller's own PID is
    excluded so the running update invocation never reports itself.
    ...
    for shim in _agent_exe_shims(scripts_dir):
        try:
...
def _format_concurrent_instances_message(
    ...
    shim_names = ", ".join(_WINDOWS_ENTRYPOINT_SHIM_NAMES)
    lines = ["✗ Another Superforecasting Agent executable is running:"]
    for pid, name in matches:
        lines.append(f"    PID {pid}  {name}")
    lines.append("")
    lines.append(f"  Updating now may fail to overwrite entry-point shims in {scripts_dir}")
    lines.append("  because Windows blocks REPLACE on a running executable.")
    lines.append(f"  Protected shims: {shim_names}.")
    lines.append("")
    lines.append("  Close any open Superforecasting Agent REPLs and")
    lines.append("  stop the gateway (`superforecasting-agent gateway stop`) before retrying.")
    lines.append("  Override with `superforecasting-agent update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_agent_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file locks on running console-script shims.
    ...
    when ``superforecasting-agent update`` runs, one of these shims IS the
    live process, and uv fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``<name>.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next CLI invocation by ``_cleanup_quarantined_exes``.
    ...
    3. Print a clear warning naming the most likely culprit (running
       Superforecasting Agent desktop, legacy Hermes Desktop, gateway, or REPL)
       and pointing to ``--force``.
    ...
    for shim in _agent_exe_shims(scripts_dir):
        ...
        print(
            "    Close any Superforecasting Agent desktop window, legacy Hermes Desktop, or other `superforecasting-agent` "
            "or `hermes` REPLs, stop the gateway, or pause AV scanning, "
            "then re-run `superforecasting-agent update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_agent_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``*.exe.old.*`` left by prior updates.

    Called early on every CLI invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``superforecasting-agent update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `superforecasting-agent update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live console-script shims in the venv Scripts dir
    before each install attempt so uv can write fresh copies (Windows blocks
    REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_agent_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_agent_exe(scripts_dir)
        try:
...
class _UpdateOutputStream:
    """Stream wrapper for update runs that need to survive terminal loss.
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``superforecasting-agent
    update`` safe to run in a plain SSH session that might disconnect
    mid-install.
    """
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``superforecasting-agent update`` in an SSH session or
    a terminal that may close mid-install. Without protection, ``SIGHUP`` from
    the terminal kills the Python process during ``pip install`` and leaves the
    venv half-installed; the documented workaround ("use screen / tmux")
    shouldn't be required for something as routine as an update.
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``superforecasting-agent update --gateway``) the update is
    already spawned detached from a terminal, so this function is a no-op.
    ...
    try:
        ...
        # superforecasting_agent.runtime.config.get_agent_home to simulate setup failure.
        from superforecasting_agent.runtime.config import get_agent_home as _get_agent_home

        logs_dir = _get_agent_home() / "logs"
        ...
        log_file.write(
            f"\n=== superforecasting-agent update started "
...
def _cmd_update_check():
    ...
    if method in {"pip", "pipx", "release"}:
        from superforecasting_agent.runtime.config import recommended_update_command
        from superforecasting_agent.runtime.banner import check_via_release
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``superforecasting-agent update`` without requiring a reinstall. The
    ...
    path_comment = (
        "# Superforecasting Agent — ensure /usr/local/bin is on PATH "
...
def _run_pre_update_backup(args) -> None:
    ...
    HERMES_HOME directories. The ``--backup`` flag on
    ``superforecasting-agent update`` opts in for a single run; ``--no-backup``
    forces it off when config has it enabled. Never raises — a backup failure
    should not block the update itself.
    """
    ...
    try:
        from superforecasting_agent.runtime.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_agent_home so the user sees ~/.hermes/...
    try:
        from superforecasting_agent.constants import get_agent_home, display_agent_home

        home = get_agent_home()
        try:
            display_path = f"{display_agent_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  superforecasting-agent import {out_path}")
...
def cmd_update(args):
    """Update Superforecasting Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Superforecasting Agent")
        return
...
def _cmd_update_release(args):
    ...
    print("✓ Update complete! Restart superforecasting-agent to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("Updating Superforecasting Agent...")
    print()

    # On Windows, abort early if another generated console-script executable
    # is holding a venv shim open. Continuing would result in a string of
    # WinError 32 warnings and then either a deferred-rename leftover or a
    # failed git-pull fast path that silently falls back to the slower ZIP
    # route. See issue #26670.
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_agent_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        try:
            from superforecasting_agent.runtime.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in superforecasting_agent/runtime/config.py bricked
        # every user who ran ``superforecasting-agent update`` for the 7 minutes between
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print(
                            "  Try ``superforecasting-agent update`` again later once a fix lands."
                        )
                    else:
        ...
        # the old bytecode (e.g. get_agent_home added to superforecasting_agent.constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload superforecasting_agent.constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_agent_home() added since the last release.
        try:
            import importlib
            import superforecasting_agent.constants as _hc
        ...
        try:
            from superforecasting_agent.runtime.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print(
                        "  ℹ API keys require manual entry: "
                        "superforecasting-agent config migrate"
                    )
            else:
                print()
                print(
                    "Skipped. Run 'superforecasting-agent config migrate' later to configure."
                )
        else:
        ...
        # Tying the refresh to ``superforecasting-agent update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from superforecasting_agent.runtime.tools_config import install_cua_driver
        ...
        # When running as ``superforecasting-agent update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_agent_home() / ".update_exit_code"
            try:
        ...
        try:
            from superforecasting_agent.runtime.gateway import (
                is_macos,
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: superforecasting-agent gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: superforecasting-agent -p <profile> gateway run  for each profile)"
                        )
        ...
        # every `superforecasting-agent update` surfaces the issue until the user migrates.
        try:
            from superforecasting_agent.runtime.gateway import (
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                ...
                print()
                print("    superforecasting-agent gateway migrate-legacy")
                print()
        ...
        print("  superforecasting-agent model  # Select provider and model")
...
def _try_launch_top_level_tui_fast_path(argv: list[str]) -> bool:
    ...
    from superforecasting_agent.runtime.config import get_container_exec_info
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `superforecasting-agent profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_agent_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → superforecasting-agent -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                ...
                print(f"Switched to: default ({_get_default_agent_home()})")
            else:
        ...
        from superforecasting_agent.runtime import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from superforecasting_agent.constants import get_agent_home as _hh
        ...
        from superforecasting_agent.runtime import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `superforecasting-agent profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from superforecasting_agent.runtime.profiles import profile_exists
        ...
        from superforecasting_agent.runtime.profiles import rename_profile
        ...
        from superforecasting_agent.runtime.profiles import export_profile
        ...
        from superforecasting_agent.runtime.profiles import import_profile
        ...
        import tempfile
        from superforecasting_agent.runtime.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `superforecasting-agent profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"superforecasting-agent -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from superforecasting_agent.runtime.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     Runtime {data['hermes_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from superforecasting_agent.runtime.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.hermes_requires:
        print(f"  Requires: Runtime {mf.hermes_requires}")
...
def _report_dashboard_status() -> int:
    """Print dashboard PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``superforecasting-agent dashboard --status``
    ...
    print(f"{len(pids)} dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `superforecasting-agent update`.
    ...
    web_dist_override = _configured_dashboard_web_dist()
    if not web_dist_override and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(web_dist_override)
            if web_dist_override
            else PROJECT_ROOT / 'superforecasting_agent/runtime' / "web_dist"
        )
    ...
    from superforecasting_agent.runtime.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from superforecasting_agent.runtime.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Superforecasting Agent log files."""
    from superforecasting_agent.runtime.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from superforecasting_agent.runtime.models import CANONICAL_PROVIDERS as _cp
...
# the top-level flags declared in ``superforecasting_agent/runtime/_parser.py``.
#
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``hermes`` or only flags → defaults to the forecast desk.
        return False
...
def _main():
    ...
    # =========================================================================
    from superforecasting_agent.runtime.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `superforecasting-agent model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `superforecasting-agent proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Superforecasting Agent with an interactive wizard. "
        "Run a specific section: superforecasting-agent setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'superforecasting-agent setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Superforecasting Agent.",
    )
    ...
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Superforecast")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from superforecasting_agent.runtime.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Superforecasting Agent CLI",
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Superforecasting Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under the runtime home scripts/ directory. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="Superforecasting Agent profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under the runtime home scripts/ directory. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="Superforecasting Agent profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    from superforecasting_agent.runtime.kanban import build_parser as _build_kanban_parser
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Superforecasting Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `superforecasting-agent "
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Superforecasting Agent setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description=(
            "Debug utilities for Superforecasting Agent. Use "
            "'superforecasting-agent debug share' to upload a debug report "
            "(system info + recent logs) to a paste service and get a "
            "shareable URL."
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    superforecasting-agent debug share              Upload debug report and print URL
    ...
    superforecasting-agent debug share --local      Print report locally (no upload)
    superforecasting-agent debug share --no-redact  Disable upload-time secret redaction
    superforecasting-agent debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'superforecasting-agent debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up the Superforecasting Agent home directory to a zip file",
        description="Create a zip archive of your entire Superforecasting Agent configuration, "
        "skills, sessions, and data (excludes codebase checkouts). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/superforecasting-agent-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect, prune, or clear runtime checkpoint files",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo used to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from superforecasting_agent.runtime.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Superforecasting Agent backup from a zip file",
        description="Extract a previously created Superforecasting Agent backup into your "
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Superforecasting Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from superforecasting_agent.runtime.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from superforecasting_agent.runtime.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from superforecasting_agent.runtime.skills_hub import skills_command
    ...
    from superforecasting_agent.runtime.bundles import register_cli as _bundles_register, bundles_command
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `superforecasting-agent plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from superforecasting_agent.runtime.plugins_cmd import plugins_command
    ...
    try:
        from superforecasting_agent.runtime.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_agent_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_agent_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_agent_home()}/memories/\n")
        else:
            from superforecasting_agent.runtime.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'superforecasting-agent tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            ...
            _require_tty("tools")
            from superforecasting_agent.runtime.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `superforecasting-agent computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `superforecasting-agent tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print(
                    "  Refresh to latest: "
                    "superforecasting-agent computer-use install --upgrade"
                )
                return
            print("cua-driver: not installed")
            print("  Run: superforecasting-agent computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Superforecasting Agent as an MCP server",
        description=(
            "Manage MCP server connections and run Superforecasting Agent as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'superforecasting-agent mcp add' to connect to a new server, or\n"
            "'superforecasting-agent mcp serve' to expose forecast-scoped conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Superforecasting Agent as an MCP server",
    )
    ...
    def cmd_mcp(args):
        from superforecasting_agent.runtime.mcp_config import mcp_command
    ...
    claw_parser = subparsers.add_parser(
        ...
        description=(
            "Migrate settings, memories, skills, and API keys from OpenClaw "
            "to Superforecasting Agent"
        ),
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Superforecasting Agent",
    ...
    claw_migrate.add_argument(
        ...
        action="store_true",
        help=(
            "Skip the pre-migration zip snapshot of the runtime home (by "
            "default a single restore-point archive is written to backups/ "
            "before apply; restorable with 'superforecasting-agent import')."
        ),
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from superforecasting_agent.runtime.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Superforecasting Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        help=(
            ...
            "API-key entry is skipped; run 'superforecasting-agent config migrate' "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another CLI executable is detected. The concurrent process will likely cause WinError 32 warnings and may leave a reboot-deferred .exe replacement.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Superforecasting Agent",
        description="Remove Superforecasting Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Superforecasting Agent as an ACP (Agent Client Protocol) server",
        description="Start Superforecasting Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Superforecasting Agent ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
    ...
    def cmd_acp(args):
        """Launch Superforecasting Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Superforecasting Agent instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `superforecasting-agent update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `superforecasting-agent profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Superforecasting Agent profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Superforecasting Agent web dashboard for forecast review, config, API keys, and sessions",
    )
    ...
    # for `superforecasting-agent dashboard` and legacy `hermes dashboard`
    # cmdlines and SIGTERM them directly — the same path
    # `superforecasting-agent update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Superforecasting Agent log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    superforecasting-agent logs                    Show last 50 lines of agent.log
    superforecasting-agent logs -f                 Follow agent.log in real time
    superforecasting-agent logs errors             Show last 50 lines of errors.log
    superforecasting-agent logs gateway -n 100     Show last 100 lines of gateway.log
    superforecasting-agent logs --level WARNING    Only show WARNING and above
    superforecasting-agent logs --session abc123   Filter by session ID
    superforecasting-agent logs --component tools  Only show tool-related lines
    superforecasting-agent logs --since 1h         Lines from the last hour
    superforecasting-agent logs --since 30m -f     Follow, starting from 30 min ago
    superforecasting-agent logs list               List available log files with sizes
""",
    )
    ...
    _default_agent_run = args.command is None and (
        bool(getattr(args, "oneshot", None))
        or bool(getattr(args, "resume", None))
        or bool(getattr(args, "continue_last", None))
        or bool(getattr(args, "tui", False))
    )
    if args.command in _AGENT_COMMANDS or _default_agent_run or (
        ...
        try:
            ...
            # so inline is safe.  Moved here from superforecasting_agent/tooling/runtime.py module scope
