...
2. **User plugins**   – ``~/.superforecasting-agent/plugins/<name>/`` or the
   active runtime home returned by ``get_agent_home()``
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to superforecasting_agent/runtime/. Two layouts are
        ...
        user_dir = get_agent_home() / "plugins"
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``superforecasting-agent tools`` TUI so plugin-provided toolsets appear
