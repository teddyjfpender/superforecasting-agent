...
Add, remove, or reorder entries here — both `superforecasting-agent setup` and
`superforecasting-agent model` provider-selection will pick up the change
automatically.
"""
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older runtime that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older runtime that doesn't ship
...
# this list:  superforecasting-agent model, /model, list_authenticated_providers.
#
# Fields:
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    Superforecasting Agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from superforecasting_agent.runtime.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — the forecast runtime
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from superforecasting_agent.runtime.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from superforecasting_agent.constants import AI_GATEWAY_BASE_URL
...
def _resolve_static_model_alias(
    ...
    try:
        from superforecasting_agent.runtime.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether the CLI should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``superforecasting-agent auth add copilot`` and by
         ``_seed_from_env`` when the env var is set in
         ``~/.superforecasting-agent/.env``; legacy ``~/.hermes/.env`` remains
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Superforecasting Agent release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Superforecasting Agent release). Falls back to the hardcoded catalog if no token
...
def _provider_models_cache_path() -> Path:
    ...
    return get_agent_home() / "provider_models_cache.json"
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _AGENT_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: legacy default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _AGENT_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from superforecasting_agent.constants import get_agent_home
    return get_agent_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Superforecasting Agent will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Superforecasting Agent cannot verify the model name."
