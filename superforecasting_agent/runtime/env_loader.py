...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``superforecasting_agent.runtime.config._sanitize_env_lines`` which already
    ...
    if not path.exists():
        return
    try:
        from superforecasting_agent.runtime.config import _sanitize_env_lines
    except ImportError:
