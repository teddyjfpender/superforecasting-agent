"""
Interactive setup wizard for Superforecasting Agent.
...
Config files are stored in the Superforecasting Agent home directory.
"""
...
from superforecasting_agent.constants import display_agent_home, get_optional_skills_dir
...
from superforecasting_agent.runtime.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color(f"⚕ {_PRODUCT_NAME} Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info(f"Configure {_PRODUCT_NAME} using environment variables or config commands:")
    print_info(f"  {_PRIMARY_CLI} config set model.provider custom")
    print_info(f"  {_PRIMARY_CLI} config set model.base_url http://localhost:8080/v1")
    print_info(f"  {_PRIMARY_CLI} config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info(f"Run '{_PRIMARY_CLI} setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from superforecasting_agent.runtime.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from superforecasting_agent.runtime.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning(f"  Skipped (configure later with '{_PRIMARY_CLI} setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, f"run '{_PRIMARY_CLI} setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from superforecasting_agent.runtime.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via the tools command's Video Generation section.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    try:
        from agent.video_gen_registry import list_providers as _list_video_providers
        from superforecasting_agent.runtime.plugins import _ensure_plugins_discovered as _ensure_plugins
        _ensure_plugins()
        _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, f"run '{_PRIMARY_CLI} setup tts'"))
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, f"run '{_PRIMARY_CLI} setup terminal'"))
    ...
    if disabled_tools:
        print_warning(
            f"Some tools are disabled. Run '{_PRIMARY_CLI} setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('superforecasting-agent setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('superforecasting-agent setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('superforecasting-agent setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('superforecasting-agent setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('superforecasting-agent setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('superforecasting-agent config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('superforecasting-agent config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('superforecasting-agent config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color(_PRIMARY_CLI, Colors.GREEN)}              Open the forecasting desk")
    print(f"   {color(f'{_PRIMARY_CLI} gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color(f'{_PRIMARY_CLI} doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between the tools command and the setup tools section).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``superforecasting-agent model``)
    ...
    provider added to ``superforecasting-agent model`` is automatically available here.
    ...
    # Delegate to the shared model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from superforecasting_agent.runtime.main import select_provider_and_model
    try:
        ...
        print_info(f"You can try again later with: {_PRIMARY_CLI} model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            print_info(
                f"{_PRODUCT_NAME} can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info(f"doesn't currently provide one {_PRODUCT_NAME} can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info(f"Skipped — add later with '{_PRIMARY_CLI} setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                f"Direct OpenAI credentials are still configured and may take precedence until removed from {display_agent_home()}/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the runtime auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via {_PRIMARY_CLI} setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_terminal_backend(config: dict):
    ...
    print_info(f"Choose where {_PRODUCT_NAME} runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("Requires the optional SDK: pip install 'superforecasting-agent[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'superforecasting-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info(f"  Run `{_PRIMARY_CLI} setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    print_info(
        "Each message adds to the forecast-session transcript, which means growing API costs."
    )
    print_info("")
    print_info(
    ...
    print_info(
        "things to durable memory first — but the forecast-session context is cleared."
    )
    print_info("")
    ...
    if reset_idx == 0:  # Both
        ...
        print_warning(
            "Long forecast sessions will grow in cost. Use /reset manually when needed."
        )
...
def _setup_telegram():
    ...
    print()
    print_info(f"📬 Home Channel: where {_PRODUCT_NAME} delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                f"list? (recommended after `{_PRIMARY_CLI} update`)",
                True,
            ):
    ...
    print()
    print_info(f"   Full guide: {_DOCS_BASE}/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    ...
    """
    try:
        from superforecasting_agent.runtime.slack_cli import _build_full_manifest
        from superforecasting_agent.constants import get_agent_home

        manifest = _build_full_manifest(
            bot_name=_PRODUCT_NAME,
            bot_description=f"Your {_PRODUCT_NAME} on Slack",
        )
        target = Path(get_agent_home()) / "slack-manifest.json"
        ...
        print_info(
            f"   Re-run `{_PRIMARY_CLI} slack manifest --write` anytime to refresh after "
            f"{_PRODUCT_NAME} adds new commands."
        )
        ...
        print_info(
            ...
            f"{_PRIMARY_CLI} slack manifest --write"
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info(f"📬 Home Room: where {_PRODUCT_NAME} delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info(f"📬 Home Channel: where {_PRODUCT_NAME} delivers cron job results and notifications.")
    print_info("   To get a channel ID: click channel name → View Info → copy the ID")
    print_info("   You can also set this later by typing /set-home in a Mattermost channel.")
    home_channel = prompt("Home channel ID (leave empty to set later with /set-home)")
    if home_channel:
        save_env_value("MATTERMOST_HOME_CHANNEL", home_channel)
    print_info(f"   Open config in your editor:  {_PRIMARY_CLI} config edit")
...
def _setup_bluebubbles():
    ...
    print_info(f"Connects {_PRODUCT_NAME} to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from superforecasting_agent.runtime.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info(f"   Full guide: {_DOCS_BASE}/user-guide/messaging/webhooks/")
    print()
    ...
    print_info(f"   {_DOCS_BASE}/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info(f"   Open config in your editor:  {_PRIMARY_CLI} config edit")
    print_info(f"   Open config in your editor:  {_PRIMARY_CLI} config edit")
...
def setup_gateway(config: dict):
    ...
    from superforecasting_agent.runtime.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info(f"Connect to messaging platforms to chat with {_PRODUCT_NAME} from anywhere.")
    ...
    if not selected:
        print_info(f"No platforms selected. Run '{_PRIMARY_CLI} setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     {_PRIMARY_CLI} config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info(f"  You can try manually: {_PRIMARY_CLI} gateway install")
            else:
                print_info(f"  You can install later: {_PRIMARY_CLI} gateway install")
                if supports_systemd:
                    print_info(f"  Or as a boot-time service: sudo {_PRIMARY_CLI} gateway install --system")
                print_info(f"  Or run in foreground:  {_PRIMARY_CLI} gateway")
        else:
            from superforecasting_agent.constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info(f"   {_PRIMARY_CLI} gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info(f"   {_PRIMARY_CLI} gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both setup tools and the tools command use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``superforecasting_agent.runtime.auth`` — lists every supported
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from superforecasting_agent.runtime.config
    ...
    if section_key == "model":
        ...
        from superforecasting_agent.runtime.gateway import _all_platforms, _platform_status
...
def _load_openclaw_migration_module():
    """Load the bundled migration helper when it is available."""
    from superforecasting_agent.runtime.openclaw_loader import load_migration_module

    if not _OPENCLAW_SCRIPT.exists():
        return None
    return load_migration_module(_OPENCLAW_SCRIPT)
...
# Config values may have different semantics between OpenClaw and Superforecasting Agent.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Superforecasting Agent to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Superforecasting Agent at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Superforecasting Agent at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Superforecasting Agent at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Superforecasting Agent at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Superforecasting Agent equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Superforecasting Agent config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Superforecasting Agent.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" is not the same as unrestricted tool execution.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info(f"{_PRODUCT_NAME} can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            f"Skipping migration. You can run it later with: {_PRIMARY_CLI} claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            f"Migration cancelled. You can run it later with: {_PRIMARY_CLI} claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Superforecasting Agent configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Superforecasting Agent config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Superforecasting Agent (use {_PRIMARY_CLI} claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      superforecasting-agent setup           — full or quick (auto-detected)
      superforecasting-agent setup model     — just model/provider
      superforecasting-agent setup tts       — just text-to-speech
      superforecasting-agent setup terminal  — just terminal backend
      superforecasting-agent setup gateway   — just messaging platforms
      superforecasting-agent setup tools     — just tool configuration
      superforecasting-agent setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ {_PRODUCT_NAME} Setup — {label:<22s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            f"│             ⚕ {_PRODUCT_NAME} Setup Wizard          │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            f"│  Let's configure your {_PRODUCT_NAME} installation.    │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success(f"You already have {_PRODUCT_NAME} configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info(f"Tip: jump straight to a section with '{_PRIMARY_CLI} setup model|terminal|")
        ...
        setup_mode = prompt_choice(f"How would you like to set up {_PRODUCT_NAME}?", [
    ...
    print()
    print_info(f"You can edit these files directly or use '{_PRIMARY_CLI} config edit'")
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    the user can customize later via the setup command.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            f"Skip — set up later with '{_PRIMARY_CLI} setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info(f"  Configure all settings:    {_PRIMARY_CLI} setup")
    if gateway_choice != 0:
        print_info(f"  Connect Telegram/Discord:  {_PRIMARY_CLI} setup gateway")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info(f"Run '{_PRIMARY_CLI} setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info(f"Connect {_PRODUCT_NAME} to messaging apps to chat from anywhere.")
        print_info(f"You can configure these later with '{_PRIMARY_CLI} setup gateway'.")
