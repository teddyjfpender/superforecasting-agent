"""
Session-scoped context variables shared by runtime hosts and transports.
...
task gets its own context. Executor calls must explicitly propagate it
with ``contextvars.copy_context().run``; ``asyncio.to_thread`` does so automatically.
...
def set_session_vars(
    ...
    Call ``clear_session_vars(tokens)`` in a ``finally`` block to clear
    routing values when the handler exits. This does not restore outer scopes.
