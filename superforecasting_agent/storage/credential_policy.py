...
``auth.json``.  The pool model and final file writer share this policy without importing
credential execution or presentation.
"""
...
# Sources Superforecasting Agent owns and can intentionally persist in
# auth.json.  Everything else with a non-empty source is treated as
# borrowed/reference-only by default so future external secret providers fail
# closed at the disk boundary.
#
# NOTE: these are INTERNAL source identifiers stored in auth.json, not
# user-facing strings -- they are NOT rebranded.  The anthropic OAuth source is
# still literally "hermes_pkce" in this fork (see credential_pool.py
# _seed_from_singletons and agent/credential_sources.py), so it must stay
# "hermes_pkce" here or borrowed-secret stripping would silently drop the raw
# token for the agent-managed Anthropic PKCE login.
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Superforecasting-Agent-owned OAuth/
    device-code state) pass through unchanged.  Borrowed/reference-only sources
    keep labels, source refs, status/cooldown metadata, counters, and a
    non-reversible fingerprint, but raw secret value fields are removed.
    """
    result = dict(payload)
    ...
    sanitized = {
        key: _sanitize_nested_value(value)
...
def _sanitize_nested_value(value: Any) -> Any:
    if isinstance(value, Mapping):
        return {
            key: _sanitize_nested_value(item)
...
def sanitize_auth_store(store: Mapping[str, Any]) -> Dict[str, Any]:
    """Prepare a disk snapshot without changing caller-held runtime credentials."""
    result = dict(store)
    pool = store.get("credential_pool")
    if isinstance(pool, dict):
        result["credential_pool"] = {
            provider: [
