"""Backend worker entrypoint: ``python -m superforecasting_agent.worker run <job_id>``.
...
def main(
    ...
    *,
    program: str = "python -m superforecasting_agent.worker",
) -> int:
    ...
    try:
        ...
        discover_plugins()
    except Exception:  # noqa: BLE001 — existing registered providers remain usable
        logger.warning(
            "Worker plugin discovery failed; using registered providers", exc_info=True
        )
