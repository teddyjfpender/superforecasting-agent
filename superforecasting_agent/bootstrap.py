...
should be imported at the very top of every Superforecasting Agent entry point
(``hermes``, ``hermes-agent``, ``hermes-acp``, ``python -m gateway.run``,
``superforecasting_agent/trajectories/batch.py``, ``cron/scheduler.py``) before any other imports
...
# Apply on import — entry points just need ``import superforecasting_agent.bootstrap``
# (or ``from superforecasting_agent.bootstrap import apply_windows_utf8_bootstrap``) at
