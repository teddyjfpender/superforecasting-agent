...
import { api, FORECAST_BASE_PATH } from "@/lib/api";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${FORECAST_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${FORECAST_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?forecast_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-superforecasting-agent-plugin", manifest.name);
      script.setAttribute("data-forecast-plugin", manifest.name);
