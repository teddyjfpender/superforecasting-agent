 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (superforecasting_agent/runtime/web_server.py)                          .
 *          ▼                                                              .
...
// Channel id ties this Forecast Desk tab's PTY child (publisher) to its sidebar
...
export default function ForecastDeskPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    typeof window !== "undefined" && !getDashboardSessionTokenSync()
      ? "Session token unavailable. Open this page through the dashboard server, not directly."
      : null,
  );
  const [copyState, setCopyState] = useState<"idle" | "copied">("idle");
  const copyResetRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  // Raw state for the mobile side-sheet + a derived value that force-
  // closes whenever the Forecast Desk tab isn't active.  The *derived* value is
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch(() => {
        // Best-effort: old servers or missing sessions should not block Forecast Desk.
      });
  ...
  useEffect(() => {
    // When hidden (non-Forecast-Desk tab) we must not register the header button —
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      // Browser-embedded Forecast Desk runs the TUI in inline mode. Keep transcript
    ...
    //      browser clipboard, so the flow works just like it does in the
    //      terminal forecast desk.
    //
    ...
    // Dashboard Forecast Desk should scroll the browser-side transcript, not send
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[forecast-dashboard] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    //
    // IMPORTANT:
    // The embedded Forecast Desk has occasionally surfaced stray letters/digits
    ...
    // forwarding them into the PTY. Keyboard input, paste, and resize still
    // behave normally.
  ...
  // When the user returns to the Forecast Desk tab (isActive: false → true), the
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          ...
          <Button
            ...
            title="Copy last forecast desk response as raw markdown"
            aria-label="Copy last forecast desk response"
            className={cn(
              "absolute z-10",
