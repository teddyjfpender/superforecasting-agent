...
Usage (via cron with --no-agent):
    ...
    superforecasting-agent cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$AGENT_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "SuperforecastingAgent-Watcher/1.0"})
