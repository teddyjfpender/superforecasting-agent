...
the actual `setup.sh` that creates the project workspace, configures Superforecasting Agent
...
2. **Create profiles** — `superforecasting-agent profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.superforecasting-agent/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `superforecasting-agent kanban create` assigned to the director
...
```bash
superforecasting-agent profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.superforecasting-agent/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.superforecasting-agent/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Superforecasting Agent
...
Each profile gets a `SOUL.md` at `~/.superforecasting-agent/profiles/<name>/SOUL.md` that
...
```bash
superforecasting-agent kanban create "Direct production of <video title>" \
    --assignee director \
...
`~/.superforecasting-agent/.env` and macOS Keychain (if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    if grep -q "^${var}=" ~/.superforecasting-agent/.env 2>/dev/null && \
       [ -n "$(grep "^${var}=" ~/.superforecasting-agent/.env | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ~/.superforecasting-agent/.env or Keychain (${kc_account}/${kc_service})"
    return 1
}
