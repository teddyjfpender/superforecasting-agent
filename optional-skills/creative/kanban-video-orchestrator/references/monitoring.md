...
superforecasting-agent kanban watch --tenant <project-slug>

# Snapshot of the board
superforecasting-agent kanban list --tenant <project-slug>
superforecasting-agent kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
superforecasting-agent kanban stats --tenant <project-slug>
...
Verify available subcommands with `superforecasting-agent kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `superforecasting-agent kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `superforecasting-agent kanban block <id>` then `superforecasting-agent kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `superforecasting-agent kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `superforecasting-agent kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `superforecasting-agent profile list` (profile exists?), `superforecasting-agent status` (gateway/dispatcher up?), and verify tenant. |
...
superforecasting-agent kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
superforecasting-agent kanban create "Scene 3 — re-render with feedback" \
...
superforecasting-agent kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
superforecasting-agent kanban block <task-id>
superforecasting-agent kanban archive <task-id>

# Diagnose what happened
superforecasting-agent kanban show <task-id>      # task body, comments, recent events
superforecasting-agent kanban tail <task-id>      # follow the live event stream
superforecasting-agent kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `superforecasting-agent kanban create` for the director
...
while true; do
    clear
    superforecasting-agent kanban list --tenant <slug>
    echo "---"
    superforecasting-agent kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `superforecasting-agent kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `superforecasting-agent profile list` and start
