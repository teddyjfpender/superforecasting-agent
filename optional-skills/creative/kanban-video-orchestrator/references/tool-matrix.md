...
These are **not** Superforecasting Agent skills but external CLIs / APIs that profiles invoke.
...
## Built-in Superforecasting Agent tools for media review

These are native Superforecasting Agent tools — not invoked via terminal but through their own
...
key is present in `~/.superforecasting-agent/.env` (or macOS Keychain) before firing the kanban.
...
preference: macOS Keychain → `~/.superforecasting-agent/.env` → environment variable.
...
When a new Superforecasting Agent video skill ships:
