...
- **Note:** The previous Open Refine reconciliation endpoint at
  ...
  the ZIP locally (default `~/.cache/superforecasting-agent-osint/icij/`, refreshes after
...
First call downloads the ~70 MB ZIP under `~/.cache/superforecasting-agent-osint/icij/`
(or `$SUPERFORECASTING_AGENT_OSINT_CACHE/icij/`). Subsequent calls reuse the cache for 30 days.
