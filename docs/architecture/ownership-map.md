...
| `superforecasting_agent/hosting/browser_sessions.py` | browser allocation admission | Per-task creation serialization with cache recheck and failure release; unrelated tasks remain independent | No provider, tools, runtime or presentation imports; hosting contracts apply |
| `superforecasting_agent/hosting/browser_connection.py` | browser connection transition | Serialize endpoint publication and cleanup; CLI and TUI supply the same runtime adapter | No runtime, tools or presentation imports; hosting contracts apply |
