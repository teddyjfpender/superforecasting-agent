...
Long-running desk work runs as a **detached job** on one runtime (`forecasting/jobs/runtime.py`) with shared progress coalescing, cancellation, persistence, and desk re-attach. Each capability is a registered `JobType`. There are **9 types**. `spend_class` `agent` means the job spends model budget (it drives the agent); `free` means it does not.
...
| `forecast_interview` | `llm` | 0.125 | `—` | Adaptive questionnaire generation on the shared durable/cancellable job runtime. |
| `forecast_scenarios` | `llm` | 0.125 | `—` | Durable scenario evaluation; never changes the interview or active forecast. |
