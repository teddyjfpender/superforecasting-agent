...
Watched sources should be treated as alert inputs, not automatic forecast updates. A changed watched file, RSS/Atom feed, GDELT search, FRED series, Census or Socrata open-data query, Stooq/Yahoo Finance market price feed, CoinGecko coin snapshot, SEC filing or company-fact observation, arXiv query, PubMed query, GitHub commit or Actions feed, PyPI package release feed, npm package feed, public-attention feed such as Hacker News, Reddit, or Bluesky search, source adapter feed, or future external monitor should create a review alert tied to a question, domain, topic, or portfolio. The agent can then research the changed source and append an explicit forecast update if the probability should move.
