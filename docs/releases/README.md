# Release notes archive

Historical release notes, preserved for provenance. For current setup and usage,
see the [project README](../../README.md) and [development guide](../development.md).

- [v0.14.0](RELEASE_v0.14.0.md)
- [v0.13.0](RELEASE_v0.13.0.md)
- [v0.12.0](RELEASE_v0.12.0.md)
- [v0.11.0](RELEASE_v0.11.0.md)
- [v0.10.0](RELEASE_v0.10.0.md)
- [v0.9.0](RELEASE_v0.9.0.md)
- [v0.8.0](RELEASE_v0.8.0.md)
- [v0.7.0](RELEASE_v0.7.0.md)
- [v0.6.0](RELEASE_v0.6.0.md)
- [v0.5.0](RELEASE_v0.5.0.md)
- [v0.4.0](RELEASE_v0.4.0.md)
- [v0.3.0](RELEASE_v0.3.0.md)
- [v0.2.0](RELEASE_v0.2.0.md)

Beta publication defaults to a prerelease and preserves stable aliases. See the
[current beta verification plan](../verification/2026-09-13-beta-release/README.md).
