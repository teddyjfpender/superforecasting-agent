...
A pre-run snapshot of the active agent-home ``skills/`` directory (excluding
``.curator_backups/`` itself) is taken before any mutating curator pass.
Snapshots are tar.gz files under ``skills/.curator_backups/<utc-iso>/`` with a
...
Alongside the skills tarball, each snapshot also captures a copy of the active
agent-home ``cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def snapshot_skills(reason: str = "manual") -> Optional[Path]:
    """Create a tar.gz snapshot of the active-home ``skills/`` tree and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No active-home skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore the active-home ``skills/`` tree from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into the active-home ``skills/`` tree.
