"""Unified removal contract for every credential source Superforecasting Agent reads from.

Superforecasting Agent seeds its credential pool from many places:
    ...
    manual        — user ran `superforecasting-agent auth add`
...
unify here is **removal**:
    ...
    ``superforecasting-agent auth remove <provider> <N>`` must make the pool entry stay gone.
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    """
    from superforecasting_agent.runtime.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Superforecasting Agent.",
            f"  The pool entry is now suppressed — Superforecasting Agent will ignore "
            f"{env_var} until you run `superforecasting-agent auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Superforecasting Agent stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `superforecasting-agent auth add anthropic` to re-enable if needed.",
    ])
...
def _remove_hermes_pkce(provider: str, removed) -> RemovalResult:
    ...
    result = RemovalResult()
    oauth_file = get_agent_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Superforecasting Agent Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    user's next `superforecasting-agent login` run from writing providers.nous again
    before they decide to.  Suppression forces them to go through
    `superforecasting-agent auth add nous` to re-engage, which is the documented re-add
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``superforecasting-agent auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``superforecasting-agent auth add
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `superforecasting-agent model` → xAI Grok OAuth (SuperGrok Subscription) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Superforecasting Agent auth store is not enough — _seed_from_singletons() would
    ...
    via ``superforecasting-agent auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `superforecasting-agent auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `superforecasting-agent auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Superforecasting Agent stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `superforecasting-agent auth add copilot` to re-enable if needed.",
    ])
