...
from superforecasting_agent.tooling.runtime import check_toolset_requirements, get_tool_definitions
...
def init_agent(
    ...
    agent._cache_ttl = "5m"
    try:
        from superforecasting_agent.runtime.config import load_config as _load_pc_cfg
    ...
    if agent.verbose_logging:
        ...
        # on the fact that superforecasting_agent.logging.setup_logging() does not
        ...
        # handler level inside superforecasting_agent/logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from superforecasting_agent.runtime.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `superforecasting-agent model` to "
                        "select a provider, or run `superforecasting-agent setup` for first-time "
                        "configuration."
                    )
    ...
    try:
        from superforecasting_agent.runtime.config import load_config as _load_agent_config
        _agent_cfg = _load_agent_config()
    except Exception:
        _agent_cfg = {}
    agent.tool_discovery_config = _agent_cfg.get("tool_discovery", {})
    try:
        agent._tool_guardrails = ToolCallGuardrailController(
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from superforecasting_agent.runtime.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Superforecasting Agent.  Choose a model with at least "
