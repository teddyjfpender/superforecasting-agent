...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``superforecasting-agent model`` /
        ``superforecasting-agent auth`` which performs a fresh device-code
        login and writes new tokens to ``auth.json`` under
        ``_auth_store_lock``. Without this sync the pool entry stays frozen
        until ``last_error_reset_at`` elapses — even though fresh credentials
        are sitting on disk — and every request fails with "no available
        entries (all exhausted or empty)".
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        setup silently flips the ``active_provider`` flag — the next CLI
        invocation that defaults to the active provider (e.g. setup wizard,
        ``superforecasting-agent auth status``) would land on whatever provider
        happened to refresh last, not whatever the user actually chose.
        """
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # re-authed via `superforecasting-agent model` /
            # `superforecasting-agent auth` after a 429/401,
            ...
            # tokens that another process (or a fresh
            # `superforecasting-agent model` -> xAI Grok OAuth login) has
            # since rotated in auth.json.
            if (self.provider == "xai-oauth"
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code,
        # agent-managed PKCE) when the user has explicitly configured
        # anthropic as their provider.
        ...
        for source_name, creds in (
            ("hermes_pkce", read_agent_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `superforecasting-agent auth add nous --label <name>`). Fall back to the
        ...
        # agent auth store or env vars, so resolve them here.
        ...
        # MiniMax OAuth tokens live in the active agent-home auth.json under
        # providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `superforecasting-agent auth remove minimax-oauth <N>` flow
        # works.
        ...
        # Respect user suppression — `superforecasting-agent auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the agent auth store. Without this gate the removal is instantly
        ...
        # The agent owns its own Codex auth state — we do NOT auto-import from
        ...
        # via `superforecasting-agent auth openai-codex`.
        ...
        # When the user logs in via ``superforecasting-agent model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``superforecasting-agent auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer the active agent-home .env over os.environ — the user's config file
    # is the authoritative source for agent credentials. Stale env vars from
    # parent processes (Codex CLI, test scripts, etc.) should not override
    # deliberate changes to the .env file.
    def _get_env_prefer_dotenv(key: str) -> str:
        env_file = load_env()
        val = env_file.get(key) or os.environ.get(key) or ""
        return val.strip()

    # Honour user suppression — `superforecasting-agent auth remove <provider>
    # <N>` for an env-seeded credential marks the env:<VAR> source as
    # suppressed so it won't be re-seeded from the user's shell environment or
    ...
    if provider == "openrouter":
        # Prefer active agent-home .env over os.environ.
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            # hermes_pkce is Superforecasting-Agent-owned/persistable while
            # present, but it is still a file-backed singleton and should
            # disappear from the pool when the backing OAuth file is gone.
