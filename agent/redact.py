...
# (bridged to these env vars in superforecasting_agent/runtime/main.py, gateway/run.py, and
...
# Adapted from upstream repr-redaction fixes; provenance: docs/upstream-sync.md.
...
def _redact_python_repr_fields(text: str) -> str:
    ...
    def _sub(match: re.Match) -> str:
        ...
        if re.fullmatch(r"(?:[A-Za-z]+ )?\*{3}", value) or (value.startswith("«redacted:") and value.endswith("»")):
            return match.group(0)
...
def redact_terminal_output(
    ...
    """Redact secrets from background-process stdout.

    Redaction policy for the background ``process(action=poll/log/wait)``
    output path (see ``_redact_process_result`` in ``tools/process_registry``).
    The foreground ``terminal`` result path does NOT route through here — it
    calls ``redact_sensitive_text(..., code_file=False)`` directly (see
    ``tools/terminal_tool.py``) and is deliberately not migrated, as its
    output-transform hook contract depends on that direct call. Picks
    ``code_file`` based on whether ``command`` is an environment dump:
    ...
    text = redact_sensitive_text(output, force=force, code_file=code_file)
    if code_file and (force or _REDACT_ENABLED):
        text = _redact_python_diagnostic_repr_fields(text)
    return text
