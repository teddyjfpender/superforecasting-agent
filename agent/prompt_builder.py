...
def _scan_context_content(content: str, filename: str) -> str:
    ...
    Uses the "context" scope from the shared threat-pattern library (classic
    injection + promptware/C2 + role-play hijack). Strict-scope patterns
    (SSH backdoor, persistence, exfil-URL) are intentionally NOT applied here
    — too aggressive for a context file in a cloned repo (security research,
    infra docs). A match BLOCKS the file at this layer because it would
    otherwise enter the system prompt verbatim with no chance for the user
    to intervene.
    """
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at active agent-home `kanban.db`. Your task id is in "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `superforecasting-agent kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Superforecasting Agent web dashboard, a browser-based forecast desk interface. "
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Superforecasting Agent itself is running. The host OS, home, and cwd "
                f"of the agent process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Superforecasting Agent "
