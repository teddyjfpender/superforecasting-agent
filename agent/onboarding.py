...
def openclaw_residue_hint_cli() -> str:
    """Banner shown when the forecast desk first finds ``~/.openclaw/``.

    Points users at ``superforecasting-agent claw migrate`` (non-destructive
    port of config, memory, and skills) first. Cleanup is mentioned as the
    follow-up step for users who have already migrated and want to archive the
    old directory — with a warning that archiving breaks OpenClaw.
    """
    return (
        "A legacy OpenClaw directory was detected at ~/.openclaw/.\n"
        "To port your config, memory, and skills over to Superforecasting Agent, run "
        "`superforecasting-agent claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `superforecasting-agent claw cleanup` (renames it to ~/.openclaw.pre-migration — "
