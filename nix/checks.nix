...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      superforecastingAgent = self'.packages.default;
      superforecastingAgentVenv = superforecastingAgent.superforecastingAgentVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "superforecasting-agent-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${superforecastingAgentVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "superforecasting-agent-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "superforecasting-agent-package-contents" { } ''
          ...
          test -x ${superforecastingAgent}/bin/hermes || (echo "FAIL: hermes compatibility binary missing"; exit 1)
          test -x ${superforecastingAgent}/bin/hermes-agent || (echo "FAIL: hermes-agent compatibility binary missing"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "superforecasting-agent-entry-points-sync" { } ''
          ...
          for bin in forecast superforecast superforecasting-agent hermes hermes-agent hermes-acp; do
            test -x ${superforecastingAgent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "superforecasting-agent-cli-commands" { } ''
          ...
          echo "=== Checking superforecasting-agent inherited subcommands ==="
          ${superforecastingAgent}/bin/superforecasting-agent gateway --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${superforecastingAgent}/bin/superforecasting-agent config --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "superforecasting-agent-bundled-skills" { } ''
          ...
          test -d ${superforecastingAgent}/share/hermes-agent/skills || (echo "FAIL: compatibility skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${superforecastingAgent}/share/superforecasting-agent/skills -name "SKILL.md" | wc -l)
        ...
        bundled-plugins = pkgs.runCommand "superforecasting-agent-bundled-plugins" { } ''
          ...
          test -d ${superforecastingAgent}/share/hermes-agent/plugins || (echo "FAIL: compatibility plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${superforecastingAgent}/share/superforecasting-agent/plugins/platforms/irc/plugin.yaml || \
        ...
        bundled-tui = pkgs.runCommand "superforecasting-agent-bundled-tui" { } ''
          ...
          test -f ${superforecastingAgent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
        ...
        managed-guard = pkgs.runCommand "superforecasting-agent-managed-guard" { } ''
          ...
          check_blocked "config set (native)" SUPERFORECASTING_AGENT_MANAGED ${superforecastingAgent}/bin/superforecasting-agent config set model foo
          check_blocked "config edit (short)" FORECAST_MANAGED ${superforecastingAgent}/bin/superforecasting-agent config edit
          check_blocked "config set (legacy)" HERMES_MANAGED ${superforecastingAgent}/bin/superforecasting-agent config set model foo
        ...
        in pkgs.runCommand "superforecasting-agent-extra-dependency-groups" { } ''
          ...
          echo "derivation: ${superforecastingAgentWithGroups}"
          echo "venv: ${superforecastingAgentWithGroups.superforecastingAgentVenv}"
        ...
        in pkgs.runCommand "superforecasting-agent-config-roundtrip" {
