# nix/nixosModules.nix — NixOS module for Superforecasting Agent
#
# Two modes:
  ...
  in {
    ...
    options.services.superforecasting-agent = with lib; {
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into the shared forecast home .env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in the shared forecast home mcp-tokens/.
              '';
            };
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in this fork's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add fork-native and compatibility CLIs to environment.systemPackages
          and export SUPERFORECASTING_AGENT_HOME, FORECAST_HOME, and legacy
          HERMES_HOME system-wide so interactive shells share state with the
          gateway service.
        '';
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the CLI to system PATH and export fork-native home aliases plus
      # legacy HERMES_HOME so interactive shells share state with the gateway
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.superforecasting-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.superforecasting-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed superforecasting-agent
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure superforecasting-agent is on PATH.
          ''
        ];
      })
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.superforecasting-agent = {
          ...
          serviceConfig = {
            ...
            # the shared .env by the activation script. load_hermes_dotenv()
