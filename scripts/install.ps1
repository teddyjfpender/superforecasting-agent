# ============================================================================
# Superforecasting Agent Installer for Windows
# ============================================================================
...
param(
    [switch]$NoVenv,
    [switch]$SkipSetup,
    [string]$Branch = "superforecasting-agent-snapshot",
...
$RepoUrlSsh = "git@github.com:teddyjfpender/superforecasting-agent.git"
$RepoUrlHttps = "https://github.com/teddyjfpender/superforecasting-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Install-Git {
    ...
    which Superforecasting Agent uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\superforecasting-agent\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Superforecasting Agent needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the agent-managed copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\superforecasting-agent\git``
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent forecast runtime features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Superforecasting Agent needs Git Bash on Windows to run"
...
function Test-Node {
    ...
    if (Test-Path $managedNode) {
        ...
        $env:Path = "$HermesHome\node;$env:Path"
        Write-Success "Node.js $version found (forecast-runtime-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/teddyjfpender/superforecasting-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/teddyjfpender/superforecasting-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/teddyjfpender/superforecasting-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\superforecasting-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\superforecasting-agent-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where console-script exes
        # exist in the wrong directory and imports fail with ModuleNotFoundError.
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install superforecasting-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and console-script exes broken with
    ...
    # here, not 30 seconds later when the user runs `superforecasting-agent`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `superforecasting-agent dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `superforecasting-agent dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `superforecasting-agent dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up superforecasting-agent command..."
    ...
    # Add the venv Scripts dir to user PATH so superforecasting-agent is globally available.
    # On Windows, the generated console-script exe in venv\Scripts\ has the venv Python baked in.
    ...
    # Set forecast-native home aliases so the Python code finds config/data in
    # the right place. Keep HERMES_HOME bridged for inherited modules.
    $currentForecastHome = [Environment]::GetEnvironmentVariable("SUPERFORECASTING_AGENT_HOME", "User")
    if (-not $currentForecastHome -or $currentForecastHome -ne $HermesHome) {
        [Environment]::SetEnvironmentVariable("SUPERFORECASTING_AGENT_HOME", $HermesHome, "User")
        Write-Success "Set SUPERFORECASTING_AGENT_HOME=$HermesHome"
    }
    $currentShortForecastHome = [Environment]::GetEnvironmentVariable("FORECAST_HOME", "User")
    if (-not $currentShortForecastHome -or $currentShortForecastHome -ne $HermesHome) {
        [Environment]::SetEnvironmentVariable("FORECAST_HOME", $HermesHome, "User")
        Write-Success "Set FORECAST_HOME=$HermesHome"
    }
    ...
    $env:SUPERFORECASTING_AGENT_HOME = $HermesHome
    $env:FORECAST_HOME = $HermesHome
    $env:HERMES_HOME = $HermesHome
    
    # Update current session
    $env:Path = "$hermesBin;$env:Path"
    
    Write-Success "superforecasting-agent command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create the Superforecasting Agent directory structure.
    ...
    # (the default PS5 behaviour), and the prompt-injection scanner flags
    # the BOM as an invisible unicode character and refuses to
...
"@
    ...
    # Seed bundled skills into the forecast-native home (manifest-based, one-time per skill).
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'superforecasting-agent setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
...
function Install-PlatformSdks {
    ...
    # the configured agent home .env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'superforecasting-agent setup'."
        return
    }
...
function Start-GatewayIfConfigured {
    ...
    $hermesCmd = "$InstallDir\venv\Scripts\superforecasting-agent.exe"
    if (-not (Test-Path $hermesCmd)) {
        $legacyHermesCmd = "$InstallDir\venv\Scripts\hermes.exe"
        if (Test-Path $legacyHermesCmd) {
            $hermesCmd = $legacyHermesCmd
        } else {
            $hermesCmd = "superforecasting-agent"
        }
    }
    ...
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        ...
        Write-Info "Running 'superforecasting-agent whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: superforecasting-agent gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            Write-Warn "Failed to start gateway. Run manually: superforecasting-agent gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: superforecasting-agent gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host ""
    Write-Host "   superforecasting-agent" -NoNewline -ForegroundColor Green
    Write-Host "Open the forecast desk"
    Write-Host "   forecast            " -NoNewline -ForegroundColor Green
    Write-Host "Open the forecast CLI"
    Write-Host "   superforecasting-agent setup" -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   superforecasting-agent config" -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   superforecasting-agent config edit" -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   superforecasting-agent gateway" -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   superforecasting-agent update" -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Superforecasting Agent repository"; Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    ...
    @{ Name = "path";             Title = "Adding Superforecasting Agent to PATH"; Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/teddyjfpender/superforecasting-agent/superforecasting-agent-snapshot/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
