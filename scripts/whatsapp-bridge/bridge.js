#!/usr/bin/env node
/**
 * Superforecasting Agent WhatsApp Bridge
 *
 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.superforecasting-agent/whatsapp/session
 */
...
const FORECAST_HOME = process.env.SUPERFORECASTING_AGENT_HOME || process.env.FORECAST_HOME || process.env.HERMES_HOME || path.join(process.env.HOME || '~', '.superforecasting-agent');
const SESSION_DIR = getArg('session', path.join(FORECAST_HOME, 'whatsapp', 'session'));
const IMAGE_CACHE_DIR = path.join(FORECAST_HOME, 'image_cache');
const DOCUMENT_CACHE_DIR = path.join(FORECAST_HOME, 'document_cache');
const AUDIO_CACHE_DIR = path.join(FORECAST_HOME, 'audio_cache');
const PAIR_ONLY = args.includes('--pair-only');
const WHATSAPP_MODE = getArg('mode', process.env.WHATSAPP_MODE || 'self-chat'); // "bot" or "self-chat"
const ALLOWED_USERS = parseAllowedUsers(process.env.WHATSAPP_ALLOWED_USERS || '');
const DEFAULT_REPLY_PREFIX = '*Superforecasting Agent*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Superforecasting Agent', 'Chrome', '120.0'],
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore the agent's own reply messages in self-chat mode to avoid loops.
  ...
  try {
    ...
    switch (type) {
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `superforecasting_agent_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execSync(
