#!/usr/bin/env python3
"""Superforecasting Agent Release Script

Generates a legacy CalVer changelog preview. It never publishes.
...
Usage:
    ...
    python scripts/release.py --bump minor --first-release
"""
...
VERSION_FILE = REPO_ROOT / 'superforecasting_agent/runtime' / "__init__.py"
...
AUTHOR_MAP = {
    "teddyjfpender@gmail.com": "teddyjfpender",
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"superforecasting-agent[acp]=={semver}"
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/teddyjfpender/superforecasting-agent",
    ...
    lines.append(f"# Superforecasting Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Superforecasting Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Superforecasting Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Superforecasting Agent Release Preview")
    print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print("  Dry run complete. Publish only through production-release.yml.")
    print(f"{'='*60}")
