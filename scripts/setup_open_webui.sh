...
# Bootstrap Open WebUI against Superforecasting Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.superforecasting-agent/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-superforecasting-agent.sh
...
#   FORECAST_API_PORT=8642
#   FORECAST_API_HOST=127.0.0.1
#   FORECAST_API_MODEL_NAME='superforecasting-agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Superforecasting Agent WebUI}"
...
FORECAST_HOME="${SUPERFORECASTING_AGENT_HOME:-${FORECAST_HOME:-$HOME/.superforecasting-agent}}"
FORECAST_ENV_FILE="${FORECAST_ENV_FILE:-${HERMES_ENV_FILE:-$FORECAST_HOME/.env}}"
FORECAST_API_PORT="${FORECAST_API_PORT:-${HERMES_API_PORT:-8642}}"
FORECAST_API_HOST="${FORECAST_API_HOST:-${HERMES_API_HOST:-127.0.0.1}}"
FORECAST_API_CONNECT_HOST="${FORECAST_API_CONNECT_HOST:-${HERMES_API_CONNECT_HOST:-127.0.0.1}}"
FORECAST_API_MODEL_NAME="${FORECAST_API_MODEL_NAME:-${HERMES_API_MODEL_NAME:-superforecasting-agent}}"
FORECAST_API_BASE_URL="http://${FORECAST_API_CONNECT_HOST}:${FORECAST_API_PORT}/v1"
HERMES_ENV_FILE="$FORECAST_ENV_FILE"
HERMES_API_PORT="$FORECAST_API_PORT"
HERMES_API_HOST="$FORECAST_API_HOST"
HERMES_API_CONNECT_HOST="$FORECAST_API_CONNECT_HOST"
HERMES_API_MODEL_NAME="$FORECAST_API_MODEL_NAME"
HERMES_API_BASE_URL="$FORECAST_API_BASE_URL"
LAUNCHER_PATH="${LAUNCHER_PATH:-$HOME/.local/bin/start-open-webui-superforecasting-agent.sh}"
LOG_DIR="${LOG_DIR:-$FORECAST_HOME/logs}"
AGENT_BIN="${SUPERFORECASTING_AGENT_BIN:-${HERMES_BIN:-}}"
...
resolve_agent_bin() {
  ...
  if command -v superforecasting-agent >/dev/null 2>&1; then
    AGENT_BIN="superforecasting-agent"
  elif command -v hermes >/dev/null 2>&1; then
    AGENT_BIN="hermes"
  else
    echo "Missing required command: superforecasting-agent" >&2
    exit 1
  fi
}
...
install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.superforecasting-agent.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.superforecasting-agent</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.superforecasting-agent"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.superforecasting-agent"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-superforecasting-agent.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Superforecasting Agent
...
EOF
  ...
  systemctl --user enable --now openwebui-superforecasting-agent.service
}
...
main() {
  ...
  log 'Ensuring Superforecasting Agent API server is configured...'
  ...
  log 'Restarting Superforecasting Agent gateway so API server settings take effect...'
  "$AGENT_BIN" gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${HERMES_API_CONNECT_HOST}:${HERMES_API_PORT}/health" >/dev/null; then
    log 'Superforecasting Agent API server did not answer on the first check. Trying to start gateway in the background...'
    nohup "$AGENT_BIN" gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  ...
  log "Superforecasting Agent API endpoint: ${HERMES_API_BASE_URL}"
