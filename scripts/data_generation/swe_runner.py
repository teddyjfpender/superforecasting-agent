...
compatible with superforecasting_agent/trajectories/batch.py and the trajectory compression module.
...
Usage:
    ...
    python -m scripts.data_generation.swe_runner --task "Create a hello world Python script" --env local
    
    # Run with Docker
    python -m scripts.data_generation.swe_runner --task "List files in /tmp" --env docker --image python:3.11-slim
    
    # Run with Modal (cloud)
    python -m scripts.data_generation.swe_runner --task "Install numpy and test it" --env modal --image python:3.11-slim
    
    # Batch mode from JSONL file
    python -m scripts.data_generation.swe_runner --prompts_file prompts.jsonl --output_file trajectories.jsonl --env docker
"""
...
class MiniSWERunner:
    ...
    def _convert_to_hermes_format(
        ...
        This produces the exact format used by superforecasting_agent/trajectories/batch.py.
        """
        trajectory = []
...
def main(
    ...
    Examples:
        ...
        python -m scripts.data_generation.swe_runner --task "Create hello.py that prints Hello World"
        
        # Single task with Docker
        python -m scripts.data_generation.swe_runner --task "List files" --env docker
        
        # Batch from file
        python -m scripts.data_generation.swe_runner --prompts_file tasks.jsonl --output_file results.jsonl
    """
    ...
    if task:
        ...
        print("   Example: python -m scripts.data_generation.swe_runner --task 'Create a hello world script'")
