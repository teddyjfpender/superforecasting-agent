#!/usr/bin/env python3
"""Build the Superforecasting Agent Skills Index.
...
`superforecasting-agent skills search/install` can use it without hitting the
GitHub API.
...
# Ensure a home is set for tools/skills_hub.py imports. HERMES_HOME remains
# accepted by the runtime, but new invocations should use a fork-native home.
os.environ.setdefault(
    "SUPERFORECASTING_AGENT_HOME",
    os.path.join(os.path.expanduser("~"), ".superforecasting-agent"),
)
...
def main():
    print("Building Superforecasting Agent Skills Index...", flush=True)
