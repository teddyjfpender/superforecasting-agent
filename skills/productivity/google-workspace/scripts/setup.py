#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Superforecasting Agent.
...
# Ensure sibling modules (_workspace_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _workspace_home import display_agent_home, get_agent_home
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Superforecasting Agent profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'superforecasting-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to the agent home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_agent_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Superforecasting Agent")
