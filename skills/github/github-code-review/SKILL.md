...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.superforecasting-agent/.env ] && grep -q "^GITHUB_TOKEN=" ~/.superforecasting-agent/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.superforecasting-agent/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Superforecasting Agent\",
    \"comments\": [
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Superforecasting Agent. Code looks clean — good test coverage, no security concerns."
...
curl -s -X POST \
  ...
  -d "{
    ...
    \"body\": \"## Superforecasting Agent Review\n\nFound 2 issues, 1 suggestion. See inline comments.\",
    \"comments\": [
...
---
*Reviewed by Superforecasting Agent*
EOF
)"
```
