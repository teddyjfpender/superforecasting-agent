...
AGENT_HOME_DIR="${SUPERFORECASTING_AGENT_HOME:-${FORECAST_HOME:-${HERMES_HOME:-$HOME/.superforecasting-agent}}}"
AGENT_CFG="${AGENT_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Superforecasting Agent config has twozero_td MCP entry ──
if [[ ! -f "$AGENT_CFG" ]]; then
    echo -e " ${FAIL} Superforecasting Agent config not found at ${AGENT_CFG}"
    manual_steps+=("Create ${AGENT_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$AGENT_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Superforecasting Agent config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Superforecasting Agent config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${AGENT_CFG} manually"); }
    manual_steps+=("Restart Superforecasting Agent session to pick up config change")
fi
