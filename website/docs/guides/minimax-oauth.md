...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax OAuth for forecast-runtime inference"
---

# MiniMax OAuth

Superforecasting Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and the forecast runtime automatically refreshes your session.
...
Use MiniMax as one judgment and decomposition engine inside the forecasting desk. Forecast records should store the provider id, model variant, region, prompt version, and run timestamp so later scoring can distinguish MiniMax-assisted rationales from other model runs and from calibrated ensemble probabilities.
...
# → Superforecasting Agent opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.superforecasting-agent/auth.json` and are refreshed automatically before each session.
...
```bash
superforecasting-agent auth add minimax-cn --type oauth   # if OAuth is supported on your CN account
# or simpler:
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.superforecasting-agent/.env
```
...
```bash
superforecasting-agent auth add minimax-oauth --no-browser
```

Superforecasting Agent will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Superforecasting Agent implements a PKCE device-code flow against the MiniMax OAuth endpoints:

1. Superforecasting Agent generates a PKCE verifier / challenge pair and a random state value.
...
4. Superforecasting Agent polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.superforecasting-agent/auth.json` under the `minimax-oauth` key.
...
```bash
superforecasting-agent config set model MiniMax-M2.7
superforecasting-agent config set provider minimax-oauth
```

## Configuration Reference

After login, `~/.superforecasting-agent/config.yaml` will contain entries similar to:
...
```bash
superforecasting-agent --provider minimax-oauth    # canonical
superforecasting-agent --provider minimax-portal   # alias
superforecasting-agent --provider minimax-global   # alias
superforecasting-agent --provider minimax_oauth    # alias (underscore form)
```
...
To force the `minimax-oauth` provider for a single run:

```bash
superforecasting-agent --provider minimax-oauth
```
...
Superforecasting Agent refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Superforecasting Agent marks the session as requiring re-login.
...
**Fix:** run `superforecasting-agent auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Superforecasting Agent raises a timeout error.

**Fix:** re-run `superforecasting-agent auth add minimax-oauth` (or `superforecasting-agent model`). The flow starts fresh.

### State mismatch (possible CSRF)

Superforecasting Agent detected that the `state` value returned by the authorization server does not match what it sent.
...
If `superforecasting-agent` cannot open a browser window, use `--no-browser`:

```bash
superforecasting-agent auth add minimax-oauth --no-browser
```

Superforecasting Agent prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `superforecasting-agent model` and select MiniMax (OAuth), or run `superforecasting-agent auth add minimax-oauth`.
...
```bash
superforecasting-agent auth remove minimax-oauth
```
...
- [superforecasting-agent doctor](../reference/cli-commands.md)
