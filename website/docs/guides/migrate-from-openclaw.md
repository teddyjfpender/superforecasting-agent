...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup into the Superforecasting Agent fork."
---

# Migrate from OpenClaw

`superforecasting-agent claw migrate` imports your OpenClaw or legacy Clawdbot/Moldbot setup into the Superforecasting Agent fork. This guide covers exactly what gets migrated, the config key mappings, and what to verify before using the migrated setup as a forecast desk.

## Quick start

```bash
# Preview then migrate (always shows a preview first, then asks to confirm)
superforecasting-agent claw migrate

# Preview only, no changes
superforecasting-agent claw migrate --dry-run

# Full migration including API keys, skip confirmation
superforecasting-agent claw migrate --preset full --migrate-secrets --yes
```
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.superforecasting-agent/` (by default a single restore-point archive is written before apply, under `~/.superforecasting-agent/backups/pre-migration-*.zip`; restorable with `superforecasting-agent import`). |
...
| What | OpenClaw source | Superforecasting Agent destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.superforecasting-agent/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.superforecasting-agent/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.superforecasting-agent/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.superforecasting-agent/memories/MEMORY.md` | All daily files merged into main memory. |
...
| Source | OpenClaw location | Superforecasting Agent destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.superforecasting-agent/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.superforecasting-agent/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.superforecasting-agent/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.superforecasting-agent/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Superforecasting Agent skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | Superforecasting Agent destination | Notes |
|------|---------------------|-------------------|-------|
...
| Provider API keys | `models.providers.*.apiKey` | `~/.superforecasting-agent/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |

### Agent behavior

| What | OpenClaw config path | Superforecasting Agent config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | Superforecasting Agent config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | Superforecasting Agent field | Notes |
|----------------|-------------|-------|
...
| What | Superforecasting Agent destination |
|------|-------------------|
...
| TTS assets | `~/.superforecasting-agent/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | Superforecasting Agent `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | Superforecasting Agent path | Notes |
|------|-------------|-------------|-------|
...
| Gateway auth token | `gateway.auth.token` | `.env` secret imported by setup | Requires `--migrate-secrets` |
| Working directory | `agents.defaults.workspace` | `config.yaml` → `terminal.cwd` | |

### Archived (no direct Superforecasting Agent equivalent)

These are saved to `~/.superforecasting-agent/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in Superforecasting Agent |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | Superforecasting Agent has built-in tool instructions |
...
| Multi-agent list | `archive/agents-list.json` | Use Superforecasting Agent profiles |
...
The migration resolves all three formats. For env templates and SecretRef objects with `source: "env"`, it looks up the value in `~/.openclaw/.env` and the `openclaw.json` env sub-object. SecretRef objects with `source: "file"` or `source: "exec"` can't be resolved automatically — the migration warns about these, and those values must be added to Superforecasting Agent manually via `superforecasting-agent config set`.

## After migration

1. **Check the migration report** — printed on completion with counts of migrated, skipped, and conflicting items.

2. **Review archived files** — anything in `~/.superforecasting-agent/migration/openclaw/<timestamp>/archive/` needs manual attention.

3. **Start a new session** — imported skills and memory entries take effect in new sessions, not the current one.

4. **Verify API keys** — run `superforecasting-agent status` to check provider authentication.

5. **Test messaging** — if you migrated platform tokens, restart the gateway: `superforecasting-agent gateway restart`

6. **Check session policies** — verify `superforecasting-agent config get session_reset` matches your expectations.

7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `superforecasting-agent whatsapp` to pair.

8. **Archive cleanup** — after confirming everything works, run `superforecasting-agent claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
Keys might be stored in several places depending on your OpenClaw version: inline in `openclaw.json` under `models.providers.*.apiKey`, in `~/.openclaw/.env`, in the `openclaw.json` `"env"` sub-object, or in `agents/main/agent/auth-profiles.json`. The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `superforecasting-agent config set`.

### Skills not appearing after migration

Imported skills land in `~/.superforecasting-agent/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

### TTS voice not migrated

OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `superforecasting-agent config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
