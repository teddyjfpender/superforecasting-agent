...
title: "Build a Superforecasting Agent Plugin"
description: "Build a plugin for forecast tools, hooks, data files, and skills."
---

# Build a Superforecasting Agent Plugin

This guide walks through building a complete Superforecasting Agent plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — the general plugin surface inherited by the forecasting fork.

:::info Not sure which guide you need?
Superforecasting Agent has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `SUPERFORECASTING_AGENT_LOCAL_STT_COMMAND` to a shell template; legacy `HERMES_LOCAL_STT_COMMAND` is still accepted |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.superforecasting-agent/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `superforecasting-agent skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
```bash
mkdir -p ~/.superforecasting-agent/plugins/calculator
cd ~/.superforecasting-agent/plugins/calculator
```
...
This tells Superforecasting Agent: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** The runtime may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `superforecasting-agent my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Superforecasting Agent continues fine
...
The same logs are always written to `~/.superforecasting-agent/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
superforecasting-agent logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `superforecasting-agent plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout** — must be `~/.superforecasting-agent/plugins/<plugin-name>/plugin.yaml` (flat) or `~/.superforecasting-agent/plugins/<category>/<plugin-name>/plugin.yaml` (one level of category nesting, max). Anything deeper is ignored.
...
```
~/.superforecasting-agent/plugins/calculator/
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.superforecasting-agent/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.superforecasting-agent/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `superforecasting-agent plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing the runtime into installing arbitrary packages — only specs the fork itself ships are eligible |
...
auditable in `~/.superforecasting-agent/logs/agent.log`. Plugins load after built-in
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), the runtime injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers terse forecast rationales\n- Last project: inflation-forecast-ledger"}
...
- **The system prompt is the fork runtime's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `superforecasting-agent <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for superforecasting-agent my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: superforecasting-agent my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for superforecasting-agent my-plugin."""
...
After registration, users can run `superforecasting-agent my-plugin status`, `superforecasting-agent my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `superforecasting-agent name` in a terminal |
...
Superforecasting Agent has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.superforecasting-agent/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `superforecasting-agent gateway status` without SDK instantiation.
...
Superforecasting Agent also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Superforecasting Agent without any Python plugin. Declare them in `~/.superforecasting-agent/config.yaml`:
...
Superforecasting Agent connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.superforecasting-agent/hooks/<name>/`:

```yaml
# ~/.superforecasting-agent/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.superforecasting-agent/hooks/long-task-alert/handler.py
...
```bash
superforecasting-agent skills tap add myorg/skills-repo
superforecasting-agent skills search my-workflow --source myorg/skills-repo
superforecasting-agent skills install myorg/skills-repo/my-workflow
```
...
`hermes_agent.plugins` is the inherited entry-point group name retained for compatibility; use it until the plugin loader grows a fork-native group.

```bash
pip install hermes-plugin-calculator
# Plugin auto-discovered on next superforecasting-agent startup
```
...
```python
# Wrong — will break if the runtime passes extra context
def handler(args):
    ...
