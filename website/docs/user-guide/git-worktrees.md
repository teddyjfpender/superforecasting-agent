...
title: "Git Worktrees"
description: "Run multiple Superforecasting Agent sessions safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Superforecasting Agent can operate on large, long-lived repositories when you are building source adapters, benchmark importers, model code, or forecast dashboards. When you want to:
...
This page shows how to combine worktrees with Superforecasting Agent so each session has a clean, isolated working directory.

## Why Use Worktrees?

Superforecasting Agent treats the **current working directory** as the project root:

- CLI: the directory where you run `superforecasting-agent`, `forecast`, or compatibility `hermes`
- Messaging gateways: the directory set by `terminal.cwd`
...
git worktree add ../repo-feature feature/forecast-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/forecast-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Superforecasting Agent there:

```bash
cd ../repo-feature

# Start the forecast desk in the worktree
...
git worktree add ../repo-experiment-a feature/forecast-a
git worktree add ../repo-experiment-b feature/forecast-b
```
...
Each forecast desk process:

- Works on its own branch (`feature/forecast-a` vs `feature/forecast-b`).
...
- Forecast checkpoint data under `~/.superforecasting-agent/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small. Legacy `~/.hermes/checkpoints/` may still exist during migration.

## Best Practices

- **One worktree per forecast-desk experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/forecast-ledger-docs`, `feature/source-adapter-tests`.
- **Commit frequently**
...
- **Avoid running the forecast desk from the bare repo root when using worktrees**
...
- Use **git worktrees** to give each forecast-desk session its own clean checkout.
