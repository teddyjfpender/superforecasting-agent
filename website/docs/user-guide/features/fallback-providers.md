...
When the main provider fails, the runtime can switch to a configured backup provider/model for the current turn without losing conversation context.
...
If no compression provider is available, the runtime drops middle conversation turns without generating a summary rather than failing the session. Forecast state remains in the ledger.
