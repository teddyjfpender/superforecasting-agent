...
description: "Operate the Teams meeting summary pipeline via Superforecasting Agent CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph ..."
---
...
Operate the Teams meeting summary pipeline via Superforecasting Agent CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions.
...
Everything operator-facing is a `superforecasting-agent teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline; the CLI is the surface.
...
Before using the pipeline, verify these are set in `~/.superforecasting-agent/.env`:
...
```bash
superforecasting-agent teams-pipeline validate              # config snapshot — run first after any change
superforecasting-agent teams-pipeline token-health          # Graph token status
superforecasting-agent teams-pipeline token-health --force-refresh   # force a fresh token acquisition
superforecasting-agent teams-pipeline list                  # recent meeting jobs
superforecasting-agent teams-pipeline list --status failed  # only failed jobs
superforecasting-agent teams-pipeline show <job-id>         # full detail of one job
superforecasting-agent teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
superforecasting-agent teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
superforecasting-agent teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
superforecasting-agent teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```
...
superforecasting-agent teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
superforecasting-agent teams-pipeline delete-subscription <sub-id>
superforecasting-agent teams-pipeline maintain-subscriptions            # renew near-expiry ones
superforecasting-agent teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `superforecasting-agent teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `superforecasting-agent cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
