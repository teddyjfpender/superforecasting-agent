...
4. Store the token in `~/.superforecasting-agent/.env` (or via `superforecasting-agent setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for Superforecasting Agent. Pipe through `python3 -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
## Typical Superforecasting Agent Workflow
...
## Important Notes for Superforecasting Agent
...
- **`AIRTABLE_API_KEY` flows from `~/.superforecasting-agent/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
