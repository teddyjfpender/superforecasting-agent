...
- Give it a name like "superforecasting-agent"
- Select scopes:
...
- Give it a title like "superforecasting-agent-&lt;machine-name>"
...
elif [ -f ~/.superforecasting-agent/.env ] && grep -q "^GITHUB_TOKEN=" ~/.superforecasting-agent/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.superforecasting-agent/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
