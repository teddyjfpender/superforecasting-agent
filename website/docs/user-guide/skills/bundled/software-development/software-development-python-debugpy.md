...
| Related skills | [`systematic-debugging`](/user-guide/skills/bundled/software-development/software-development-systematic-debugging), [`node-inspect-debugger`](/user-guide/skills/bundled/software-development/software-development-node-inspect-debugger), [`debugging-superforecasting-tui-commands`](/user-guide/skills/bundled/software-development/software-development-debugging-hermes-tui-commands) |
...
- A long-running process (Superforecasting Agent gateway, tui_gateway) misbehaves and you can't restart it
...
The Superforecasting Agent test runner and pytest both support this:
...
For long-lived processes: Superforecasting Agent gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source /home/bb/superforecasting-agent/.venv/bin/activate
pip install debugpy
```
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Superforecasting Agent you have two practical options:
...
```json
{
  "name": "Attach to Superforecasting Agent",
  "type": "debugpy",
  "request": "attach",
  "connect": { "host": "127.0.0.1", "port": 5678 },
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "/home/bb/superforecasting-agent" }
  ]
}
```
...
## Debugging Superforecasting Agent-specific Processes
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Superforecasting Agent subagents, debug one process at a time.
