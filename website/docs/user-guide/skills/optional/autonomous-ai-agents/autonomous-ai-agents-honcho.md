...
description: "Configure and use Honcho memory with Superforecasting Agent -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reaso..."
---
...
Configure and use Honcho memory with Superforecasting Agent -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `superforecasting-agent skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Superforecasting Agent

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Superforecasting Agent profile its own peer identity while sharing a unified view of the user.

Honcho is auxiliary recall. Durable forecast questions, probabilities, evidence, model runs, resolutions, scores, postmortems, calibration lessons, and domain/topic error profiles belong in the forecast ledger.
...
```bash
superforecasting-agent honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Superforecasting Agent profile's AI peer
...
Honcho models conversations as interactions between **peers**. Superforecasting Agent creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Superforecasting Agent instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Superforecasting Agent run |
| `global` | Single session across all directories |

Manual override: `superforecasting-agent honcho map my-project-name`
...
Each Superforecasting Agent profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
superforecasting-agent profile create coder --clone
...
```bash
superforecasting-agent honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Superforecasting Agent when Honcho memory is active.
...
Run `superforecasting-agent honcho setup`. Ensure `memory.provider: honcho` is in `~/.superforecasting-agent/config.yaml`.

### Memory not persisting across sessions
Check `superforecasting-agent honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `superforecasting-agent profile create <name> --clone`. For existing profiles: `superforecasting-agent honcho sync`.
...
| Command | Description |
|---------|-------------|
| `superforecasting-agent honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `superforecasting-agent honcho status` | Show resolved config, connection test, peer info for active profile |
| `superforecasting-agent honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `superforecasting-agent honcho disable` | Disable Honcho for the active profile |
| `superforecasting-agent honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `superforecasting-agent honcho peers` | Show peer identities across all profiles |
| `superforecasting-agent honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `superforecasting-agent honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `superforecasting-agent honcho sessions` | List known directory-to-session-name mappings |
| `superforecasting-agent honcho map <name>` | Map current working directory to a Honcho session name |
| `superforecasting-agent honcho identity` | Seed AI peer identity or show both peer representations |
| `superforecasting-agent honcho sync` | Create host blocks for all Superforecasting Agent profiles that don't have one yet |
| `superforecasting-agent honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Superforecasting Agent + Honcho |
| `superforecasting-agent memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `superforecasting-agent memory status` | Show active memory provider and config |
| `superforecasting-agent memory off` | Disable external memory provider |
