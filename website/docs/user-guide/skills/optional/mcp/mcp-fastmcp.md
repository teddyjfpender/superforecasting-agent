...
| | |
|---|---|
| Source | Optional — install with `superforecasting-agent skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Superforecasting Agent or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Superforecasting Agent. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.superforecasting-agent/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.superforecasting-agent/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Superforecasting Agent integration, either:

- configure the server in `~/.superforecasting-agent/config.yaml` using the `native-mcp` skill, or
...
### Superforecasting Agent cannot see the deployed server

The server-building part may be correct while the Superforecasting Agent config is not. Load the `native-mcp` skill and configure the server in `~/.superforecasting-agent/config.yaml`, then restart Superforecasting Agent.
