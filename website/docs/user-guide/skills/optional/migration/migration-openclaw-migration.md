---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Superforecasting Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Superforecasting Agent"
---
...
Migrate a user's OpenClaw customization footprint into Superforecasting Agent. Imports Superforecasting Agent-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
...
| Version | `1.0.0` |
| Author | Superforecasting Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Hermes`, `Memory`, `Persona`, `Import` |
| Related skills | [`superforecasting-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hermes-agent) |
...
# OpenClaw -> Superforecasting Agent Migration

Use this skill when a user wants to move their OpenClaw setup into Superforecasting Agent with minimal manual cleanup.
...
```bash
superforecasting-agent claw migrate              # Full interactive migration
...
superforecasting-agent claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `superforecasting-agent setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hermes.py` to:

- import `SOUL.md` into the Superforecasting Agent home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Superforecasting Agent memory entries
- merge OpenClaw command approval patterns into Superforecasting Agent `command_allowlist`
- migrate Superforecasting Agent-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.superforecasting-agent/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Superforecasting Agent workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.superforecasting-agent/tts/`
- archive non-secret docs that do not have a direct Superforecasting Agent destination
...
- `~/.superforecasting-agent/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py`

Do not guess a shorter path like `~/.superforecasting-agent/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.superforecasting-agent/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Superforecasting Agent-compatible secrets, currently:
...
Superforecasting Agent CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Superforecasting Agent. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Superforecasting Agent workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Superforecasting Agent skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.superforecasting-agent/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.superforecasting-agent/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.superforecasting-agent/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.superforecasting-agent/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.superforecasting-agent/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.superforecasting-agent/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Superforecasting Agent unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Superforecasting Agent targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Superforecasting Agent destination. Unsupported auth blobs must still be reported as skipped.
...
- Superforecasting Agent persona state imported
- Superforecasting Agent memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.superforecasting-agent/skills/openclaw-imports/`
