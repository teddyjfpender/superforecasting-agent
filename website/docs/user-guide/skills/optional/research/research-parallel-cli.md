...
| | |
|---|---|
| Source | Optional — install with `superforecasting-agent skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a core Superforecasting Agent capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with inherited Superforecasting Agent `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer inherited Superforecasting Agent `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer inherited Superforecasting Agent native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
- Do not silently choose Parallel when inherited Superforecasting Agent native tools are already sufficient.
