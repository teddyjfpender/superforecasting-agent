...
| | |
|---|---|
| Source | Optional — install with `superforecasting-agent skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Superforecasting Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Superforecasting Agent terminal calls (desktop app flow only)
...
### Service Account (recommended for Superforecasting Agent)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.superforecasting-agent/.env` (the skill will prompt for this on first load).
...
## Superforecasting Agent Execution Pattern (desktop app flow)

Superforecasting Agent terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/superforecasting-agent-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/superforecasting-agent-op.sock"
