...
| | |
|---|---|
| Source | Optional — install with `superforecasting-agent skills install official/productivity/siyuan` |
...
3. Store it in `~/.superforecasting-agent/.env`:
   ```
...
```yaml
# In ~/.superforecasting-agent/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
