...
| | |
|---|---|
| Source | Optional — install with `superforecasting-agent skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Superforecasting Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.superforecasting-agent/.env`:
...
CANVAS="python $AGENT_HOME/skills/productivity/canvas/scripts/canvas_api.py"
