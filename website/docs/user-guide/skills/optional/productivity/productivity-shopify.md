...
| | |
|---|---|
| Source | Optional — install with `superforecasting-agent skills install official/productivity/shopify` |
...
4. Save to `~/.superforecasting-agent/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Superforecasting Agent","productType":"Apparel","tags":["test"]}}'
```
