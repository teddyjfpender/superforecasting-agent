...
bash "$PUBLISH" {file-or-dir} --client superforecasting-agent
```
...
```bash
PUBLISH="${HERMES_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client superforecasting-agent
```
...
| `--client {name}`      | Agent name for attribution (e.g. `superforecasting-agent`) |
