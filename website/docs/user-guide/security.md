...
sidebar_position: 8
title: "Security"
description: "Forecast desk security, command approval, user authorization, container isolation, and production deployment best practices"
---

# Security

Superforecasting Agent is designed with a defense-in-depth security model. This page covers the security boundaries around the forecasting desk: command approval, container isolation, messaging-platform authorization, source access, and credential handling.

## Overview

The security model has seven layers:

1. **User authorization** — who can talk to the forecast runtime (allowlists, DM pairing)
...
Before executing any command, Superforecasting Agent checks it against a curated list of dangerous patterns. If a match is found, the operator must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.superforecasting-agent/config.yaml`. Legacy `~/.hermes/config.yaml` remains readable during the fork transition.
...
YOLO mode is available in both CLI and gateway sessions. Internally, the inherited runtime uses `HERMES_YOLO_MODE`, which is checked before every command execution.

When YOLO is active, Superforecasting Agent shows two persistent visual reminders so it is hard to forget that approval prompts are bypassed:
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Superforecasting Agent refuses to run them **regardless** of:
...
If you hit the blocklist, the tool call returns an explanatory error to the forecast runtime and nothing runs. If a legitimate workflow needs one of these commands, run it outside the agent process.
...
Configure the timeout in `~/.superforecasting-agent/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, agent `.env` files | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, agent `.env` files | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` targeting the agent or gateway | Self-termination prevention |
...
On messaging platforms, the forecast runtime sends the dangerous command details to the chat and waits for the operator to reply:
...
Commands approved with "always" are saved to `~/.superforecasting-agent/config.yaml`:
...
:::tip
Use `superforecasting-agent config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, Superforecasting Agent controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.superforecasting-agent/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.superforecasting-agent/.env to allow open access,
...
For more flexible authorization, Superforecasting Agent includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `superforecasting-agent pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.superforecasting-agent/config.yaml`:
...
superforecasting-agent pairing approve telegram ABC12DEF

# Revoke a user's access
superforecasting-agent pairing revoke telegram 123456789

# Clear all pending codes
superforecasting-agent pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.superforecasting-agent/pairing/` with per-platform JSON files; legacy `~/.hermes/pairing/` remains readable for compatibility.
...
When using the `docker` terminal backend, Superforecasting Agent applies strict security hardening to every container.
...
Container resources are configurable in `~/.superforecasting-agent/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.superforecasting-agent/sandboxes/docker/<task_id>/`
- **Ephemeral mode** (`container_persistent: false`): Uses tmpfs for workspace — everything is lost on cleanup

:::tip
For production gateway deployments, use the `docker`, `modal`, `daytona`, or `vercel_sandbox` backend to isolate forecast-runtime commands from your host system. This eliminates the need for dangerous command approval entirely.
:::
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active agent home. Inherited modules still see this path through `HERMES_HOME`. Skills declare credential files in frontmatter:
...
When loaded, Superforecasting Agent checks if these files exist in the active profile's agent home and registers them for mounting:
...
Paths are relative to `~/.superforecasting-agent/`. Legacy `~/.hermes/` paths remain accepted during the transition. Files are mounted into the container under the active agent-home path.
...
| **terminal** (local) | Blocks explicit agent infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Superforecasting Agent infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
You can restrict which websites the forecast runtime can access through its web and browser tools. This is useful for preventing source-research steps from accessing internal services, admin panels, or other sensitive URLs.
...
security:
  website_blocklist:
    ...
    domains:
      ...
      - "admin.example.com"
    shared_files:
      - "/etc/superforecasting-agent/blocked-sites.txt"
```
...
When on, web tools, the browser, vision URL fetches, and gateway media downloads no longer reject RFC 1918 / loopback / link-local / CGNAT / cloud-metadata destinations. **This is a deliberate trust boundary** — only enable it on machines where forecast-runtime access to arbitrary prompt-injected URLs on the local network is an acceptable risk. Public-facing gateways should leave it off.
...
Superforecasting Agent integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
4. **Store secrets securely** — keep API keys in `~/.superforecasting-agent/.env` with proper file permissions
...
7. **Set `terminal.cwd`** — do not let the gateway operate from sensitive directories
8. **Run as non-root** — never run the gateway as root
9. **Monitor logs** — check `~/.superforecasting-agent/logs/` for unauthorized access attempts
10. **Keep updated** — run `superforecasting-agent update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.superforecasting-agent/.env`:
...
TERMINAL_SSH_USER=forecast
TERMINAL_SSH_KEY=~/.ssh/superforecasting_agent_key
```

The SSH connection details live in `.env` (not `config.yaml`) so they are not checked in or shared along with profile exports. This keeps the gateway's messaging connections separate from forecast-runtime command execution.
...
- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `superforecasting-agent doctor` for the full remediation.
- **`superforecasting-agent doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
```bash
superforecasting-agent doctor --ack <advisory-id>
```
...
Many inherited features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, and others) depend on Python packages that not every forecaster needs. Superforecasting Agent installs these **lazily** on first use rather than eagerly under `superforecasting-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `superforecasting-agent tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install ...`) or pick a different backend via `superforecasting-agent tools`.
