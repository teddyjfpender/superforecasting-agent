...
sidebar_position: 3
title: "Discord"
description: "Set up Superforecasting Agent as a Discord bot"
---

# Discord Setup

Superforecasting Agent integrates with Discord as a forecast-desk bot for review alerts, approval prompts, scheduled digests, voice capture, and server-channel coordination. The bot receives messages, routes them through the forecast-scoped runtime, and responds in real time. It supports text, voice messages, file attachments, and slash commands.

Before setup, here's the part most people want to know: how Superforecasting Agent behaves once it's in your server.

## How Superforecasting Agent Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | Superforecasting Agent responds to every message. No `@mention` needed. Each DM has its own session. |
| **Server channels** | By default, Superforecasting Agent only responds when you `@mention` it. If you post in a channel without mentioning it, Superforecasting Agent ignores the message. |
...
| **Threads** | Superforecasting Agent replies in the same thread. Mention rules still apply unless that thread or its parent channel is configured as free-response. Threads stay isolated from the parent channel for session history. |
| **Shared channels with multiple users** | By default, Superforecasting Agent isolates session history per user inside the channel for safety and clarity. Two people talking in the same channel do not share one transcript unless you explicitly disable that. |
| **Messages mentioning other users** | When `DISCORD_IGNORE_NO_MENTION` is `true` (the default), Superforecasting Agent stays silent if a message @mentions other users but does **not** mention the bot. This prevents the bot from jumping into conversations directed at other people. Set to `false` if you want the bot to respond to all messages regardless of who is mentioned. This only applies in server channels, not DMs. |

:::tip
If you want a normal bot-help channel where people can talk to Superforecasting Agent without tagging it every time, add that channel to `DISCORD_FREE_RESPONSE_CHANNELS`.
:::

### Discord Gateway Model

Superforecasting Agent on Discord is not a webhook that replies statelessly. It runs through the full messaging gateway, which means each incoming message goes through:

1. authorization (`DISCORD_ALLOWED_USERS`)
2. mention / free-response checks
3. session lookup
4. session transcript loading
5. normal Superforecasting Agent agent execution, including tools, memory, and slash commands
6. response delivery back to Discord

That matters because behavior in a busy server depends on both Discord routing and Superforecasting Agent session policy.
...
So if Alice and Bob both talk to Superforecasting Agent in `#research`, Superforecasting Agent treats those as separate conversations by default even though they are using the same visible Discord channel.
...
Superforecasting Agent tracks running agents by session key.
...
3. Enter a name for your application (e.g., "Superforecasting Agent") and accept the Developer Terms of Service.
4. Click **Create**.
...
The bot token is the credential Superforecasting Agent uses to log in as your bot. Still on the **Bot** page:
...
After authorizing, the bot will appear in your server's member list (it will show as offline until you start the Superforecasting Agent gateway).

## Step 7: Find Your Discord User ID

Superforecasting Agent uses your Discord User ID to control who can interact with the bot. To find it:
...
## Step 8: Configure Superforecasting Agent
...
Discord behavior is controlled through two files: **`~/.superforecasting-agent/.env`** for credentials and env-level toggles, and **`~/.superforecasting-agent/config.yaml`** for structured settings. Environment variables always take precedence over config.yaml values when both are set.
...
| `DISCORD_ALLOW_BOTS` | No | `"none"` | Controls how the bot handles messages from other Discord bots. `"none"` — ignore all other bots. `"mentions"` — only accept bot messages that `@mention` Superforecasting Agent. `"all"` — accept all bot messages. |
...
The `discord` section in `~/.superforecasting-agent/config.yaml` mirrors the env vars above. Config.yaml settings are applied as defaults — if the equivalent env var is already set, the env var wins.
...
- If a message arrives inside a thread or forum post and that thread has no explicit entry, Superforecasting Agent falls back to the parent channel/forum ID.
...
When `true`: Alice and Bob talking in `#research` each have their own separate conversation with Superforecasting Agent. When `false`: the entire channel shares one conversation transcript and one running-agent slot.
...
Superforecasting Agent automatically registers installed skills as **native Discord Application Commands**. This means skills appear in Discord's autocomplete `/` menu alongside built-in commands.
...
No extra configuration is needed — any skill installed via `superforecasting-agent skills install` is automatically registered as a Discord slash command on the next gateway restart.

### Disabling Slash Command Registration

If you run multiple Superforecasting Agent gateways against the same Discord application (e.g. staging + production), only one of them should own the global slash-command registration — otherwise the last startup wins and the registrations flap. Turn slash registration off on the "follower" gateway:
...
Discord's per-upload size limit depends on the server's boost tier (25 MB free, up to 500 MB). If Superforecasting Agent gets an HTTP 413, the adapter falls back to a link pointing at the local cache path rather than failing silently.
...
Add these to your `~/.superforecasting-agent/.env`:
...
Superforecasting Agent supports Discord voice messages:
...
**Cause**: The Superforecasting Agent gateway isn't running, or the token is incorrect.

**Fix**: Check that `superforecasting-agent gateway` is running. Verify `DISCORD_BOT_TOKEN` in your `.env` file. If you recently reset the token, update it.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `DISCORD_ALLOWED_USERS`.

**Fix**: Add your User ID to `DISCORD_ALLOWED_USERS` in `~/.superforecasting-agent/.env` and restart the gateway.
...
**Fix**: Set this in `~/.superforecasting-agent/config.yaml` and restart the gateway:
...
```bash
# ~/.superforecasting-agent/.env — works alongside or instead of DISCORD_ALLOWED_USERS
...
```bash
# ~/.superforecasting-agent/.env — env vars win over config.yaml
