...
sidebar_position: 4
title: "Slack"
description: "Set up Superforecasting Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Superforecasting Agent to Slack as a forecast-desk bot for review alerts, approval prompts, scheduled digests, and team-channel coordination. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your instance doesn't need to be publicly accessible; it works behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Superforecasting Agent uses the modern
...
The fastest path is to paste a manifest Superforecasting Agent generates for you. It
...
### Option A: From a Superforecasting Agent-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   superforecasting-agent slack manifest --write
   ```
   This writes `~/.superforecasting-agent/slack-manifest.json` and prints paste-in
   instructions.
...
4. Enter an app name (e.g., "Superforecasting Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Superforecasting Agent can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `forecast-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Superforecasting Agent configuration issue.
:::
...
Superforecasting Agent uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
superforecasting-agent gateway setup    # Select Slack when prompted
```
...
Every Superforecasting Agent command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Superforecasting Agent command with its description.

Under the hood: Superforecasting Agent ships with a generated Slack app manifest (see
...
When Superforecasting Agent adds new commands (e.g. after `superforecasting-agent update`), regenerate
...
1. Open [https://api.slack.com/apps](https://api.slack.com/apps) →
   your Superforecasting Agent app
2. **Features → App Manifest → Edit**
3. Paste the new contents of `~/.superforecasting-agent/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy catch-all command still works

For backward compatibility with older manifests, the inherited catch-all slash command still routes subcommands the same way as first-class slash commands. Free-form text sent through that command is treated as a regular message.
...
Slack never delivers them to Superforecasting Agent.

As a workaround, Superforecasting Agent recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Superforecasting Agent treats it identically to the slash form and replies in the same
thread.

Only the first token is checked against the known command list, so
casual messages like `!nice work` pass through to the runtime unchanged.
...
```bash
superforecasting-agent slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Superforecasting Agent behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Superforecasting Agent what time is it?`). In channels, Superforecasting Agent replies in a thread attached to that message. |
| **Threads** | If you @mention Superforecasting Agent inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.superforecasting-agent/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Superforecasting Agent in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Superforecasting Agent will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Superforecasting Agent will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Superforecasting Agent`).

---

## Multi-Workspace Support

Superforecasting Agent can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Superforecasting Agent also loads tokens from an **OAuth token file** at:

```
~/.superforecasting-agent/slack_tokens.json
```
...
- When a message arrives, Superforecasting Agent uses the correct workspace-specific client to respond.
...
Superforecasting Agent supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Superforecasting Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Superforecasting Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Superforecasting Agent now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Superforecasting Agent`)
...
- Tokens should be stored in `~/.superforecasting-agent/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Superforecasting Agent config directory
