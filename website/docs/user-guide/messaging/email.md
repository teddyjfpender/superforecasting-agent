...
sidebar_position: 7
title: "Email"
description: "Use email for forecast source notes and review alerts."
---

# Email Setup

The forecast gateway can receive and reply to emails using standard IMAP and SMTP protocols. Use a dedicated mailbox for source notes, forecast review requests, scheduled digests, and approval prompts. Works with Gmail, Outlook, Yahoo, Fastmail, or any provider that supports IMAP/SMTP.

Email threads are delivery and capture surfaces. Durable probabilities, source evidence, assumptions, resolutions, postmortems, and calibration lessons should be written to the forecast ledger.
...
- **A dedicated email account** for forecast gateway mail (do not use your personal email)
...
## Step 1: Configure Superforecasting Agent
...
```bash
# Required
EMAIL_ADDRESS=forecast-desk@gmail.com
...
- **Subject line** is included as context (e.g., `[Subject: Q142 source update]`)
...
- **Message-ID** generated with the gateway domain
- Responses are sent as plain text (UTF-8)

### File Attachments

The forecast runtime can send file attachments in replies. Include `MEDIA:/path/to/file` in the response and the file is attached to the outgoing email.
...
Email access follows the same pattern as other forecast gateway platforms:
...
:::warning
**Always configure `EMAIL_ALLOWED_USERS`.** Without it, anyone who knows the mailbox address could send commands. The runtime may have terminal and source-access tools enabled.
:::
...
| **Duplicate replies** | Ensure only one gateway instance is running. Check `superforecasting-agent gateway status`. |
...
:::warning
**Use a dedicated email account.** Do not use your personal email — the gateway stores the password in `.env` and has full inbox access via IMAP.
:::

- Use **App Passwords** instead of your main password (required for Gmail with 2FA)
- Set `EMAIL_ALLOWED_USERS` to restrict who can interact with the gateway
- The password is stored in `~/.superforecasting-agent/.env` — protect this file (`chmod 600`)
...
|----------|----------|---------|-------------|
| `EMAIL_ADDRESS` | Yes | — | Gateway email address |
