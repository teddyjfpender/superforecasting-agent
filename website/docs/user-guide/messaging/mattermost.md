...
sidebar_position: 8
title: "Mattermost"
description: "Send forecast-desk alerts and approvals through Mattermost"
---

# Mattermost Setup

Superforecasting Agent integrates with Mattermost as a secondary forecast-desk bot for review alerts, approvals, scheduled digests, evidence delivery, and operator coordination. Mattermost is a self-hosted, open-source Slack alternative, so it is useful when forecast discussions and alerts need to stay on your own infrastructure. The bot connects via Mattermost's REST API (v4) and WebSocket for real-time events, processes messages through the Superforecasting Agent runtime, and responds in real time. It supports text, file attachments, images, and slash commands.

No external Mattermost library is required — the adapter uses `aiohttp`, which is already a Superforecasting Agent dependency.

Before setup, here's the part most people want to know: how Superforecasting Agent behaves once it's in your Mattermost instance.
...
| **Public/private channels** | Superforecasting Agent responds when you `@mention` it. Without a mention, Superforecasting Agent ignores the message. |
| **Threads** | If `MATTERMOST_REPLY_MODE=thread`, Superforecasting Agent replies in a thread under your message. Thread context stays isolated from the parent channel. |
| **Shared channels with multiple users** | By default, Superforecasting Agent isolates session history per user inside the channel. Two people talking in the same channel do not share one transcript unless you explicitly disable that. |

Mattermost sessions are not the forecast ledger. Durable probabilities, evidence records, assumptions, resolutions, postmortems, and calibration lessons should still be written through the ledger before they are treated as forecast state.

:::tip
If you want Superforecasting Agent to reply as threaded conversations (nested under your original message), set `MATTERMOST_REPLY_MODE=thread`. The default is `off`, which sends flat messages in the channel.
:::
...
3. Fill in the details:
   - **Username**: e.g., `superforecasting-agent`
   - **Display Name**: e.g., `Superforecasting Agent`
...
:::tip
You can also use a **personal access token** instead of a bot account. Go to **Profile** → **Security** → **Personal Access Tokens** → **Create Token**. This is useful if you want Superforecasting Agent to post as your own user rather than a separate bot user.
:::
...
3. Search for your bot username (e.g., `superforecasting-agent`) and add it.
...
Superforecasting Agent uses your Mattermost User ID to control who can interact with the bot. To find it:
...
You can designate a "home channel" where the bot sends proactive messages such as scheduled review output, watched-source alerts, backtest digests, and notifications. There are two ways to set it:
...
The `MATTERMOST_REPLY_MODE` setting controls how Superforecasting Agent posts responses:

| Mode | Behavior |
|------|----------|
| `off` (default) | Superforecasting Agent posts flat messages in the channel, like a normal user. |
| `thread` | Superforecasting Agent replies in a thread under your original message. Keeps channels clean when there's lots of back-and-forth. |

Set it in your `~/.superforecasting-agent/.env`:
...
**Fix**: Check that `superforecasting-agent gateway` is running. Look at the terminal output for error messages. Common issues: wrong URL, expired token, Mattermost server unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATTERMOST_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATTERMOST_ALLOWED_USERS` in `~/.superforecasting-agent/.env` and restart the gateway. Remember: the User ID is a 26-character alphanumeric string, not your `@username`.
...
mattermost:
  channel_prompts:
    "channel_id_abc123": |
      You are a forecast analyst. Focus on evidence,
      source quality, and uncertainty.
    "channel_id_def456": |
      Release-risk review. Be precise about evidence,
      assumptions, and resolution criteria.
```
...
:::warning
Always set `MATTERMOST_ALLOWED_USERS` to restrict who can interact with the bot. Without it, the gateway denies all users by default as a safety measure. Only add User IDs of people you trust — authorized users have full access to runtime capabilities, including tool use and system access.
:::
...
- **No extra dependencies**: The adapter uses `aiohttp` for HTTP and WebSocket, which is already included with Superforecasting Agent.
