...
sidebar_position: 11
title: "Feishu / Lark"
description: "Send forecast-desk alerts and approvals through Feishu or Lark"
---

# Feishu / Lark Setup

Superforecasting Agent integrates with Feishu and Lark as a secondary forecast-desk bot for review alerts, approvals, scheduled digests, document evidence review, file delivery, and operator coordination. The CLI and forecast ledger remain the primary product surface; Feishu/Lark sessions are delivery and discussion channels.

The integration supports both connection modes:

- `websocket` — recommended; Superforecasting Agent opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | Superforecasting Agent responds to every message. |
| Group chats | Superforecasting Agent responds only when the bot is @mentioned in the chat. |
| Shared group chats | By default, session history is isolated per user inside a shared chat. |

Forecast probabilities, evidence records, assumptions, postmortems, and calibration lessons should be written through the forecast ledger before they are treated as durable state.
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. Superforecasting Agent will automatically create a bot application with the correct permissions and save the credentials.
...
5. Run `superforecasting-agent gateway setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
Use WebSocket mode when Superforecasting Agent runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run Superforecasting Agent behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, Superforecasting Agent starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
Use `/set-home` in a Feishu/Lark chat to mark it as the home channel for scheduled forecast reviews, watched-source alerts, and cross-platform notifications.
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how Superforecasting Agent responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | Superforecasting Agent responds to @mentions from any user in any group. |
| `allowlist` | Superforecasting Agent only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | Superforecasting Agent ignores all group messages entirely. |
...
Set `FEISHU_REQUIRE_MENTION=false` to let Superforecasting Agent read all group traffic without requiring an @mention:
...
Superforecasting Agent auto-detects the bot's `open_id` and display name on startup. You only need to set these manually when auto-detection cannot reach the Feishu API, or when your app uses tenant-scoped user IDs:
...
By default Superforecasting Agent ignores messages sent by other bots. Enable bot-to-bot messaging when you want Superforecasting Agent to participate in A2A orchestration or receive notifications from other bots in the same group.
...
| `mentions` | Accept only when the peer bot @mentions Superforecasting Agent. |
...
Gateway-driven update prompts use a native Feishu `Yes` / `No` card instead of falling back to plain text replies. When `superforecasting-agent update --gateway` needs confirmation, the adapter records the selected answer in Superforecasting Agent's `.update_response` file and replaces the card inline with a resolved state.
...
Beyond chat, the adapter can also answer `@`-mentions left on **Feishu/Lark documents**. When a user comments on a document (local text selection or whole-doc comment) and @-mentions the bot, Superforecasting Agent reads the document plus the surrounding comment thread and posts a forecast-review reply inline on the thread.
...
- Runs the forecast runtime with the `feishu_doc` + `feishu_drive` toolsets scoped to that single comment session.
...
Rules live in `~/.superforecasting-agent/feishu_comment_rules.json` (pairing grants in `~/.superforecasting-agent/feishu_comment_pairing.json`) with mtime-cached hot-reload — edits take effect on the next comment event without restarting the gateway.
...
For small text-based documents (.txt, .md), the file content is automatically injected into the message text so the forecast runtime can read it directly without needing tools.
...
While the forecast runtime is working, the bot shows a `Typing` reaction on your message. It's cleared when the reply arrives, or replaced with `CrossMark` if processing failed.

Set `FEISHU_REACTIONS=false` to turn it off.

## Burst Protection and Batching

The adapter includes debouncing for rapid message bursts to avoid overwhelming the forecast runtime:

### Text Batching

When a user sends multiple text messages in quick succession, they are merged into a single event before being dispatched:

- Quiet period: 0.6 seconds
- Max messages per batch: 8
- Max characters per batch: 4000

### Media Batching

Multiple media attachments sent in quick succession (e.g., dragging several images) are merged into a single event:

- Quiet period: 0.8 seconds
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.superforecasting-agent/feishu_seen_message_ids.json`.

The default dedup cache tracks 2048 message IDs.
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `superforecasting-agent gateway setup` |
| `Another local Superforecasting Agent gateway is already using this Feishu app_id` | Only one Superforecasting Agent instance can use the same app_id at a time. Stop the other gateway first. |
...
| Peer bot messages still ignored after enabling `FEISHU_ALLOW_BOTS` | Superforecasting Agent can't identify itself yet — set `FEISHU_BOT_OPEN_ID` (and `FEISHU_BOT_USER_ID` if your app uses `sender_id_type=user_id`). |
...
Feishu / Lark uses the Feishu gateway platform preset, which exposes the messaging-safe core tool bundle used by other gateway-based platforms.
