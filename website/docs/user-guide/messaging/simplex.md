...
Use SimpleX for private forecast review alerts, approvals, source notes, and scheduled digests. SimpleX conversations are not durable forecast memory; ledger records remain the source of truth for probabilities, evidence, assumptions, resolutions, postmortems, and calibration lessons.
...
Add these to `~/.superforecasting-agent/.env`:
...
| `SIMPLEX_ALLOWED_USERS` | Recommended | Comma-separated contact IDs allowed to use the forecast runtime |
| `SIMPLEX_ALLOW_ALL_USERS` | Optional | Set `true` to allow every contact (use carefully) |
| `SIMPLEX_HOME_CHANNEL` | Optional | Default contact ID for scheduled review delivery |
| `SIMPLEX_HOME_CHANNEL_NAME` | Optional | Human label for the home channel |

## Find your contact ID

After starting the daemon, open a conversation with your forecast gateway contact. The contact ID will appear in session logs or via `superforecasting-agent send_message action=list`.
...
2. Use **DM pairing** — send any message to the bot and it will reply with a pairing code. Enter that code via `superforecasting-agent gateway pair`.

## Using SimpleX with scheduled reviews
...
cronjob(
    ...
    prompt="Check stale forecasts and new source alerts."
)
```
...
- The connection between Superforecasting Agent and the daemon is local WebSocket (`ws://127.0.0.1:5225`) — no data leaves your machine
