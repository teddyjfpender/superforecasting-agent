...
sidebar_position: 10
title: "DingTalk"
description: "Send forecast-desk alerts and approvals through DingTalk"
---

# DingTalk Setup

Superforecasting Agent integrates with DingTalk as a secondary forecast-desk bot for review alerts, approvals, scheduled digests, evidence delivery, and operator coordination. The bot connects via DingTalk's Stream Mode — a long-lived WebSocket connection that requires no public URL or webhook server — and replies using markdown-formatted messages through DingTalk's session webhook API.

Before setup, here's the part most people want to know: how Superforecasting Agent behaves once it's in your DingTalk workspace.

## How Superforecasting Agent Behaves

| Context | Behavior |
|---------|----------|
| **DMs (1:1 chat)** | Superforecasting Agent responds to every message. No `@mention` needed. Each DM has its own session. |
| **Group chats** | Superforecasting Agent responds when you `@mention` it. Without a mention, Superforecasting Agent ignores the message. |
| **Shared groups with multiple users** | By default, Superforecasting Agent isolates session history per user inside the group. Two people talking in the same group do not share one transcript unless you explicitly disable that. |

DingTalk sessions are conversational context only. Forecast probabilities, evidence records, assumptions, resolutions, postmortems, and calibration lessons should still be written through the forecast ledger before they are treated as durable.
...
```bash
pip install "superforecasting-agent[dingtalk]"
```
...
4. Fill in:
   - **App Name**: e.g., `Superforecasting Agent`
...
Superforecasting Agent uses your DingTalk User ID to control who can interact with the bot. DingTalk User IDs are alphanumeric strings set by your organization's admin.
...
- **QR-code device flow (recommended).** Scan the QR that prints in your terminal with the DingTalk mobile app — your Client ID and Client Secret are returned automatically and written to `~/.superforecasting-agent/.env`. No developer-console trip needed.
...
Superforecasting Agent can reply using DingTalk AI Cards instead of plain markdown messages. Cards provide a richer, more structured display and support streaming updates as the forecast runtime generates its response.
...
Superforecasting Agent automatically adds emoji reactions to your messages to show processing status:
...
**Fix**: Verify `DINGTALK_CLIENT_ID` and `DINGTALK_CLIENT_SECRET` are set correctly in `~/.superforecasting-agent/.env`. The Client ID is your AppKey, and the Client Secret is your AppSecret from the DingTalk Developer Console.
...
**Fix**: Check that `superforecasting-agent gateway` is running. Look at the terminal output for error messages. Common issues: wrong credentials, app deactivated, `dingtalk-stream` or `httpx` not installed.
...
:::warning
Always set `DINGTALK_ALLOWED_USERS` to restrict who can interact with the bot. Without it, the gateway denies all users by default as a safety measure. Only add User IDs of people you trust — authorized users have full access to runtime capabilities, including tool use and system access.
:::
