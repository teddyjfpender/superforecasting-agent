...
sidebar_position: 9
title: "Matrix"
description: "Set up Superforecasting Agent as a Matrix bot"
---

# Matrix Setup

Superforecasting Agent integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org, so forecast-desk alerts and review conversations can live in infrastructure you control. The bot connects via the `mautrix` Python SDK, routes messages through the forecast-desk runtime, and supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

Superforecasting Agent works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how Superforecasting Agent behaves once it's connected.

## How Superforecasting Agent Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | Superforecasting Agent responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, Superforecasting Agent requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | Superforecasting Agent supports Matrix threads (MSC3440). If you reply in a thread, Superforecasting Agent keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, Superforecasting Agent auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. |
| **Shared rooms with multiple users** | By default, Superforecasting Agent isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
2. Choose a username like `superforecasting-agent` — the full user ID will be `@superforecasting-desk:your-server.org`.

### Option B: Use matrix.org or Another Public Homeserver

1. Go to [Element Web](https://app.element.io) and create a new account.
2. Pick a username for your bot (e.g., `superforecasting-agent-bot`).

### Option C: Use Your Own Account

You can also run Superforecasting Agent as your own user. This means the bot posts as you, which can be useful for a personal forecast desk.

## Step 2: Get an Access Token

Superforecasting Agent needs an access token to authenticate with the homeserver. You have two options:
...
curl -X POST https://your-server/_matrix/client/v3/login \
  ...
  -d '{
    ...
    "user": "@superforecasting-desk:your-server.org",
...
Instead of providing an access token, you can give Superforecasting Agent the bot's user ID and password. Superforecasting Agent will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.

```bash
MATRIX_USER_ID=@superforecasting-desk:your-server.org
MATRIX_PASSWORD=your-password
```

## Step 3: Find Your Matrix User ID

Superforecasting Agent uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
## Step 4: Configure Superforecasting Agent
...
# MATRIX_USER_ID=@superforecasting-desk:matrix.example.org
...
:::tip
You can run `superforecasting-agent gateway` in the background or as a systemd service for persistent operation. See the deployment docs for details.
:::

## End-to-End Encryption (E2EE)

Superforecasting Agent supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
# Or install with superforecasting-agent extras
pip install 'superforecasting-agent[matrix]'
```
...
Add to your `~/.superforecasting-agent/.env`:

```bash
MATRIX_ENCRYPTION=true
```

When E2EE is enabled, Superforecasting Agent:

- Stores encryption keys in `~/.superforecasting-agent/platforms/matrix/store/` (legacy installs: `~/.superforecasting-agent/matrix/store/`)
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, Superforecasting Agent imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.

:::warning[Deleting the crypto store]
If you delete `~/.superforecasting-agent/platforms/matrix/store/crypto.db`, the bot loses its encryption identity. Simply restarting with the same device ID will **not** fully recover — the homeserver still holds one-time keys signed with the old identity key, and peers cannot establish new Olm sessions.

Superforecasting Agent detects this condition on startup and refuses to enable E2EE, logging: `device XXXX has stale one-time keys on the server signed with a previous identity key`.
...
1. Stop Synapse and delete the old device from its database:
   ...
   sudo sqlite3 /var/lib/matrix-synapse/homeserver.db "
     DELETE FROM e2e_device_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@superforecasting-desk:your-server';
     DELETE FROM e2e_one_time_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@superforecasting-desk:your-server';
     DELETE FROM e2e_fallback_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@superforecasting-desk:your-server';
     DELETE FROM devices WHERE device_id = 'DEVICE_ID' AND user_id = '@superforecasting-desk:your-server';
   "
...
2. Delete the local crypto store and restart Superforecasting Agent:
   ```bash
   rm -f ~/.superforecasting-agent/platforms/matrix/store/crypto.db*
   # restart superforecasting-agent
   ```
...
Add this to your `~/.superforecasting-agent/.env`:
...
**Cause**: The host's system clock is set ahead of real time. The Matrix adapter applies a 5-second startup-grace filter (`event_ts < startup_ts - 5`) to ignore events replayed from initial sync. When the wall clock is ahead, every incoming event looks "older than startup" and is dropped before reaching the message handler, so the bot appears connected but never replies. See upstream issue `#12614`.
...
Or with Superforecasting Agent extras:
...
If you previously used Superforecasting Agent with `MATRIX_ENCRYPTION=true` and are upgrading to
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       "type": "m.login.password",
       "identifier": {"type": "m.id.user", "user": "@superforecasting-desk:your-server.org"},
       "password": "***",
       "initial_device_display_name": "Superforecasting Agent"
     }'
   ```

   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in `~/.superforecasting-agent/.env`.

2. **Delete old encryption state**:

   ```bash
   rm -f ~/.superforecasting-agent/platforms/matrix/store/crypto.db
   rm -f ~/.superforecasting-agent/platforms/matrix/store/crypto_store.*
   ```

3. **Set your recovery key** (if you use cross-signing — most Element users do). Add to `~/.superforecasting-agent/.env`:
...
a working E2EE setup with a previous version of Superforecasting Agent and are upgrading.
...
Matrix E2EE requires `libolm`, which doesn't compile on macOS ARM64 (Apple Silicon). The `superforecasting-agent[matrix]` extra is gated to Linux only. If you're on macOS, proxy mode lets you run E2EE in a Docker container on a Linux VM while the actual agent runs natively on macOS with full access to your local files, memory, and skills.
...
Linux VM (Docker):
  └─ superforecasting-agent gateway (proxy mode)
...
services:
  superforecasting-agent-matrix:
    ...
    volumes:
      - ./matrix-store:/root/.superforecasting-agent/platforms/matrix/store
```
...
RUN pip install 'superforecasting-agent[matrix]'

CMD ["superforecasting-agent", "gateway"]
```
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote Superforecasting Agent API server (e.g., `http://192.168.1.100:8642`) |
...
:::tip
Session continuity is maintained via the API server's compatibility session-continuity header. The host's API server tracks sessions by this ID, so conversations persist across messages just like they would with a local agent.
:::
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the Superforecasting Agent logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The Superforecasting Agent gateway isn't running, or it failed to connect.

**Fix**: Check that `superforecasting-agent gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.superforecasting-agent/.env` and restart the gateway. Use the full `@user:server` format.
...
For more information on securing your Superforecasting Agent deployment, see the [Security Guide](../security.md).
...
- **Media support**: Superforecasting Agent can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
