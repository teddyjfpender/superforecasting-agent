...
description: "Install and deploy Superforecasting Agent with Nix — from quick `nix run` to the inherited NixOS module with container mode"
---
...
**For non-NixOS users**, this only changes the install step. Everything after (`superforecasting-agent setup`, `superforecasting-agent gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage the forecast desk the same way you manage any other NixOS service.
:::
...
After `nix profile install`, `forecast`, `superforecast`, `superforecasting-agent`, `hermes`, `hermes-agent`, and `hermes-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md): `superforecasting-agent setup` walks you through provider selection, `superforecasting-agent gateway install` sets up a launchd (macOS) or systemd user service, and new config lives in `~/.superforecasting-agent/` with legacy `~/.hermes/` homes still readable.
...
Setting `addToSystemPackages = true` does two things: puts the `forecast`, `superforecasting-agent`, and inherited compatibility CLIs on your system PATH **and** sets `SUPERFORECASTING_AGENT_HOME`, `FORECAST_HOME`, and legacy `HERMES_HOME` system-wide so the interactive CLI shares state with the gateway service. Without it, a shell command can create a separate user home.
:::
...
superforecasting-agent config       # shows the generated config
```
...
{
  sops = {
    defaultSopsFile = ./secrets/forecast.yaml;
...
```yaml
# secrets/forecast.yaml (encrypted with sops)
forecast-env: |
...
The `documents` option installs files into the agent's working directory (the `workingDirectory`, which the agent reads as its workspace). Superforecasting Agent looks for specific filenames by convention:
...
:::tip
Environment variables in `env` values are resolved from `$SUPERFORECASTING_AGENT_HOME/.env` at runtime. Use `environmentFiles` to inject secrets — never put tokens directly in Nix config.
:::
...
Set `auth = "oauth"` for servers using OAuth 2.1. Superforecasting Agent implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.
...
Tokens are stored in `$SUPERFORECASTING_AGENT_HOME/mcp-tokens/<server-name>.json` and persist across restarts and rebuilds.
...
When Superforecasting Agent runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `superforecasting-agent setup` | Config is declarative — edit `settings` in your Nix config |
| `superforecasting-agent config edit` | Config is generated from `settings` |
| `superforecasting-agent config set <key> <value>` | Config is generated from `settings` |
| `superforecasting-agent gateway install` | The systemd service is managed by NixOS |
| `superforecasting-agent gateway uninstall` | The systemd service is managed by NixOS |
...
When container mode is enabled, Superforecasting Agent runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:
...
If the agent relies on specific packages, consider baking them into a custom image (`container.image = "my-registry/superforecasting-agent-base:latest"`) or scripting their installation in the agent's SOUL.md.
:::

### GC Root Protection

The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `superforecasting-agent plugins install` needed.
...
Plugins are symlinked into `$SUPERFORECASTING_AGENT_HOME/plugins/` at activation time. Superforecasting Agent discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.
...
The package's `site-packages` is added to PYTHONPATH in the Nix wrapper. `importlib.metadata` discovers the entry point at session start.
...
:::note
A build-time collision check prevents plugin packages from shadowing core runtime dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
| Option | Type | Default | Description |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | Paths to env files with secrets. Merged into `$SUPERFORECASTING_AGENT_HOME/.env` at activation time |
...
cd /etc/nixos && nix flake update superforecasting-agent
