...
| `HERMES_HOME` | Legacy alias for the agent home directory. Prefer `SUPERFORECASTING_AGENT_HOME` or `FORECAST_HOME` for new installs. Also scopes the gateway PID file and service name, so multiple installations can run concurrently. |
