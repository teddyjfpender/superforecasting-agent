...
description: "Complete reference of environment variables used by Superforecasting Agent"
---

# Environment Variables Reference

Secrets go in `~/.superforecasting-agent/.env` by default. Non-secret settings usually belong in `~/.superforecasting-agent/config.yaml`; `superforecasting-agent config set KEY value` chooses the right file for you. Existing `~/.hermes` homes and `HERMES_*` variable names remain supported as compatibility aliases while the fork transitions to forecast-native paths.

## Runtime Home And Forecast Ledger
...
Prefer `SUPERFORECASTING_AGENT_HOME` or `FORECAST_HOME` for new deployments. Use `HERMES_HOME` only when compatibility with inherited scripts, services, or existing profiles matters.
...
| `MINIMAX_BASE_URL` | Override MiniMax base URL (default: `https://api.minimax.io/anthropic` — Superforecasting Agent uses MiniMax's Anthropic Messages-compatible endpoint). **Not used by `minimax-oauth`**. |
...
| `SUPERFORECASTING_AGENT_GEMINI_CLIENT_ID` / `FORECAST_GEMINI_CLIENT_ID` / `HERMES_GEMINI_CLIENT_ID` | OAuth client ID for `google-gemini-cli` PKCE login (optional; defaults to Google's public gemini-cli client); the `HERMES_*` name remains a legacy alias |
| `SUPERFORECASTING_AGENT_GEMINI_CLIENT_SECRET` / `FORECAST_GEMINI_CLIENT_SECRET` / `HERMES_GEMINI_CLIENT_SECRET` | OAuth client secret for `google-gemini-cli` (optional); the `HERMES_*` name remains a legacy alias |
| `SUPERFORECASTING_AGENT_GEMINI_PROJECT_ID` / `FORECAST_GEMINI_PROJECT_ID` / `HERMES_GEMINI_PROJECT_ID` | GCP project ID for paid Gemini tiers (free tier auto-provisions); the `HERMES_*` name remains a legacy alias |
...
| `SUPERFORECASTING_AGENT_LOCAL_STT_COMMAND` / `FORECAST_LOCAL_STT_COMMAND` / `HERMES_LOCAL_STT_COMMAND` | Optional local speech-to-text command template. Supports `{input_path}`, `{output_dir}`, `{language}`, and `{model}` placeholders. The `HERMES_*` name remains a legacy alias |
| `SUPERFORECASTING_AGENT_LOCAL_STT_LANGUAGE` / `FORECAST_LOCAL_STT_LANGUAGE` / `HERMES_LOCAL_STT_LANGUAGE` | Default language passed to a local STT command or auto-detected local `whisper` CLI fallback (default: `en`). The `HERMES_*` name remains a legacy alias |
| `HERMES_HOME` | Legacy alias for the agent home directory. Prefer `SUPERFORECASTING_AGENT_HOME` or `FORECAST_HOME` for new installs. Gateway service wrappers export all three names so inherited modules and fork-native integrations resolve the same profile-scoped state. |
| `SUPERFORECASTING_AGENT_MANAGED` / `FORECAST_MANAGED` / `HERMES_MANAGED` | Marks package-manager-owned installs such as NixOS or Homebrew so mutation commands print the package-manager update path instead of editing generated files. Values such as `true`, `nix`, or `nixos` mean NixOS; `homebrew` or `brew` means Homebrew. Prefer the fork-native aliases; `HERMES_MANAGED` remains a legacy alias. |
| `SUPERFORECASTING_AGENT_GIT_BASH_PATH` / `FORECAST_GIT_BASH_PATH` / `HERMES_GIT_BASH_PATH` | **Windows only.** Override `bash.exe` discovery for the terminal tool. Points at any bash — full Git-for-Windows install, WSL bash via symlink, MSYS2, Cygwin. The installer sets this automatically to the PortableGit it provisioned. See the [Windows (Native) Guide](../user-guide/windows-native.md). |
| `SUPERFORECASTING_AGENT_DISABLE_WINDOWS_UTF8` / `FORECAST_DISABLE_WINDOWS_UTF8` / `HERMES_DISABLE_WINDOWS_UTF8` | **Windows only.** Set to `1` to disable the UTF-8 stdio shim (`configure_windows_stdio()`) and fall back to the console's locale code page. Useful for bisecting encoding bugs; rarely the right setting in normal operation |
| `SUPERFORECASTING_AGENT_KANBAN_HOME` / `FORECAST_KANBAN_HOME` / `HERMES_KANBAN_HOME` | Override the shared agent root that anchors the kanban board (db + workspaces + worker logs). Falls back to `get_default_hermes_root()` (the parent of any active profile). The `HERMES_*` name remains a legacy alias. |
| `SUPERFORECASTING_AGENT_KANBAN_BOARD` / `FORECAST_KANBAN_BOARD` / `HERMES_KANBAN_BOARD` | Pin the active kanban board for this process. Takes precedence over `<agent-home>/kanban/current`; the dispatcher injects all aliases into worker subprocess env so workers physically cannot see tasks on other boards. Defaults to `default`. Slug validation: lowercase alphanumerics + hyphens + underscores, 1-64 chars. |
| `SUPERFORECASTING_AGENT_KANBAN_DB` / `FORECAST_KANBAN_DB` / `HERMES_KANBAN_DB` | Pin the kanban database file path directly (highest precedence; beats the board and home aliases). The dispatcher injects all aliases into worker subprocess env so profile workers converge on the dispatcher's board. |
| `SUPERFORECASTING_AGENT_KANBAN_WORKSPACES_ROOT` / `FORECAST_KANBAN_WORKSPACES_ROOT` / `HERMES_KANBAN_WORKSPACES_ROOT` | Pin the kanban workspaces root directly (highest precedence for workspaces; beats the home aliases). The dispatcher injects all aliases into worker subprocess env. |
| `SUPERFORECASTING_AGENT_KANBAN_DISPATCH_IN_GATEWAY` / `FORECAST_KANBAN_DISPATCH_IN_GATEWAY` / `HERMES_KANBAN_DISPATCH_IN_GATEWAY` | Runtime override for `kanban.dispatch_in_gateway`. Set to `0`, `false`, `no`, or `off` to keep the gateway from starting the embedded Kanban dispatcher; any other non-empty value enables it. Useful when a separate dispatcher process owns the board. |
| `SUPERFORECASTING_AGENT_KANBAN_CLAIM_TTL_SECONDS` / `FORECAST_KANBAN_CLAIM_TTL_SECONDS` / `HERMES_KANBAN_CLAIM_TTL_SECONDS` | Default claim TTL for running kanban tasks, in seconds. Positive integers override the built-in 15-minute window; invalid or non-positive values fall back silently. |

## Provider Auth (OAuth)

For native Anthropic auth, Superforecasting Agent prefers Claude Code's own credential files when they exist because those credentials can refresh automatically. **OAuth against Anthropic requires a Claude Max plan with purchased extra usage credits** — the runtime routes as Claude Code, which only draws from the Max plan's extra/overage credits, not the base Max allowance, and does not work on Claude Pro. Without Max + extra credits, use an API key instead. Environment variables such as `ANTHROPIC_TOKEN` remain useful as manual overrides, but they are no longer the preferred path for Claude Max login.
...
| `CENSUS_API_KEY` | U.S. Census API key for `forecast import census ...` evidence imports and `census:` watched sources ([api.census.gov/data/key_signup.html](https://api.census.gov/data/key_signup.html)) |
...
| `SUPERFORECASTING_AGENT_DOCKER_BINARY` / `FORECAST_DOCKER_BINARY` / `HERMES_DOCKER_BINARY` | Override for the container binary the agent shells out to (e.g. `podman`, `/usr/local/bin/docker`). When unset, the runtime auto-discovers `docker` or `podman` on `PATH`. Needed when both are installed and you want the non-default, or when the binary lives outside `PATH`; the `HERMES_*` name remains a legacy alias. |
...
| `TERMINAL_CWD` | Working directory for terminal sessions after config bridging (gateway/cron only; CLI uses launch dir). Prefer `terminal.cwd` in `config.yaml`; setting this directly in `.env` is deprecated. |
| `SUDO_PASSWORD` | Enable sudo without interactive prompt |

For cloud sandbox backends, persistence is filesystem-oriented. `TERMINAL_LIFETIME_SECONDS` controls when Superforecasting Agent cleans up an idle terminal session, and later resumes may recreate the sandbox rather than keep the same live processes running.
...
| `TERMINAL_SANDBOX_DIR` | Host directory for workspaces and overlays (default: `<agent-home>/sandboxes/`) |
...
| `MATRIX_USER_ID` | Matrix user ID (e.g. `@forecast:matrix.org`) — required for password login, optional with access token |
...
| `MSGRAPH_CLIENT_SECRET` | Client secret value for the app registration. Store in the active agent `.env` with `chmod 600`; rotate periodically via the Azure portal. |
...
| `SUPERFORECASTING_AGENT_VISION_DOWNLOAD_TIMEOUT` / `FORECAST_VISION_DOWNLOAD_TIMEOUT` / `HERMES_VISION_DOWNLOAD_TIMEOUT` | Timeout in seconds for downloading an image before handing it to vision models (default: `30`); the `HERMES_*` name remains a legacy alias. |
| `SUPERFORECASTING_AGENT_RESTART_DRAIN_TIMEOUT` / `FORECAST_RESTART_DRAIN_TIMEOUT` / `HERMES_RESTART_DRAIN_TIMEOUT` | Gateway: seconds to wait for active runs to drain on `/restart` before forcing the restart (default: `900`); the `HERMES_*` name remains a legacy alias. |
| `SUPERFORECASTING_AGENT_GATEWAY_PLATFORM_CONNECT_TIMEOUT` / `FORECAST_GATEWAY_PLATFORM_CONNECT_TIMEOUT` / `HERMES_GATEWAY_PLATFORM_CONNECT_TIMEOUT` | Per-platform connect timeout during gateway startup (seconds); the `HERMES_*` name remains a legacy alias. |
| `SUPERFORECASTING_AGENT_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT` / `FORECAST_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT` / `HERMES_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT` | Per-adapter disconnect timeout during gateway shutdown (seconds); the `HERMES_*` name remains a legacy alias. |
| `SUPERFORECASTING_AGENT_AUTO_CONTINUE_FRESHNESS` / `FORECAST_AUTO_CONTINUE_FRESHNESS` / `HERMES_AUTO_CONTINUE_FRESHNESS` | Freshness window for gateway auto-continue handoffs after restart/resume (seconds); the `HERMES_*` name remains a legacy alias. |
| `SUPERFORECASTING_AGENT_GATEWAY_LOCK_DIR` / `FORECAST_GATEWAY_LOCK_DIR` / `HERMES_GATEWAY_LOCK_DIR` | Machine-local directory for token-scoped gateway lock files. Defaults under `$XDG_STATE_HOME/superforecasting-agent/gateway-locks`; the `HERMES_*` name remains a legacy override. |
| `SUPERFORECASTING_AGENT_GATEWAY_BUSY_INPUT_MODE` / `FORECAST_GATEWAY_BUSY_INPUT_MODE` / `HERMES_GATEWAY_BUSY_INPUT_MODE` | Default gateway busy-input behavior: `queue`, `steer`, or `interrupt`. Can be overridden per chat with `/busy`; the `HERMES_*` name remains a legacy alias. |
| `SUPERFORECASTING_AGENT_GATEWAY_BUSY_ACK_ENABLED` / `FORECAST_GATEWAY_BUSY_ACK_ENABLED` / `HERMES_GATEWAY_BUSY_ACK_ENABLED` | Whether the gateway sends an acknowledgment message when a user sends input while the agent is busy (default: `true`). Set to `false` to suppress these messages entirely — the input is still queued/steered/interrupts as normal, only the chat reply is silenced. Bridged from `display.busy_ack_enabled` in `config.yaml`; the `HERMES_*` name remains a legacy alias. |
| `SUPERFORECASTING_AGENT_FILE_MUTATION_VERIFIER` / `FORECAST_FILE_MUTATION_VERIFIER` / `HERMES_FILE_MUTATION_VERIFIER` | Enable the per-turn file-mutation verifier footer (default: `true`). When enabled, the runtime appends an advisory listing any `write_file` / `patch` calls that failed during the turn and were not superseded by a successful write. Set to `0`, `false`, `no`, or `off` to suppress. Mirrors `display.file_mutation_verifier` in `config.yaml`; the env var wins when set. The `HERMES_*` name remains a legacy alias. |
| `SUPERFORECASTING_AGENT_CRON_TIMEOUT` / `FORECAST_CRON_TIMEOUT` / `HERMES_CRON_TIMEOUT` | Inactivity timeout for cron job agent runs in seconds (default: `600`). The agent can run indefinitely while actively calling tools or receiving stream tokens — this only triggers when idle. Set to `0` for unlimited. Forecast ledger scheduled reviews use the forecast scheduler path and can opt into `FORECAST_AUTO_SCORE` / `FORECAST_AUTO_POSTMORTEM`; the `HERMES_*` name remains a legacy alias. |
| `SUPERFORECASTING_AGENT_CRON_SCRIPT_TIMEOUT` / `FORECAST_CRON_SCRIPT_TIMEOUT` / `HERMES_CRON_SCRIPT_TIMEOUT` | Timeout for pre-run scripts attached to cron jobs in seconds (default: `120`). Override for scripts that need longer execution (e.g., randomized delays for anti-bot timing). Also configurable via `cron.script_timeout_seconds` in `config.yaml`; the `HERMES_*` name remains a legacy alias. |
| `SUPERFORECASTING_AGENT_CRON_MAX_PARALLEL` / `FORECAST_CRON_MAX_PARALLEL` / `HERMES_CRON_MAX_PARALLEL` | Max cron jobs run in parallel per tick (default: `4`); the `HERMES_*` name remains a legacy alias. |
...
| `SUPERFORECASTING_AGENT_INTERACTIVE` / `FORECAST_INTERACTIVE` / `HERMES_INTERACTIVE` | Internal/session flag that tells approval and sudo-prompt code a user-facing CLI/TUI/ACP context is present; set by the runtime, but aliases are honored for tests and controlled wrappers. |
| `SUPERFORECASTING_AGENT_ACCEPT_HOOKS` / `FORECAST_ACCEPT_HOOKS` / `HERMES_ACCEPT_HOOKS` | Auto-approve any unseen shell hooks declared in `config.yaml` without a TTY prompt. Equivalent to `--accept-hooks` or `hooks_auto_accept: true`; the `HERMES_*` name remains a legacy alias. |
...
| `SUPERFORECASTING_AGENT_RPC_SOCKET` / `FORECAST_RPC_SOCKET` / `HERMES_RPC_SOCKET` | Internal `execute_code` socket endpoint passed to sandboxed child processes so generated scripts can call parent-owned tools. Set automatically; do not edit by hand. |
| `SUPERFORECASTING_AGENT_RPC_DIR` / `FORECAST_RPC_DIR` / `HERMES_RPC_DIR` | Internal `execute_code` file-transport directory for remote backends. Defaults to a forecast-named temp directory when a parent process does not set it. |
| `SUPERFORECASTING_AGENT_API_TIMEOUT` / `FORECAST_API_TIMEOUT` / `HERMES_API_TIMEOUT` | LLM API call timeout in seconds (default: `1800`); the `HERMES_*` name remains a legacy alias. |
| `SUPERFORECASTING_AGENT_API_CALL_STALE_TIMEOUT` / `FORECAST_API_CALL_STALE_TIMEOUT` / `HERMES_API_CALL_STALE_TIMEOUT` | Non-streaming stale-call timeout in seconds (default: `300`). Auto-disabled for local providers when left unset. Also configurable via `providers.<id>.stale_timeout_seconds` or `providers.<id>.models.<model>.stale_timeout_seconds` in `config.yaml`; the `HERMES_*` name remains a legacy alias. |
| `SUPERFORECASTING_AGENT_STREAM_READ_TIMEOUT` / `FORECAST_STREAM_READ_TIMEOUT` / `HERMES_STREAM_READ_TIMEOUT` | Streaming socket read timeout in seconds (default: `120`). Auto-increased to `SUPERFORECASTING_AGENT_API_TIMEOUT` for local providers. Increase if local LLMs time out during long code generation; the `HERMES_*` name remains a legacy alias. |
| `SUPERFORECASTING_AGENT_STREAM_STALE_TIMEOUT` / `FORECAST_STREAM_STALE_TIMEOUT` / `HERMES_STREAM_STALE_TIMEOUT` | Stale stream detection timeout in seconds (default: `180`). Auto-disabled for local providers. Triggers connection kill if no chunks arrive within this window; the `HERMES_*` name remains a legacy alias. |
| `SUPERFORECASTING_AGENT_STREAM_RETRIES` / `FORECAST_STREAM_RETRIES` / `HERMES_STREAM_RETRIES` | Number of mid-stream reconnect attempts on transient network errors (default: `2` retries, `3` total attempts); the `HERMES_*` name remains a legacy alias. |
| `SUPERFORECASTING_AGENT_SIGTERM_GRACE` / `FORECAST_SIGTERM_GRACE` / `HERMES_SIGTERM_GRACE` | Seconds the classic CLI waits after SIGTERM/SIGHUP while an active agent interrupts tool subprocesses and cleans up (default: `1.5`); the `HERMES_*` name remains a legacy alias. |
| `SUPERFORECASTING_AGENT_AGENT_TIMEOUT` / `FORECAST_AGENT_TIMEOUT` / `HERMES_AGENT_TIMEOUT` | Gateway inactivity timeout for a running agent in seconds (default: `900`). Resets on every tool call and streamed token. Set to `0` to disable; the `HERMES_*` name remains a legacy alias. |
| `SUPERFORECASTING_AGENT_AGENT_TIMEOUT_WARNING` / `FORECAST_AGENT_TIMEOUT_WARNING` / `HERMES_AGENT_TIMEOUT_WARNING` | Gateway: send a warning message after this many seconds of inactivity (default: 75% of `SUPERFORECASTING_AGENT_AGENT_TIMEOUT`); the `HERMES_*` name remains a legacy alias. |
| `SUPERFORECASTING_AGENT_AGENT_NOTIFY_INTERVAL` / `FORECAST_AGENT_NOTIFY_INTERVAL` / `HERMES_AGENT_NOTIFY_INTERVAL` | Gateway: interval in seconds between progress notifications on long-running agent turns; the `HERMES_*` name remains a legacy alias. |
...
| `SUPERFORECASTING_AGENT_BUNDLED_PLUGINS` / `FORECAST_BUNDLED_PLUGINS` / `HERMES_BUNDLED_PLUGINS` | Advanced packaged-install override for the bundled plugin directory. Nix wrappers set this automatically; the `HERMES_*` name remains a legacy alias. |
| `SUPERFORECASTING_AGENT_ENABLE_PROJECT_PLUGINS` / `FORECAST_ENABLE_PROJECT_PLUGINS` / `HERMES_ENABLE_PROJECT_PLUGINS` | Enable auto-discovery of repo-local plugins from `./.hermes/plugins/` (`true`/`false`, default: `false`). The directory name and `HERMES_*` var are inherited compatibility surfaces. |
| `SUPERFORECASTING_AGENT_PLUGINS_DEBUG` / `FORECAST_PLUGINS_DEBUG` / `HERMES_PLUGINS_DEBUG` | `1`/`true` to surface verbose plugin-discovery logs on stderr — directories scanned, manifests parsed, skip reasons, and full tracebacks on parse or `register()` failure. Aimed at plugin authors; the `HERMES_*` name remains a legacy alias. |
...
| `SUPERFORECASTING_AGENT_OAUTH_FILE` / `FORECAST_OAUTH_FILE` / `HERMES_OAUTH_FILE` | Override the Anthropic PKCE OAuth credential file path (default: `<agent-home>/.anthropic_oauth.json`); the `HERMES_*` name remains a legacy alias. |
...
| Variable | Description |
|----------|-------------|
| `SUPERFORECASTING_AGENT_TUI` / `FORECAST_TUI` / `HERMES_TUI` | Launch the [TUI](../user-guide/tui.md) instead of the classic CLI when set to `1`, `true`, `yes`, or `on`. Equivalent to running `superforecasting-agent tui`; `--tui` remains accepted for compatibility. |
...
| `SUPERFORECASTING_AGENT_PYTHON` / `FORECAST_PYTHON` / `HERMES_PYTHON` | Python interpreter used by the TUI gateway subprocess before falling back to generic `PYTHON`, active virtualenvs, repo virtualenvs, and system Python. |
| `SUPERFORECASTING_AGENT_PYTHON_SRC_ROOT` / `FORECAST_PYTHON_SRC_ROOT` / `HERMES_PYTHON_SRC_ROOT` | Source root inserted ahead of cwd for the TUI Python gateway subprocess. Used by local checkouts, distro wrappers, and tests to prevent cwd shadowing. |
...
| `SUPERFORECASTING_AGENT_TUI_DISABLE_MOUSE` / `FORECAST_TUI_DISABLE_MOUSE` / `HERMES_TUI_DISABLE_MOUSE` | Disable terminal mouse tracking for the TUI. The embedded dashboard PTY sets this so browser scrollback remains usable. |
...
| `SUPERFORECASTING_AGENT_TUI_SCROLL_SPEED` / `FORECAST_TUI_SCROLL_SPEED` / `HERMES_TUI_SCROLL_SPEED` | Set the base TUI wheel-scroll speed. Values are clamped to the `(0, 20]` range; `CLAUDE_CODE_SCROLL_SPEED` remains a portability fallback. |
| `SUPERFORECASTING_AGENT_TUI_NO_CONFIRM` / `FORECAST_TUI_NO_CONFIRM` / `HERMES_TUI_NO_CONFIRM` | Suppress destructive-action confirmation prompts in the TUI. Intended for controlled automation and tests, not normal interactive forecasting work. |
| `SUPERFORECASTING_AGENT_TUI_PROVIDER` / `FORECAST_TUI_PROVIDER` / `HERMES_TUI_PROVIDER` | Override the provider used by the Python TUI gateway at startup. Model-switching writes the fork-native aliases while preserving the legacy compatibility name. |
| `SUPERFORECASTING_AGENT_TUI_TOOLSETS` / `FORECAST_TUI_TOOLSETS` / `HERMES_TUI_TOOLSETS` | Comma-separated TUI gateway toolset override. Use `forecast-desk` for the narrowed forecasting surface, `all` for every inherited toolset, or enabled MCP server names for explicit extensions. |
| `SUPERFORECASTING_AGENT_TUI_MAX_TURNS` / `FORECAST_TUI_MAX_TURNS` / `HERMES_TUI_MAX_TURNS` | Override the maximum tool-calling turns for agents launched through the TUI gateway. |
| `SUPERFORECASTING_AGENT_TUI_CHECKPOINTS` / `FORECAST_TUI_CHECKPOINTS` / `HERMES_TUI_CHECKPOINTS` | Enable filesystem checkpoints for agents launched through the TUI gateway. |
| `SUPERFORECASTING_AGENT_TUI_PASS_SESSION_ID` / `FORECAST_TUI_PASS_SESSION_ID` / `HERMES_TUI_PASS_SESSION_ID` | Pass the active session id into agents launched through the TUI gateway. |
| `SUPERFORECASTING_AGENT_TUI_SKILLS` / `FORECAST_TUI_SKILLS` / `HERMES_TUI_SKILLS` | Comma-separated bundled skill names to attach to agents launched by the TUI gateway. |
| `SUPERFORECASTING_AGENT_TUI_TOOL_PROGRESS` / `FORECAST_TUI_TOOL_PROGRESS` / `HERMES_TUI_TOOL_PROGRESS` | Override TUI gateway tool-progress rendering mode: `off`, `new`, `all`, or `verbose`. |
| `SUPERFORECASTING_AGENT_TUI_SLASH_TIMEOUT_S` / `FORECAST_TUI_SLASH_TIMEOUT_S` / `HERMES_TUI_SLASH_TIMEOUT_S` | Timeout in seconds for slow TUI slash-command handlers. Defaults to `45`. |
| `SUPERFORECASTING_AGENT_TUI_RPC_POOL_WORKERS` / `FORECAST_TUI_RPC_POOL_WORKERS` / `HERMES_TUI_RPC_POOL_WORKERS` | Worker count for slow TUI RPC handlers. Values below `2` are clamped to `2`; default is `4`. |
| `SUPERFORECASTING_AGENT_TUI_THEME` / `FORECAST_TUI_THEME` / `HERMES_TUI_THEME` | Force the TUI or classic CLI color theme: `light`, `dark`, or a raw 6-character background hex (e.g. `ffffff` or `1a1a2e`). When unset, Superforecasting Agent auto-detects using `COLORFGBG` and terminal background queries; this variable overrides detection on terminals (Ghostty, Warp, iTerm2, etc.) that don't set `COLORFGBG`. |
| `SUPERFORECASTING_AGENT_TUI_TRUECOLOR` / `FORECAST_TUI_TRUECOLOR` / `HERMES_TUI_TRUECOLOR` | Force or disable 24-bit TUI color handling with a boolean value. Useful when a terminal advertises RGB color incorrectly or needs an explicit opt-in. |
| `SUPERFORECASTING_AGENT_TUI_DIM` / `FORECAST_TUI_DIM` / `HERMES_TUI_DIM` | Force or disable ANSI dim styling in the TUI. Useful on terminals where dim text is unreadable or unsupported. |
| `SUPERFORECASTING_AGENT_TUI_DISABLE_MOUSE_CLICKS` / `FORECAST_TUI_DISABLE_MOUSE_CLICKS` / `HERMES_TUI_DISABLE_MOUSE_CLICKS` | Disable mouse-click handling while keeping keyboard navigation available. Useful when a terminal or multiplexer sends unreliable mouse click events. |
| `SUPERFORECASTING_AGENT_TUI_FORCE_OSC52` / `FORECAST_TUI_FORCE_OSC52` / `HERMES_TUI_FORCE_OSC52` | Force or disable OSC52 text clipboard writes from the TUI with a boolean value. Useful over SSH, tmux, and terminals whose native clipboard tooling is unavailable. |
| `SUPERFORECASTING_AGENT_TUI_DEBUG_CLIPBOARD` / `FORECAST_TUI_DEBUG_CLIPBOARD` / `HERMES_TUI_DEBUG_CLIPBOARD` | Print TUI clipboard path diagnostics to stderr. Use only while debugging local copy/paste failures. |
| `SUPERFORECASTING_AGENT_LIGHT` / `SUPERFORECASTING_AGENT_TUI_LIGHT` / `FORECAST_LIGHT` / `FORECAST_TUI_LIGHT` / `HERMES_LIGHT` / `HERMES_TUI_LIGHT` | Force light or dark TUI and classic CLI detection with a boolean value. This takes precedence over theme/background hints. |
| `SUPERFORECASTING_AGENT_TUI_BACKGROUND` / `FORECAST_TUI_BACKGROUND` / `HERMES_TUI_BACKGROUND` | Provide a 3- or 6-digit background color hint for automatic TUI and classic CLI light/dark selection. |
| `SUPERFORECASTING_AGENT_TUI_GATEWAY_URL` / `FORECAST_TUI_GATEWAY_URL` / `HERMES_TUI_GATEWAY_URL` | Attach the TUI to an existing gateway websocket instead of spawning the local Python stdio gateway. |
...
| `SUPERFORECASTING_AGENT_TUI_GATEWAY_SHUTDOWN_GRACE_S` / `FORECAST_TUI_GATEWAY_SHUTDOWN_GRACE_S` / `HERMES_TUI_GATEWAY_SHUTDOWN_GRACE_S` | Seconds the TUI Python gateway allows for shutdown cleanup before a hard exit after broken-pipe or signal handling. |
| `SUPERFORECASTING_AGENT_TUI_GATEWAY_NO_FLUSH` / `FORECAST_TUI_GATEWAY_NO_FLUSH` / `HERMES_TUI_GATEWAY_NO_FLUSH` | Disable per-frame stdout flushes in the TUI Python gateway. Only safe with unbuffered Python; mainly for diagnosing blocked half-closed pipes. |
...
| `SUPERFORECASTING_AGENT_DASHBOARD_TUI` / `FORECAST_DASHBOARD_TUI` / `HERMES_DASHBOARD_TUI` | Expose the in-browser Forecast Desk tab backed by the embedded TUI when running the dashboard. The forecast dashboard remains the primary surface. |
...
For task-specific direct endpoints, Superforecasting Agent uses the task's configured API key or `OPENAI_API_KEY`. It does not reuse `OPENROUTER_API_KEY` for those custom endpoints.
...
These go in the active agent `config.yaml` under the `provider_routing` section:
...
:::tip
Use `superforecasting-agent config set` to set environment variables — it automatically saves them to the right file (`.env` for secrets, `config.yaml` for everything else).
:::
