...
| `SUPERFORECASTING_AGENT_KANBAN_HOME` / `FORECAST_KANBAN_HOME` / `HERMES_KANBAN_HOME` | Override the shared agent root that anchors the kanban board (db + workspaces + worker logs). Falls back to `get_default_agent_root()` (the parent of any active profile). The `HERMES_*` name remains a legacy alias. |
