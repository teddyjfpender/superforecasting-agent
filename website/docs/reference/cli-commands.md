...
| `forecast sources` | List adapters, run `forecast sources --question <id>` to plan official data/RSS/news/market priors/watched searches, or run `forecast sources --question <id> --search-watched` to search configured RSS/Atom watches for candidate evidence. |
...
prompt packets without calling a model or writing a backtest run. The same mode
works with one dataset or with `--all-benchmarks` for a built-in-suite packet
