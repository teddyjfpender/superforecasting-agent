...
description: "Frequently asked questions and solutions to common issues with Superforecasting Agent"
---
...
The main surface is the local forecasting desk:
...
No. Metaculus import is a useful read-only adapter for extracting question metadata, crowd forecasts, and resolved benchmark data. It is not the center of the product.

The intended workflow works for politics, macro, companies, markets, science, AI, policy, sports, operational risk, or any other scoreable question where you can define an outcome space, close time, resolution criteria, and resolution source.

### Where are forecasts stored?

Forecasts live in the local append-only forecast ledger, not ordinary chat memory. The default ledger is under the agent home, normally `~/.superforecasting-agent/forecasting/forecasting.db`; legacy `~/.hermes` homes remain readable during the compatibility transition.

Each question records forecast history, timestamped evidence, baselines, assumptions, reference classes, model runs, snapshots, scores, postmortems, corrections, and calibration lessons.

### How does it learn from mistakes?

Resolve and score forecasts, then write postmortems and calibration lessons:
...
Scheduled self-checks can also trigger scoring and postmortem learning when enabled:

```bash
forecast schedule add --domain macro --cadence daily --auto-score --auto-postmortem
forecast schedule run
```

### What is backtesting for?

Backtesting replays resolved questions with an evidence cutoff so the forecast engine can be compared against naive, market, crowd, base-rate, or imported baselines without leaking future information.
...
Replay results are not proof that the live system beats expert forecasters, but they are the feedback loop needed to measure and improve it.

### What LLM providers work with Superforecasting Agent?

Superforecasting Agent works with any OpenAI-compatible API. Supported providers include:

- **[OpenRouter](https://openrouter.ai/)** — access hundreds of models through one API key (recommended for flexibility)
- **Nous Portal** — Nous Research's own inference endpoint
- **OpenAI** — GPT-5.4, GPT-5-codex, GPT-4.1, GPT-4o, etc.
- **Anthropic** — Claude models (direct API, OAuth via `superforecasting-agent auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `superforecasting-agent model` or by editing `~/.superforecasting-agent/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
### I run Superforecasting Agent in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run Superforecasting Agent inside WSL2
...
- let Superforecasting Agent use the resulting MCP browser tools

This is more reliable than trying to force Superforecasting Agent core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with the inherited runtime](../guides/use-mcp-with-hermes.md#wsl2-bridge-hermes-in-wsl-to-windows-chrome)
...
Yes, for basic inherited CLI usage. The forecast fork's recommended development path is still a local checkout with Python 3.11, but the inherited Termux path remains useful for lightweight terminal sessions.
...
API calls go **only to the LLM provider you configure** (for example OpenRouter, Anthropic, or your local Ollama instance). Superforecasting Agent does not collect telemetry, usage data, or analytics. Forecast ledgers, conversations, memory, and skills are stored locally under `~/.superforecasting-agent/` by default, with legacy `~/.hermes/` compatibility.

### Can I use it offline / with local models?

Yes. Run `superforecasting-agent model`, select **Custom endpoint**, and enter your server's URL:
...
Superforecasting Agent persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 16384`), make sure to set the matching context length in Superforecasting Agent — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::
...
Superforecasting Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Superforecasting Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).

### What's the difference between memory and skills?

- **Forecast ledger** stores **scoreable beliefs** — questions, probabilities, evidence, baselines, scores, postmortems, and calibration lessons. This is the durable learning substrate.
- **Memory** stores **facts and preferences** — useful context about you, projects, and recurring domains. It is secondary to the ledger for forecasting performance.
...
All three can persist across sessions. See [Memory](../user-guide/features/memory.md) and [Skills](../user-guide/features/skills.md) for details.

### Can I use it in my own Python project?

Yes. For forecast-native code, import the ledger API:
...
question = ledger.create_question(
    title="Will the test forecast resolve yes?",
    resolution_criteria="Resolve yes if the local smoke test passes.",
)
```

The inherited runtime class is still available for general agent sessions:
...
#### `superforecasting-agent: command not found` after installation
...
ls ~/.local/bin/superforecasting-agent
```
...
**Cause:** Superforecasting Agent builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Superforecasting Agent runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Superforecasting Agent auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.superforecasting-agent/config.yaml`:
...
sudo rm /usr/local/bin/superforecasting-agent
# Then reinstall from your fork checkout
uv pip install -e ".[all,dev]"
```
...
| Want to... | Use |
|-----------|-----|
| Add a new provider | `superforecasting-agent model` (from terminal) |
| Enter/change API keys | `superforecasting-agent model` (from terminal) |
...
superforecasting-agent config show

# Re-configure your provider
superforecasting-agent model

# Or set directly
superforecasting-agent config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.superforecasting-agent/.env` for conflicting entries.
:::
...
**Cause:** The conversation has grown too long for the model's context window, or Superforecasting Agent detected the wrong context length for your model.
...
If this happens on the first long conversation, Superforecasting Agent may have the wrong context length for your model. Check what it detected:
...
**Cause:** Superforecasting Agent detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Superforecasting Agent never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
cat ~/.superforecasting-agent/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `superforecasting-agent gateway setup`
- Check gateway logs: `cat ~/.superforecasting-agent/logs/gateway.log | tail -50`
...
Configure in `~/.superforecasting-agent/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
pip install "superforecasting-agent[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
#### WSL: Gateway keeps disconnecting or `superforecasting-agent gateway start` fails {#wsl-gateway-keeps-disconnecting-or-hermes-gateway-start-fails}
...
tmux new -s superforecasting-agent 'superforecasting-agent gateway run'
# Reattach later: tmux attach -t superforecasting-agent

# Option 3: Background via nohup
nohup superforecasting-agent gateway run > ~/.superforecasting-agent/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'superforecasting-agent gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `superforecasting-agent gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
superforecasting-agent gateway install    # Re-snapshots your current PATH
superforecasting-agent gateway start      # Detects the updated plist and reloads
```
...
**Solution:**
```bash
# Ensure MCP dependencies are installed from your fork checkout
...
Verify your `~/.superforecasting-agent/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
:::warning
If an MCP server crashes mid-request, Superforecasting Agent will report a timeout. Check the server's own logs (not just Superforecasting Agent logs) to diagnose the root cause.
:::
...
### How do profiles differ from just setting SUPERFORECASTING_AGENT_HOME?

Profiles are a managed layer on top of the agent home. You *could* manually set `SUPERFORECASTING_AGENT_HOME=/some/path` before every command, but profiles handle the directory structure, generated shell aliases such as `superforecasting-agent-work`, the active-profile marker in `~/.superforecasting-agent/active_profile`, and skill sync across profiles. Legacy `HERMES_HOME` still works as a compatibility alias for inherited modules.
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `superforecasting-agent profile create newname --clone-all` to copy everything from the current profile.

### What happens when I run `superforecasting-agent update`?

`superforecasting-agent update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `superforecasting-agent update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is just a directory under `~/.superforecasting-agent/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Scenario:** You see gateway exec logs, Superforecasting Agent reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `superforecasting-agent skills config` settings don't seem to take effect.

**Solution:** Use `superforecasting-agent skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`superforecasting-agent gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Superforecasting Agent creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
2. On the **source machine**, create a full backup:
   ...
   ```
   This creates a zip of your entire `~/.superforecasting-agent/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/superforecasting-agent-backup-<timestamp>.zip`.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/superforecasting-agent-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   superforecasting-agent import ~/superforecasting-agent-backup-<timestamp>.zip
   ```

4. On the new machine, run `superforecasting-agent setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
superforecasting-agent profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
superforecasting-agent profile import ./work-backup.tar.gz work
```
...
### `superforecasting-agent backup` vs `superforecasting-agent profile export`

| Feature | `superforecasting-agent backup` | `superforecasting-agent profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Global (entire `~/.superforecasting-agent` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='superforecasting-agent' ~/.superforecasting-agent/ newmachine:~/.superforecasting-agent/
```

:::tip
`superforecasting-agent backup` produces a consistent snapshot even while Superforecasting Agent is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the Superforecasting Agent installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Superforecasting Agent-specific issue — it's a shell config permissions problem.
...
superforecasting-agent config show | head -20
