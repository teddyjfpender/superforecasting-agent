...
description: "How to build a memory provider plugin for Superforecasting Agent."
---
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          hermes_home (str): Active runtime home path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `superforecasting-agent memory setup` | **Yes** |
...
`get_config_schema()` returns a list of field descriptors used by `superforecasting-agent memory setup`:
...
Every field in `get_config_schema()` is prompted during `superforecasting-agent memory setup`. Providers with many options should keep the schema minimal - only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$SUPERFORECASTING_AGENT_HOME/myprovider.json`, with legacy `$HERMES_HOME/myprovider.json` still readable for compatibility) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example: it only prompts for the API key; all other options live in `supermemory.json`.
:::
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `superforecasting-agent my-provider status`, `superforecasting-agent my-provider config`). This uses a convention-based discovery system; no changes to core files are needed.
...
4. Your commands appear under `superforecasting-agent <provider-name> <subcommand>` (and under legacy `hermes` entry points when compatibility shims are installed)

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `superforecasting-agent --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: superforecasting-agent my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the superforecasting-agent my-provider argparse tree.
