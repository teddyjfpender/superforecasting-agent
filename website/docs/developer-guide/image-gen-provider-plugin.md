...
description: "Build image-generation backend plugins"
---
...
The runtime scans for image-gen backends in three places:

1. **Bundled** — `<repo>/plugins/image_gen/<name>/` (auto-loaded with `kind: backend`, always available)
2. **User** — `~/.superforecasting-agent/plugins/image_gen/<name>/` (opt-in via `plugins.enabled`). The legacy `~/.hermes/plugins/image_gen/<name>/` path remains accepted for inherited installs.
...
Each plugin's `register(ctx)` function calls `ctx.register_image_gen_provider(...)` — that puts it into the registry in `agent/image_gen_registry.py`. The active provider is picked by `image_gen.provider` in `config.yaml`; `superforecasting-agent tools` walks users through selection.

The `image_generate` tool wrapper asks the registry for the active provider and dispatches there. If no provider is registered, the tool surfaces a helpful error pointing at `superforecasting-agent tools`.
...
A bundled plugin is complete at this point. User plugins at `~/.superforecasting-agent/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `superforecasting-agent plugins enable <name>`).
...
class MyBackendImageGenProvider(ImageGenProvider):
    ...
    def display_name(self) -> str:
        # Human label shown in `superforecasting-agent tools`. Defaults to name.title() if omitted.
        return "My Backend"
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        # Catalog shown in `superforecasting-agent tools` model picker.
        return [
            {
                "id": "my-model-fast",
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        # Metadata for the tools picker — keys to prompt for at setup.
        return {
            "name": "My Backend",
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            #   - base64 data: save under the active agent home cache/images/ via save_b64_image()
...
`kind: backend` is what routes the plugin to the image-gen registration path. `requires_env` is prompted during `superforecasting-agent plugins install`.
...
| `display_name` | — | `name.title()` | Label shown in `superforecasting-agent tools` |
| `is_available()` | — | `True` | Gate for missing creds/deps |
| `list_models()` | — | `[]` | Catalog for `superforecasting-agent tools` model picker |
...
Some backends return image URLs (fal, Replicate); others return base64 payloads (OpenAI gpt-image-2). For the base64 case, use `save_b64_image()` — it writes to the active agent home, normally `~/.superforecasting-agent/cache/images/<prefix>_<timestamp>_<uuid>.<ext>`, and returns the absolute `Path`. Legacy `$HERMES_HOME/cache/images/` remains accepted by inherited installs. Pass that path (as `str`) as `image=` in `success_response()`. Gateway delivery (Telegram photo bubble, Discord attachment) recognizes both URLs and absolute paths.

## User overrides

Drop a user plugin at `~/.superforecasting-agent/plugins/image_gen/<name>/` with the same `name` property as a bundled one and enable it via `superforecasting-agent plugins enable <name>` — the registry is last-writer-wins, so your version replaces the built-in. Useful for pointing an `openai` plugin at a private proxy, or swapping in a custom model catalog.

## Testing

```bash
export SUPERFORECASTING_AGENT_HOME=/tmp/superforecasting-agent-imggen-test
mkdir -p $SUPERFORECASTING_AGENT_HOME/plugins/image_gen/my-backend
# …copy __init__.py + plugin.yaml into that dir…

export MY_BACKEND_API_KEY=your-test-key
superforecasting-agent plugins enable my-backend

# Pick it as the active provider
echo "image_gen:" >> $SUPERFORECASTING_AGENT_HOME/config.yaml
