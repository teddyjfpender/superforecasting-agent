...
description: "How to build a model provider plugin for Superforecasting Agent."
---
...
1. **Bundled plugins** - `<repo>/plugins/model-providers/<name>/` - ship with the fork
2. **User plugins** - `$SUPERFORECASTING_AGENT_HOME/plugins/model-providers/<name>/` or legacy `$HERMES_HOME/plugins/model-providers/<name>/` - drop in any directory; no restart required for subsequent sessions
3. **Legacy single-file** - `<repo>/providers/<name>.py` - back-compat for out-of-tree editable installs

**User plugins override bundled plugins of the same name** because `register_provider()` is last-writer-wins. Drop a `$SUPERFORECASTING_AGENT_HOME/plugins/model-providers/gmi/` directory to replace the built-in GMI profile without touching the repo.
...
The only required file is `__init__.py`. `plugin.yaml` is used by `superforecasting-agent plugins` for introspection and by the general PluginManager to route the plugin to the right loader; without it, the general loader falls back to a source-text heuristic.
...
| `default_aux_model` | str | Cheap model for auxiliary tasks (compression, vision, summarization, scheduled reviews) |
...
Say you want to point `gmi` at your private staging endpoint for testing. Create `~/.superforecasting-agent/plugins/model-providers/gmi/__init__.py`:
...
Four values are recognized. The runtime picks one based on:
...
`auth_type` gates which codepaths treat your provider as a "simple api-key provider". If it's not `api_key`, the PluginManager still records the manifest but Superforecasting Agent's CLI-level automation (doctor checks, `--provider` flag, setup wizard delegation) may skip over it.
...
Point `SUPERFORECASTING_AGENT_HOME` at a temp directory so you don't pollute your real config:

```bash
export SUPERFORECASTING_AGENT_HOME=/tmp/superforecasting-agent-plugin-test
mkdir -p $SUPERFORECASTING_AGENT_HOME/plugins/model-providers/my-provider
cat > $SUPERFORECASTING_AGENT_HOME/plugins/model-providers/my-provider/__init__.py <<'EOF'
from providers import register_provider
from providers.base import ProviderProfile
register_provider(ProviderProfile(
    name="my-provider",
...
superforecasting-agent chat -q "hello" --provider my-provider -m some-model
```

## General PluginManager integration

The general `PluginManager` (the thing `superforecasting-agent plugins` operates on) **sees** model-provider plugins but does not import them; `providers/__init__.py` owns their lifecycle. The manager records the manifest for introspection and categorizes by `kind: model-provider`. When you drop an unlabeled user plugin into `$SUPERFORECASTING_AGENT_HOME/plugins/` or legacy `$HERMES_HOME/plugins/` that happens to call `register_provider` with a `ProviderProfile`, the manager auto-coerces it to `kind: model-provider` via a source-text heuristic, so the plugin still routes correctly even without `plugin.yaml`.

## Distribute via pip

Like any Superforecasting Agent plugin, model providers can ship as a pip package. The inherited entry-point group is still `hermes.plugins` for compatibility:

```toml
[project.entry-points."hermes.plugins"]
acme-inference = "acme_forecasting_plugin:register"
```
