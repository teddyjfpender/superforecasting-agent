...
description: "How Superforecasting Agent resolves providers and models."
---

# Provider Runtime Resolution

Superforecasting Agent uses the inherited shared provider runtime resolver across:

- forecast CLI and agent-assisted forecast updates
- gateway/API/ACP compatibility surfaces
- scheduled self-checks and cron jobs
- auxiliary model calls for research, summarization, compression, and vision
- benchmark and calibration workflows that need reproducible model provenance
...
`get_provider_profile()` in `providers/` returns a `ProviderProfile` for a given provider id. `runtime_provider.py` calls this at resolution time to get the canonical `base_url`, `env_vars` priority list, `api_mode`, and `fallback_models` without needing to duplicate that data in multiple files. Adding a new plugin under `plugins/model-providers/<your-provider>/` (or a user plugin under the fork-native/legacy plugin home) that calls `register_provider()` is enough for `runtime_provider.py` to pick it up; no branch is needed in the resolver itself.
...
That ordering matters because Superforecasting Agent treats the saved model/provider choice as the source of truth for normal runs. Forecast snapshots, model runs, backtests, and postmortems should be able to record which model/provider configuration was active. This also prevents a stale shell export from silently overriding the endpoint a user last selected in `superforecasting-agent model`.
...
This resolver is the main reason Superforecasting Agent can share auth/runtime logic between:
...
Set `AI_GATEWAY_API_KEY` in `~/.superforecasting-agent/.env` and run with `--provider ai-gateway`. The inherited `~/.hermes/.env` path remains a compatibility fallback. Superforecasting Agent fetches available models from the gateway's `/models` endpoint, filtering to language models with tool-use support.

## OpenRouter, AI Gateway, and custom OpenAI-compatible base URLs

The runtime contains logic to avoid leaking the wrong API key to a custom endpoint when multiple provider keys exist (e.g. `OPENROUTER_API_KEY`, `AI_GATEWAY_API_KEY`, and `OPENAI_API_KEY`).
...
When provider resolution selects `anthropic`, the runtime uses:
...
- Superforecasting Agent preflights Anthropic credential refresh before native Messages API calls
- Superforecasting Agent still retries once on a 401 after rebuilding the Anthropic client, as a fallback path
...
When an auxiliary task is configured with provider `main`, Superforecasting Agent resolves that through the same shared runtime path as normal forecast/chat execution. In practice that means:

- env-driven custom endpoints still work
- custom endpoints saved via `superforecasting-agent model` / `config.yaml` also work
- auxiliary routing can tell the difference between a real saved custom endpoint and the OpenRouter fallback

## Fallback models

Superforecasting Agent supports a configured fallback provider chain: a list of `(provider, model)` entries tried in order when the primary model encounters errors. The legacy single-pair `fallback_model` dict is still accepted for back-compat and migrated on first write.
