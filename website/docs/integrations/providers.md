...
This page covers setting up inference providers for Superforecasting Agent — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use Superforecasting Agent.

## Inference Providers

You need at least one way to connect to an LLM. Use `superforecasting-agent model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Nous Portal** | `superforecasting-agent model` (OAuth, subscription-based) |
| **OpenAI Codex** | `superforecasting-agent model` (ChatGPT OAuth, uses Codex models) |
| **GitHub Copilot** | `superforecasting-agent model` (OAuth device code flow, `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `gh auth token`) |
| **GitHub Copilot ACP** | `superforecasting-agent model` (spawns local `copilot --acp --stdio`) |
| **Anthropic** | `superforecasting-agent model` (Claude Max + extra usage credits via OAuth; also supports Anthropic API key or manual setup-token — see note below) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.superforecasting-agent/.env` |
| **NovitaAI** | `NOVITA_API_KEY` in `~/.superforecasting-agent/.env` (provider: `novita`, 200+ models, Model API, Agent Sandbox, GPU Cloud) |
| **AI Gateway** | `AI_GATEWAY_API_KEY` in `~/.superforecasting-agent/.env` (provider: `ai-gateway`) |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.superforecasting-agent/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.superforecasting-agent/.env` (provider: `kimi-coding`) |
| **Kimi / Moonshot (China)** | `KIMI_CN_API_KEY` in `~/.superforecasting-agent/.env` (provider: `kimi-coding-cn`; aliases: `kimi-cn`, `moonshot-cn`) |
| **Arcee AI** | `ARCEEAI_API_KEY` in `~/.superforecasting-agent/.env` (provider: `arcee`; aliases: `arcee-ai`, `arceeai`) |
| **GMI Cloud** | `GMI_API_KEY` in `~/.superforecasting-agent/.env` (provider: `gmi`; aliases: `gmi-cloud`, `gmicloud`) |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.superforecasting-agent/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.superforecasting-agent/.env` (provider: `minimax-cn`) |
| **xAI (Grok) — Responses API** | `XAI_API_KEY` in `~/.superforecasting-agent/.env` (provider: `xai`) |
| **xAI Grok OAuth (SuperGrok)** | `superforecasting-agent model` → "xAI Grok OAuth (SuperGrok Subscription)" — browser login, no API key. See [guide](../guides/xai-grok-oauth.md) |
| **Qwen Cloud (Alibaba DashScope)** | `DASHSCOPE_API_KEY` in `~/.superforecasting-agent/.env` (provider: `alibaba`) |
...
| **Kilo Code** | `KILOCODE_API_KEY` in `~/.superforecasting-agent/.env` (provider: `kilocode`) |
| **Xiaomi MiMo** | `XIAOMI_API_KEY` in `~/.superforecasting-agent/.env` (provider: `xiaomi`, aliases: `mimo`, `xiaomi-mimo`) |
| **Tencent TokenHub** | `TOKENHUB_API_KEY` in `~/.superforecasting-agent/.env` (provider: `tencent-tokenhub`, aliases: `tencent`, `tokenhub`, `tencentmaas`) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `~/.superforecasting-agent/.env` (provider: `opencode-zen`) |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `~/.superforecasting-agent/.env` (provider: `opencode-go`) |
| **DeepSeek** | `DEEPSEEK_API_KEY` in `~/.superforecasting-agent/.env` (provider: `deepseek`) |
| **Hugging Face** | `HF_TOKEN` in `~/.superforecasting-agent/.env` (provider: `huggingface`, aliases: `hf`) |
| **Google / Gemini** | `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) in `~/.superforecasting-agent/.env` (provider: `gemini`) |
| **Google Gemini (OAuth)** | `superforecasting-agent model` → "Google Gemini (OAuth)" (provider: `google-gemini-cli`, free tier supported, browser PKCE login) |
| **LM Studio** | `superforecasting-agent model` → "LM Studio" (provider: `lmstudio`, optional `LM_API_KEY`) |
| **Custom Endpoint** | `superforecasting-agent model` → choose "Custom endpoint" (saved in `config.yaml`) |
...
# → done — Superforecasting Agent auto-provisions your free tier on first request
```

Superforecasting Agent ships Google's **public** `gemini-cli` desktop OAuth client by default —
...
- Tokens stored at `~/.superforecasting-agent/auth/google_oauth.json` (chmod 0600, atomic write, cross-process `fcntl` lock)
...
- Responses translated back to OpenAI shape so the rest of Superforecasting Agent works unchanged
...
experience, use your own API key via the `gemini` provider instead. Superforecasting Agent shows
...
:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). Superforecasting Agent stores the resulting credentials in its own auth store under `~/.superforecasting-agent/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.

If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Superforecasting Agent marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `superforecasting-agent auth add codex-oauth` (or `superforecasting-agent model` → OpenAI Codex) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::

:::warning
Even when using Nous Portal, Codex, or a custom endpoint, some tools (vision, web summarization, MoA) use a separate "auxiliary" model. By default (`auxiliary.*.provider: "auto"`), Superforecasting Agent routes these tasks to your **main forecast model** — the same model you picked in `superforecasting-agent model`. You can override each task individually to route it to a cheaper/faster model (e.g. Gemini Flash on OpenRouter) — see [Auxiliary Models](/user-guide/configuration#auxiliary-models).
:::

:::tip Nous Tool Gateway
Paid Nous Portal subscribers also get access to the **[Tool Gateway](/user-guide/features/tool-gateway)** — web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. It's offered automatically during `superforecasting-agent model` setup, or enable it later with `superforecasting-agent tools`.
:::

### Two Commands for Model Management

Superforecasting Agent has **two** model commands that serve different purposes:
...
Subscription-based access to Hermes-4 models (`Hermes-4-70B`, `Hermes-4.3-36B`, `Hermes-4-405B`) via Nous Research's portal. Run `superforecasting-agent model`, pick **Nous Portal**, sign in through the browser — Superforecasting Agent stores a long-lived refresh token at `~/.superforecasting-agent/auth.json`.
...
Superforecasting Agent mints a short-lived JWT from your stored Nous refresh token on each inference call rather than reusing a long-lived API key. The token lifecycle is fully automatic — refresh, mint, retry on transient 401 — and you never see it.

If the portal invalidates the refresh token (password change, manual revoke, session expiry), the invalid refresh token is quarantined locally so Superforecasting Agent stops replaying it and you don't see a stream of identical 401s. The next call surfaces a clear "re-authentication required" message. Run `superforecasting-agent auth add nous` to log in again; the quarantine clears on the next successful login.
...
When you authenticate via `superforecasting-agent model` → Anthropic OAuth (or via `superforecasting-agent auth add anthropic --type oauth`), Superforecasting Agent routes as Claude Code against your Anthropic account. **It only works if you're on a Claude Max plan and have purchased extra usage credits.** The base Max plan allowance (the usage included in Claude Code by default) is not consumed by this runtime — only the extra/overage credits you've added on top are. Claude Pro subscribers cannot use this path.
...
Superforecasting Agent supports GitHub Copilot as a first-class provider with two modes:
...
If no token is found, `superforecasting-agent model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `superforecasting-agent model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `superforecasting-agent model` to authenticate via OAuth instead.
:::

:::info Copilot auth behavior in Superforecasting Agent
Superforecasting Agent sends a supported GitHub token (`gho_*`, `github_pat_*`, or `ghu_*`) directly to `api.githubcopilot.com` and includes Copilot-specific headers (`Editor-Version`, `Copilot-Integration-Id`, `Openai-Intent`, `x-initiator`).

On HTTP 401, Superforecasting Agent now performs a one-shot credential recovery before fallback:
...
Some older community proxies use `api.github.com/copilot_internal/v2/token` exchange flows. That endpoint can be unavailable for some account types (returns 404). The runtime therefore keeps direct-token auth as the primary path and relies on runtime credential refresh + retry for robustness.
:::
...
When using the Z.AI / GLM provider, Superforecasting Agent automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually — the working endpoint is detected and cached automatically.
:::

### xAI (Grok) — Responses API + Prompt Caching

xAI is wired through the Responses API (`codex_responses` transport) for automatic reasoning support on Grok 4 models — no `reasoning_effort` parameter needed, the server reasons by default. Set `XAI_API_KEY` in `~/.superforecasting-agent/.env` and pick xAI in `superforecasting-agent model`, or drop `grok` as a shortcut into `/model grok-4-1-fast-reasoning`.

SuperGrok and X Premium+ subscribers can sign in with browser OAuth instead of using an API key — pick **xAI Grok OAuth (SuperGrok Subscription)** in `superforecasting-agent model`, or run `superforecasting-agent auth add xai-oauth`. The same OAuth bearer token is automatically reused by direct-to-xAI tools (TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md) for the full flow — and if Superforecasting Agent runs on a remote host, also see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md) for the required `ssh -L` tunnel.

When using xAI as a provider (any base URL containing `x.ai`), Superforecasting Agent automatically enables prompt caching by sending the `x-grok-conv-id` header with every API request. This routes requests to the same server within a conversation session, allowing xAI's infrastructure to reuse cached system prompts and conversation history.
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `superforecasting-agent tools` → Voice & TTS, or see the [Voice & TTS](../user-guide/features/tts.md#text-to-speech) page for config.
...
[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `superforecasting-agent model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), and Superforecasting Agent auto-discovers the available models.
...
Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `superforecasting-agent model`, sign in through the browser, and Superforecasting Agent persists the refresh token.
...
If you're subscribed to Alibaba's **Coding Plan** (a pricing SKU separate from standard DashScope API access), Superforecasting Agent exposes it as its own first-class provider: `alibaba-coding-plan`. Endpoint: `https://coding-intl.dashscope.aliyuncs.com/v1`. It's OpenAI-compatible like the regular `alibaba` provider but with a different base URL and billing surface.
...
MiniMax-M2.7 via browser OAuth login — no API key needed. Pick **MiniMax (OAuth)** in `superforecasting-agent model`, sign in through the browser, and Superforecasting Agent persists the access + refresh tokens. Uses the Anthropic Messages-compatible endpoint (`/anthropic`) under the hood.
...
Superforecasting Agent automatically attaches the NIM billing-origin header on every request to `build.nvidia.com` — no configuration needed. This routes consumption against the correct origin in NVIDIA's billing dashboard.
...
Superforecasting Agent works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point Superforecasting Agent at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
`OPENAI_BASE_URL` and `LLM_MODEL` in `.env` are **removed**. Neither is read by any part of Superforecasting Agent — `config.yaml` is the single source of truth for model and endpoint configuration. If you have stale entries in your `.env`, they are automatically cleared on the next `superforecasting-agent setup` or config migration. Use `superforecasting-agent model` or edit `config.yaml` directly.
:::
...
When switching providers, Superforecasting Agent persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
Then configure Superforecasting Agent:
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like Superforecasting Agent.
:::
...
| Flag | Purpose |
|------|---------|
| `--enable-auto-tool-choice` | Required for `tool_choice: "auto"` (the default in Superforecasting Agent) |
...
Then configure Superforecasting Agent to point at it:
...
Without `--jinja`, llama-server ignores the `tools` parameter entirely. The model will try to call tools by writing JSON in its response text, but Superforecasting Agent will not recognize it as a tool call — you'll see raw JSON like `{"name": "web_search", ...}` printed as a message instead of an actual search.
...
Superforecasting Agent will automatically load a LM Studio model with 64K context length
...
Since Superforecasting Agent requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap — WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
Use that IP in your Superforecasting Agent config:
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your Superforecasting Agent config.

---

### Troubleshooting Local Models

These issues affect **all** local inference servers when used with Superforecasting Agent.

#### "Connection refused" from WSL2 to a Windows-hosted model server

If you're running Superforecasting Agent inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
**Cause:** Context window is too small. When the conversation exceeds the context limit, most servers silently drop older messages. Superforecasting Agent's system prompt + tool schemas alone can use 4k–8k tokens.

**Diagnosis:**

```bash
# Check what Superforecasting Agent thinks the context is
...
Superforecasting Agent auto-detects context length from your server's `/v1/models` endpoint. If the server reports a low value (or doesn't report one at all), the runtime uses the model's declared limit, which may be wrong.
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure Superforecasting Agent with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only — it is unrelated to how long your conversation history can be (that is `context_length`).
2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](/user-guide/configuration#context-compression) in Superforecasting Agent.
...
Then configure Superforecasting Agent with `superforecasting-agent model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure Superforecasting Agent with `superforecasting-agent model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `superforecasting-agent model` → Custom endpoint, or in `config.yaml`:
...
**`context_length`** is the **total context window** — the combined budget for input *and* output tokens (e.g. 200,000 for Claude Opus 4.6). Superforecasting Agent uses this to decide when to compress history and to validate API requests.
...
Superforecasting Agent uses a multi-source resolution chain to detect the correct context window for your model and provider:
...
`superforecasting-agent model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
custom_providers:
  - name: local
    base_url: http://localhost:8080/v1
    # api_key omitted — Superforecasting Agent uses "no-key-required" for keyless local servers
  - name: work
    base_url: https://gpu-server.internal.corp/v1
    key_env: CORP_API_KEY
    api_mode: chat_completions   # set explicitly by `superforecasting-agent model` → Custom Endpoint wizard; auto-detection still happens as a fallback
...
The `superforecasting-agent model` → Custom Endpoint wizard now prompts for `api_mode` explicitly and persists your answer to `config.yaml`. URL-based auto-detection (e.g. `/anthropic` paths → `anthropic_messages`) still happens as a fallback when the field is left blank.
...
You can also select named custom providers from the interactive `superforecasting-agent model` menu.

---

### Cookbook: Together AI, Groq, Perplexity

The cloud providers listed in [Other Compatible Providers](#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under `custom_providers:`. Three worked recipes follow. Each drops into `~/.superforecasting-agent/config.yaml` and the matching API key goes in `~/.superforecasting-agent/.env`.
...
Together's `/v1/models` endpoint works, so `superforecasting-agent model` can auto-discover available models.
...
:::tip Troubleshooting
- `superforecasting-agent doctor` should print no `Unknown provider` warnings for any of these names after the CLI validator fixes in #15083.
- If a provider's `/v1/models` endpoint is unreachable (Perplexity is the common one), `superforecasting-agent model` will persist the model with a warning rather than hard-reject — see #15136.
...
:::tip
You can switch between providers at any time with `superforecasting-agent model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point Superforecasting Agent at your instance (no API key needed):
   ```bash
   superforecasting-agent config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.superforecasting-agent/config.yaml`:
...
OpenRouter ships an experimental coding-model router at `openrouter/pareto-code` that auto-routes requests to the cheapest model meeting a coding-quality bar (ranked by [Artificial Analysis](https://artificialanalysis.ai/)). Pick this model and tune the `min_coding_score` knob in `~/.superforecasting-agent/config.yaml`:
...
Configure a chain of backup providers Superforecasting Agent tries in order when the primary model fails (rate limits, server errors, auth failures). The canonical format is a top-level `fallback_providers:` list:
...
:::tip
Fallback is configured exclusively through `config.yaml` — or interactively via `superforecasting-agent fallback`. For full details on when it triggers, how the chain advances, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](/user-guide/features/fallback-providers).
:::
