...
def render_skill_page(
    ...
    # Some inherited compatibility skill IDs still contain "hermes", while
    # their user-facing source title has been updated for the fork. Keep the
    # original display-name heuristic for ordinary skills to avoid unrelated
    # generated-doc churn.
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `superforecasting-agent skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills available to Superforecasting Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Superforecasting Agent ships with a large built-in skill library copied into `~/.superforecasting-agent/skills/` on new installs. Legacy `~/.hermes/skills/` homes remain readable during migration. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Superforecasting Agent also syncs bundled skills on `superforecasting-agent update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.superforecasting-agent/skills/` tree, it is still shipped with the fork; restore it with `superforecasting-agent skills reset <name> --restore`.",
        "",
        "Skills are procedural playbooks and tool guides. Durable forecast questions, probabilities, evidence, model runs, scores, postmortems, calibration lessons, and domain error profiles belong in the forecast ledger rather than skill memory.",
        "",
