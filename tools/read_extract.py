...
def _extract_notebook(path: str) -> str:
    ...
    if not isinstance(cells, list):
        # Legacy nbformat-3 layout. A malformed structure (e.g. ``worksheets``
        # or a worksheet's ``cells`` being a non-iterable like an int) raises
        # TypeError/AttributeError/KeyError here; re-raise as ExtractionError so
        # read_file's ``except ExtractionError`` falls back gracefully instead
        # of crashing with an uncaught exception.
        try:
            worksheets = nb.get("worksheets", [])
            if not isinstance(worksheets, list):
                raise ExtractionError("Notebook 'worksheets' is not a list")
            cells = [
                cell
            ...
            raise ExtractionError(f"Malformed notebook structure: {exc}") from exc
    if not cells:
...
def _safe_zip_read(zf: zipfile.ZipFile, name: str) -> bytes:
    """Read a zip member, refusing oversized uncompressed payloads (zip-bomb guard).

    Checks the declared uncompressed size (``ZipInfo.file_size``) against
    ``MAX_XLSX_BYTES`` *before* decompressing so a tiny compressed member can't
    expand to gigabytes in memory.
    """
    info = zf.getinfo(name)  # raises KeyError if missing
    if info.file_size > MAX_XLSX_BYTES:
        raise ExtractionError(
            f"Zip member {name!r} uncompressed size {info.file_size} exceeds "
            f"the {MAX_XLSX_BYTES}-byte limit"
        )
    return zf.read(name)
...
def _zip_xml(zf: zipfile.ZipFile, name: str) -> ET.Element:
    try:
        return ET.fromstring(_safe_zip_read(zf, name))
        ...
        with zipfile.ZipFile(path) as zf:
            ...
            for name, state, rid in sheets:
                ...
                if part not in names:
                    continue
                try:
                    rows = _sheet_rows(_safe_zip_read(zf, part), shared)
...
def _shared_strings(zf: zipfile.ZipFile, names: set[str]) -> list[str]:
    if "xl/sharedStrings.xml" not in names:
        return []
    try:
        root = ET.fromstring(_safe_zip_read(zf, "xl/sharedStrings.xml"))
    except ET.ParseError:
        return []
    s = f"{{{_NS_S}}}"
...
def _workbook_rels(zf: zipfile.ZipFile, names: set[str]) -> dict[str, str]:
    ...
    if rels_path not in names:
        return {}
    try:
        root = ET.fromstring(_safe_zip_read(zf, rels_path))
    except ET.ParseError:
        return {}
