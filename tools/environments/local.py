...
# Bedrock-specific bearer token, which is an agent inference secret exactly
...
# left inheritable.  The local terminal is the user's trusted operator shell;
# the agent having the same general AWS access the user's own shell has is the
# intended posture, not a leak.  Hard-blocklisting those vars would (a) regress
# every user who runs aws/terraform/cdk/boto3 in the agent terminal — not just
# Bedrock users, since the registry is iterated unconditionally — and (b) be
# unrecoverable, because env_passthrough.py refuses to re-allow anything in this
# blocklist (GHSA-rhgp-j443-p4rf).  See issue #32314.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter agent-managed secrets from a subprocess environment."""
    try:
...
def _make_run_env(env: dict) -> dict:
    ...
    # the forecast agent is driving via _find_bash (Git Bash), and Git Bash itself
    ...
    # Only the subprocess sees the override — the Python process keeps the
    # real HOME.
    from superforecasting_agent.constants import get_subprocess_home
    _profile_home = get_subprocess_home()
    if _profile_home:
    ...
    try:
        from superforecasting_agent.session_context import get_session_env, _UNSET, _VAR_MAP
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, the forecast agent trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        the active agent-home compatibility path instead — single-word path,
        guaranteed to exist, same string resolves in both Git Bash and native
        Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under the active agent home. Using
            ...
            try:
                ...
                cache_dir = get_agent_home() / "cache" / "terminal"
            except Exception:
