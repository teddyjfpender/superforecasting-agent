#!/usr/bin/env python3
"""
Skills Hub — source adapters and hub state management for the skills catalog.
...
from superforecasting_agent.tooling.http_io import checkpoint
from superforecasting_agent.tooling.http_io import get as hub_http_get
...
class ClawHubSource(SkillSource):
    ...
    def _load_catalog_index(self) -> List[SkillMeta]:
        ...
        for _ in range(max_pages):
            ...
            try:
                resp = hub_http_get(
                    f"{self.BASE_URL}/skills", params=params, timeout=30
                )
                if resp.status_code != 200:
                    break
                data = resp.json()
...
class OptionalSkillSource(SkillSource):
    ...
    the active skills home during setup. They are discoverable via the Skills Hub
