    ...
    if not chat_id:
        ...
        if home:
            ...
            return json.dumps({
                ...
                f"or set a home channel via: superforecasting-agent config set {platform_name.upper()}_HOME_CHANNEL <channel_id>"
            })
...
async def _send_email(extra, chat_id, message):
    ...
    try:
        ...
        msg["From"] = address
        msg["To"] = chat_id
        msg["Subject"] = "Superforecasting Agent"
...
async def _send_matrix(token, extra, chat_id, message):
    ...
    try:
        ...
        txn_id = f"forecast_{int(time.time() * 1000)}_{os.urandom(4).hex()}"
...
async def _send_feishu(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    try:
        ...
        if not FEISHU_AVAILABLE:
            return {"error": "Feishu dependencies not installed. Run: pip install 'superforecasting-agent[feishu]'"}
