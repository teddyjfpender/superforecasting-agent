"""
Lazy dependency installer for opt-in Superforecasting Agent backends.

Many optional Superforecasting Agent backends (Mistral TTS, ElevenLabs TTS,
Honcho memory, Bedrock, Slack, Matrix, etc.) require Python packages that not
every user needs. The historical approach was to bundle them all under
``pyproject.toml`` extras (``superforecasting-agent[all]``) and install them
eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``superforecasting-agent tools`` or the manual
pip command.
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # browse/search the web without an API key (see plugins/web/ddgs/provider).
    ...
    "platform.discord": ("discord.py==2.7.1", "PyNaCl==1.6.2", "davey==0.1.4", "brotlicffi==1.2.0.1"),
    "platform.slack": (
        "slack-bolt==1.27.0",
        "slack-sdk==3.40.1",
    ...
    # Dashboard (`superforecasting-agent dashboard`)
    "tool.dashboard": (
        "fastapi==0.133.1",
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``superforecasting-agent update`` propagate pin bumps in :data:`LAZY_DEPS`
    to already-installed backends instead of silently leaving stale versions in
    place.
...
def active_features() -> list[str]:
    ...
    Used by ``superforecasting-agent update`` to figure out which lazy backends
    need a refresh pass when pins move in :data:`LAZY_DEPS`.
    """
    active = []
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``superforecasting-agent update``. Never raises; lazy-install
    failures here must not block the rest of the update flow.
    """
