...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under the active agent-home
    ``logs/subagent-<sid>-<ts>.log``
...
def _apply_summary_budget(results: List[Dict[str, Any]], parent_agent) -> None:
    ...
    Used on the synchronous results.sort path (batch + single sync) AND on the
    async/background re-entry path (see _async_runner in delegate_task), where
    a single background child's final_response folds back into the parent via
...
def delegate_task(
    ...
    # Covers both the single-task and batch synchronous paths. The async /
    # background path is budgeted separately at its re-entry site (see the
    # _async_runner wrapper above). See PR #9126.
        ...
        "properties": {
            ...
            "acp_command": {
                ...
                "description": (
                    ...
                    "Leave empty to use the parent's default transport (local subagents)."
                ),
            },
            "acp_args": {
                "type": "array",
