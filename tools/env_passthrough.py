...
def _is_agent_provider_credential(name: str) -> bool:
    """True if ``name`` is an agent-managed provider credential (API key,
    token, or similar) per ``_PROVIDER_ENV_BLOCKLIST``.
    ...
    try:
        ...
        return False
    return name in _PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are agent-managed provider credentials (from
    ``_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to an agent-managed
    ...
    Third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_agent_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register agent provider "
                "credential %r (blocked by _PROVIDER_ENV_BLOCKLIST). "
