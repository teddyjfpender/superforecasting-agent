...
-----------------------------------------------------------------------------
    ...
    <active-agent-home>/checkpoints/
        store/                          — single bare-ish git repo
            HEAD, config, objects/      — standard git internals (shared)
            refs/hermes/<hash16>        — inherited per-project branch tip
...
CHECKPOINT_BASE = get_agent_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/hermes"  # inherited storage namespace; keep for compatibility
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (runtime convention — don't recursively snapshot siblings)
    ".worktrees/",
...
def _git_env(
    ...
    The shared store is internal runtime infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``superforecasting-agent checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `superforecasting-agent checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "superforecasting-agent@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Superforecasting Agent Checkpoint"], store, cfg_wd)
