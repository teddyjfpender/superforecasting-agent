"""
Cron job management tools for Superforecasting Agent.
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    job doesn't drift when the user later changes their default via the model picker.
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    # Only relative paths within the active agent-home scripts/ directory are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        scripts_display = f"{display_agent_home()}/scripts/"
        return (
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": f"Optional path to a script that runs each tick. In the default mode its stdout is injected into the agent's prompt as context (data-collection / change-detection pattern). With no_agent=True, the script IS the job and its stdout is delivered verbatim (classic watchdog pattern). Relative paths resolve under {display_agent_home()}/scripts/. ``.sh``/``.bash`` extensions run via bash, everything else via Python. On update, pass empty string to clear."
            },
            "no_agent": {
                "type": "boolean",
                "default": False,
            ...
            "profile": {
                "type": "string",
                "description": "Optional Superforecasting Agent profile name to run the job under. When set, the scheduler resolves that profile, applies a context-local agent-home override, loads that profile's config/.env for the run, and bridges HERMES_HOME into subprocesses for inherited runtime compatibility. Any temporary process-environment changes from profile .env loading are restored after the job exits. Use 'default' for the root forecast profile. Named profiles must already exist. When unset (default), preserves the scheduler's existing profile. On update, pass an empty string to clear. Jobs with profile run sequentially (not parallel) to keep profile-scoped runtime state isolated."
            },
        },
        "required": ["action"]
    }
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_alias_enabled(INTERACTIVE_ENV_NAMES)
