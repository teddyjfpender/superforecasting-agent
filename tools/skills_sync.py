...
Copies bundled skills from the repo's skills/ directory into the active
...
The manifest lives at <agent-home>/skills/.bundled_manifest.
"""
...
def _get_bundled_dir() -> Path:
    ...
    Checks fork-native bundled-skill env aliases first (set by package
    wrappers), then a wheel-installed data dir, then falls back to the relative
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> <agent-home>/skills/mlops/axolotl
    """
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `superforecasting-agent skills reset {skill_name}` "
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `superforecasting-agent skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        action = "manifest_cleared"
        message = (
            f"Cleared manifest entry for '{name}'. Future `superforecasting-agent update` runs "
