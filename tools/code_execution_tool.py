...
# available on every platform Superforecasting Agent itself runs on.
...
_TOOL_STUBS = {
    ...
    "search_files": (
        ...
        '"""Search file contents (target="content") or find files by name (target="files"). Returns dict with densified "matches_text" (str) and "matches_format" describing its shape."""',
...
def execute_code(
    ...
    try:
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.superforecasting-agent/.env')).
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (the agent runtime's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    if mode == "strict":
        cwd_note = (
            ...
            "(os.path.expanduser('~/.superforecasting-agent/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
