...
Backend is selected during ``superforecasting-agent tools`` setup (web.backend in config.yaml).
When available, Superforecasting Agent can route Firecrawl calls through a Nous-hosted tool-gateway
...
Content Handling (no LLM):
- web_extract returns clean scraped markdown directly (no auxiliary summarizer)
- Pages over web.extract_char_limit (default 15000) are head+tail truncated;
  the full clean text is stored under cache/web and the footer gives the
  read_file path + offset to page through the omitted middle
- Inline base64 images become [IMAGE: alt] placeholders (real image URLs kept)
...
async def web_crawl_tool(
    ...
    Returns clean per-page content (markdown/text) with NO LLM summarization —
    same truncate-and-store path as web_extract. Pages over ``char_limit`` are
    head+tail truncated with an explicit footer pointing at the stored full
    text; inline base64 images become ``[IMAGE: alt]`` placeholders.

    Args:
        url (str): The base URL to crawl (can include or exclude https://)
        instructions (str): Instructions for what to crawl/extract (optional)
    ...
    try:
        ...
        # post-processed by the shared truncate-and-store path below (clean
        # content + full-page cache + read_file offset; no LLM summarization).
        from agent.web_search_registry import (
        ...
        if crawl_provider is not None:
            ...
            # Truncate-and-store (no LLM): convert inline base64 images to
            # placeholders, then return clean per-page content directly if
            # within budget, or a head+tail window plus a footer pointing at
            # the stored full text. Same path as web_extract.
            effective_char_limit = char_limit if char_limit is not None else _get_extract_char_limit()
            try:
                effective_char_limit = max(2000, min(int(effective_char_limit), 500_000))
            except (TypeError, ValueError):
            ...
            for result in response.get("results", []):
                ...
                model_text, truncated = _truncate_with_footer(clean, page_url, effective_char_limit)
                result["content"] = model_text
                if truncated:
                    debug_call_data["pages_truncated"] += 1
                    debug_call_data["truncation_metrics"].append({
...
if __name__ == "__main__":
    ...
    print("")
    print("  # Extract and crawl (asynchronous, no LLM — truncate-and-store)")
