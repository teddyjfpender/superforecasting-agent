...
Output is saved as PNG under the active Superforecasting Agent home cache
(legacy ``HERMES_HOME`` is still accepted by the shared home resolver).
"""
...
class OpenAICodexImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "env_vars": [],
            "post_setup_hint": (
                "Sign in with `superforecasting-agent auth codex` "
                "(or `superforecasting-agent setup` → Codex) "
        ...
        if not _read_codex_access_token():
            return error_response(
                error=(
                    ...
                    "`superforecasting-agent auth codex` "
                    "(or `superforecasting-agent setup` → Codex) to sign in."
                ),
