...
```bash
superforecasting-agent memory setup    # select "supermemory"
```
...
echo 'SUPERMEMORY_API_KEY=***' >> ~/.superforecasting-agent/.env
```
...
|-----|---------|-------------|
| `container_tag` | `hermes` | Inherited compatibility default used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `forecast-{identity}` -> `forecast-coder`). |
...
When enabled, Superforecasting Agent can:
...
Use `{identity}` in the `container_tag` to scope memories per forecast profile:

```json
{
  "container_tag": "forecast-{identity}"
}
```

For a profile named `coder`, this resolves to `forecast-coder`. The default profile resolves to `forecast-default`. Without `{identity}`, all profiles share the same container.
