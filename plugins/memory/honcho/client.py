...
def resolve_config_path() -> Path:
    ...
    default_path = _get_default_agent_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Agent session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — agent session_id ({timestamp}_{hex})
        ...
        # per-session: inherit agent session_id (new Honcho session each run)
