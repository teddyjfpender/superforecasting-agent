...
prefers Superforecasting Agent-managed xAI Grok OAuth (via
``superforecasting-agent auth``) and falls back to ``XAI_API_KEY`` (resolved
through the active agent-home ``.env``, then ``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``superforecasting-agent tools`` repaint and at tool-registration time.
