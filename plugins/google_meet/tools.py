...
def _resolve_node_client(node: Optional[str]):
    ...
    if entry is None:
        raise RuntimeError(
            ...
            "run `superforecasting-agent meet node approve <name> <url> <token>` first"
        )
        ...
        "properties": {
            ...
            "node": {
                ...
                "description": (
                    ...
                    "approved via `superforecasting-agent meet node approve`."
                ),
            },
        },
        "required": ["url"],
    ...
    if client is not None:
        ...
        try:
            res = client.start_bot(
                url=url,
                guest_name=str(args.get("guest_name") or "Superforecasting Agent"),
