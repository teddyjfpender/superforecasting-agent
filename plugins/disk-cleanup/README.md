...
Auto-tracks and cleans up ephemeral files created during Superforecasting Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to the active Superforecasting Agent home,
`/tmp/superforecasting-agent-*`, `/tmp/forecast-*`, and legacy `/tmp/hermes-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside the active agent home, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under the active agent home | always | Never |
...
- `is_safe_path()` rejects anything outside the active agent home,
  `/tmp/superforecasting-agent-*`, `/tmp/forecast-*`, or legacy `/tmp/hermes-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `disk-cleanup/` under the active agent home is itself excluded
- `logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
