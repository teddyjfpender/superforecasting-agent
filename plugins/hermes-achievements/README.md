...
The screenshots use temporary demo tier data to show the full visual range. The plugin itself reads real local agent session history by default.
...
>
> **Share cards (2026-05-04, vendored with the inherited dashboard plugin):** Unlocked achievement cards now have a "Share" button that renders a 1200×630 PNG share card (client-side canvas, no backend, no network) with Download + Copy-to-clipboard actions. Fits X/Twitter, Discord, LinkedIn, Bluesky link-preview dimensions.

## What it does

Forecast Achievements scans local agent sessions and unlocks badges based on real behavior:

- autonomous tool chains
- debugging and recovery patterns
- vibe-coding file edits
- skills, memory, cron, and plugin usage
...
- **Secret** — hidden until the first related signal appears
...
- Skillsmith
- Memory Keeper
- Context Pressure Operator
- Plugin Builder
- Rabbit Hole Certified

## Install

Clone into your Superforecasting Agent plugins directory:

```bash
git clone https://github.com/PCinkusz/hermes-achievements ~/.superforecasting-agent/plugins/hermes-achievements
```
...
ln -s ~/hermes-achievements ~/.superforecasting-agent/plugins/hermes-achievements
```
...
If backend API routes 404, restart `superforecasting-agent dashboard`; plugin APIs are mounted at dashboard startup.
...
```bash
cd ~/.superforecasting-agent/plugins/hermes-achievements
git pull --ff-only
curl http://127.0.0.1:9119/api/dashboard/plugins/rescan
```

If the update changes backend routes or `plugin_api.py`, restart `superforecasting-agent dashboard` after pulling.
...
Achievement unlock state is stored locally in `state.json` and is not overwritten by git updates. New achievements are evaluated from your existing local session history. Achievement IDs are stable and should not be renamed casually because they are the unlock-state keys.
