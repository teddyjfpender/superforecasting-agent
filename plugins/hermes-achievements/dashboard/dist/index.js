(function () {
  ...
  async function buildShareImage(achievement) {
    ...
    ctx.fillText("SUPERFORECASTING AGENT  ·  superforecasting-agent.nousresearch.com", 70, H - 40);
  ...
  function ShareDialog({ achievement, onClose }) {
    ...
    function tweetText() {
      ...
      const tmpl = tx(t, "share.tweet_text", "Just unlocked {tier_part}\"{name}\" in Superforecasting Agent", {
        tier_part: tierPart,
        name: achievement.name,
      });
      return tmpl + "\n\n@NousResearch · https://superforecasting-agent.nousresearch.com";
    }
