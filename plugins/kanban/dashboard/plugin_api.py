...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``superforecasting-agent kanban
...
def list_diagnostics(
    ...
    AND for ``superforecasting-agent kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``superforecasting-agent kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``superforecasting-agent kanban specify <task_id>``.
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from active agent-home config.yaml.
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of active agent-home ``profiles/*`` on disk and every
...
def decompose_task_endpoint(
    ...
    1:1 to ``superforecasting-agent kanban decompose <task_id>``.
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update kanban orchestration knobs in active agent-home config.yaml.
