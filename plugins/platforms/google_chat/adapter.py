...
#     fresh "Superforecasting Agent is thinking…" card during the small window between
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # they don't sit in the chat forever as "Superforecasting Agent is thinking…".
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Superforecasting Agent is thinking…' marker message.
        ...
        "Superforecasting Agent is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Superforecasting Agent is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Superforecasting Agent is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Superforecasting Agent is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Superforecasting Agent is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``superforecasting-agent setup``.

    The setup wizard at ``superforecasting_agent/runtime/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to the active agent-home ``.env`` so the next
    gateway restart picks them up.
    """
    from superforecasting_agent.runtime.cli_output import (
        print_info,
        print_success,
        print_warning,
        prompt,
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. forecast-chat-events) and a Pull subscription.")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'superforecasting-agent[google_chat]'",
        ...
        # the "Superforecasting Agent is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Superforecasting Agent is thinking…' marker message "
