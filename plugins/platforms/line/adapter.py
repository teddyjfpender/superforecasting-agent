"""
LINE Messaging API platform adapter for Superforecasting Agent.
...
to Superforecasting Agent. It deliberately ports the *strongest* idea from each into
...
def is_connected(config) -> bool:
    """Surface in ``superforecasting-agent status`` before adapter init."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``superforecasting-agent status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``superforecasting-agent setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to the active agent-home ``.env`` via ``superforecasting_agent.runtime.config``.
    """
    print()
    ...
    try:
        from superforecasting_agent.runtime.config import get_env_var, set_env_var
    except ImportError:
        print("superforecasting_agent.runtime.config not available; set LINE_* vars manually in the active agent-home .env")
        return
