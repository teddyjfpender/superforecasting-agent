...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = tuiEnvValue('TRUECOLOR', env)
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep the forecast desk on the safer TERM-driven 256-color
  // path unless users explicitly opt back in via a TUI_TRUECOLOR alias.
