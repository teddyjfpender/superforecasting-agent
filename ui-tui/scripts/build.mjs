...
await build({
  ...
  // Skip the prebuilt @superforecasting/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@superforecasting/ink': resolve(root, 'packages/forecast-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The superforecasting_agent.runtime launcher
