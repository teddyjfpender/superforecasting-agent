"""Lightweight file-drop / attachment-path detection helpers.

These are pulled out of the 15k-line ``cli.py`` (which transitively imports
prompt_toolkit / fire / rich) so the gateway's hot ``input.detect_drop`` RPC
handler — which runs on the serial main dispatch thread on EVERY message —
can detect a dropped path WITHOUT dragging cli.py's import weight onto the
fast path. ``cli.py`` re-exports these names so its public API is unchanged.

Pure stdlib (os / pathlib / urllib) — no heavy imports, no side effects.
"""
