...
def _print_setup_summary(config: dict, hermes_home):
    ...
    # Spotify (OAuth via the auth command — check auth.json, not env vars)
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                f"Direct OpenAI credentials are still configured and may take precedence until removed from {display_hermes_home()}/.env."
            )
