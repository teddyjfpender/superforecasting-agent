"""CLI subcommand: ``superforecasting-agent send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"superforecasting-agent send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"superforecasting-agent send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"superforecasting-agent send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"superforecasting-agent send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"superforecasting-agent send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `superforecasting-agent gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.superforecasting-agent/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hermes_env() -> None:
    """Populate ``os.environ`` from the agent ``.env`` AND bridge top-level
    ...
    startup that ``superforecasting-agent send`` must replicate when invoked
    standalone:

    1. ``load_dotenv(<agent-home>/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``<agent-home>/config.yaml`` into
       ...
       via ``superforecasting-agent config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so
    ``superforecasting-agent send`` doesn't pull in the full gateway module
    just to resolve a home channel.
    """
    # Step 1: dotenv
    try:
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge the agent .env and config.yaml into os.environ so the gateway
    # config loader (invoked downstream by send_message_tool and by the
    # channel directory) can see platform credentials and home channels.
    _load_hermes_env()
    ...
    if not target:
        print(
            "superforecasting-agent send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  superforecasting-agent send --to telegram \"hello\"\n"
            "  superforecasting-agent send --to discord:#ops --file report.md\n"
            "  superforecasting-agent send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "superforecasting-agent send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `superforecasting-agent send --help` stays fast and
    # does not pull in the full tool registry / gateway config stack.
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform "
            "Superforecasting Agent is already configured for. Reuses the "
            "gateway's platform credentials (~/.superforecasting-agent/.env + "
            "~/.superforecasting-agent/config.yaml) — no LLM, no agent loop, "
            "no running gateway required for bot-token platforms like "
            "Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  superforecasting-agent send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | superforecasting-agent send --to telegram:-1001234567890\n"
            "  superforecasting-agent send --to discord:#ops --file /tmp/report.md\n"
            "  superforecasting-agent send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  superforecasting-agent send --list                  # all platforms\n"
            "  superforecasting-agent send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `superforecasting-agent send --list telegram`.",
    )
