...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Superforecasting Agent OAuth credentials."""

    auth_hint = "superforecasting-agent auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`superforecasting-agent auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`superforecasting-agent auth reset xai-oauth` or re-authenticate with "
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `superforecasting-agent auth add xai-oauth --type oauth`."
            )
