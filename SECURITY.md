# Superforecasting Agent Security Policy

This document describes Superforecasting Agent's trust model, names the
one security boundary the project treats as load-bearing, and defines
the scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via
[GitHub Security Advisories](https://github.com/teddyjfpender/superforecasting-agent/security/advisories/new).
Do not open public issues for security vulnerabilities. **Superforecasting
Agent does not operate a bug bounty program.**
...
- Environment details (`superforecasting-agent --version`, commit SHA,
  OS, Python version).
...
Superforecasting Agent is a single-tenant personal forecasting agent.
Its posture is layered, and the layers are not equally load-bearing.
Reporters and operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Superforecasting
  Agent, including any Python modules it has loaded (skills, plugins,
  hook handlers).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Superforecasting Agent access to by running it — typically,
  whatever the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Superforecasting Agent's
  documentation or code about how a consuming layer (adapter, UI,
  file writer, shell) should treat agent output — e.g. "the dashboard
  renders agent output as inert HTML."
...
Superforecasting Agent supports two OS-level isolation postures. They
address different threats and an operator should choose deliberately.
...
Superforecasting Agent supports this in two ways:

- **Superforecasting Agent's Docker image and Compose setup.** Lighter
...
Under a whole-process wrapper, Superforecasting Agent's in-process
heuristics (§2.4) function as accident-prevention layered on top of a
real boundary. This is the supported posture when the agent ingests
...
Superforecasting Agent filters the environment it passes to its
lower-trust in-process components: shell subprocesses, MCP
subprocesses, and the code-execution child. Credentials like provider
API keys and gateway tokens are stripped by default; variables
explicitly declared by the operator or by a loaded skill are passed
through.
...
A malicious or buggy plugin is not a vulnerability in Superforecasting
Agent itself. Bugs in Superforecasting Agent's plugin-install or
plugin-discovery path that prevent the operator from seeing what
they're installing are in scope under §3.1.
...
**Surfaces in Superforecasting Agent:**
...
4. **Within the authorized set, all callers are equally trusted.**
   Superforecasting Agent does not model per-caller capabilities
   inside a single adapter. Operators who need capability separation
   should run separate agent instances with separate allowlists.
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Superforecasting Agent's own documentation, or
  reasonable operator expectations would predict — including cases
  where Superforecasting Agent has documented a stance about how its
  output should be rendered by a consuming layer (dashboard, gateway
  adapter, file writer, shell) and a code path breaks that stance.
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  agent-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Superforecasting
  Agent's trust surface (§2.4, §2.5). A skill or plugin doing
  something malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Superforecasting Agent. Bugs in
  Superforecasting Agent's skill-install or plugin-install path that
  prevent the operator from seeing what they're installing are in
  scope under §3.1.
...
- Superforecasting Agent includes supply-chain guards for MCP server
